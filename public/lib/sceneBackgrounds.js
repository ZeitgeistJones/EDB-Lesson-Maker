...
(function () {
  ...
  const TOPIC_SETS = [
    ...
    // Kitchen / cooking BEFORE home — bakery-warm flats (corner motifs), not
    // anonymous house washes (Kitchen Helpers generalization miss).
    { re: /\b(kitchen|cooking|cook(?:ery)?|baking|bakery|helpers?)\b/, set: 'bakery-warm' },
    { re: /\b(home|house|family|apartment|bedroom|hotel|bathroom|shower|bath|toilet)\b/, set: 'home-warm' },
    // Soccer / pitch sports BEFORE gym-cool — gym-cool flats are basketball-court
    // washes (Manus + teacher: "Soccer Time" must not look like basketball).
    // Outdoor-fresh = green-field family; less wrong than indoor hoop lanes.
    // Soccer / pitch — dedicated soccer-fresh washes (not park picnic / zoo / camp).
    // Outdoor-fresh stays for park/zoo/camp; soccer must read as a pitch.
    { re: /\b(soccer|football|goalkeeper|kickoff|pitch|fifa|striker|goalie)\b/, set: 'soccer-fresh' },
    // Indoor gym / basketball court washes — not park meadow, not soccer pitch.
    { re: /\b(gym|workout|athletic|basketball|fitness|hoops?)\b/, set: 'gym-cool' },
    // Bare "sports" / games / racket words — gym-cool washes (not anonymous house).
    // Soccer/pitch still wins above; basketball/gym already on gym-cool.
    { re: /\b(sports?|sporty|games?|tennis|baseball|volleyball)\b/, set: 'gym-cool' },
    ...
    // Marine fauna BEFORE pool `\bswim\b` and outdoor `\banimal\b` —
    // "Sea Animals" + vocab swim must not become chlorine tile or picnic.
    { re: /\b(sea\s*animals?|ocean\s*animals?|marine|underwater|sharks?|whales?|dolphins?|orcas?|jellyfish|octopuses?|octopi|stingrays?|manatees?|walr(?:us|uses)|seals?|penguins?)\b/, set: 'aquarium-cool' },
    ...
    // Circus / carnival before outdoor — big-top wash, not board-house default.
    { re: /\b(circus|clowns?|acrobats?|trapeze|carnival|big\s*top)\b/, set: 'outdoor-fresh' },
    // Space before outdoor / classical — night flats, not board-house cream.
    { re: /\b(space|spaceships?|spacecraft|rockets?|astronauts?|planets?|orbit|galaxy|nasa|mars|lunar)\b/, set: 'space-cool' },
    // Volcano / geology BEFORE outdoor-fresh — picnic/zoo grass is a false friend
    // (Living by the volcano must not look like a park day).
    { re: /\b(volcanoes?|volcanic|lava|eruptions?|craters?|magma|ash|geothermal|seismic)\b/, set: 'volcano-cool' },
    { re: /\b(zoo|park|animal|forest|garden|nature|trampoline|farm|campsites?|camp(?:ing|fire)?)\b/, set: 'outdoor-fresh' },
  ...
  const PLACE_SIGNALS = [
    ...
    /\b(circus|clowns?|acrobats?|trapeze|carnival|big\s*top)\b/,
    /\b(aquariums?|fish\s*tanks?)\b/,
    /\b(sea\s*animals?|ocean\s*animals?|marine|underwater|sharks?|whales?|dolphins?)\b/,
    ...
    /\b(space|spaceships?|spacecraft|rockets?|astronauts?|planets?|orbit|galaxy|nasa|mars|lunar)\b/,
  ];
  ...
  const TOPIC_PALETTE = [
    ...
    { re: /\b(sea\s*animals?|ocean\s*animals?|marine|underwater|sharks?|whales?|dolphins?)\b/, want: ['cool', 'coast', 'neutral'] },
    { re: /\b(swimming\s*pools?|pools?|swim(?:ming)?)\b/, want: ['cool', 'coast', 'neutral'] },
    { re: /\b(beach|ocean|sea|shore)\b/, want: ['coast', 'outdoor', 'cool'] },
    { re: /\b(volcano|lava|eruption|crater|ash|seismic|geothermal)\b/, want: ['warm', 'cool', 'outdoor'] },
    ...
    { re: /\b(circus|clowns?|acrobats?|trapeze|carnival|big\s*top)\b/, want: ['outdoor', 'warm', 'cool'] },
    // "Living in…" titles must not steal the home palette over a real place.
    { re: /\b(home|house|family|apartment|bedroom)\b/, want: ['warm', 'neutral'] },
    { re: /\b(kitchen|cooking|baking|bakery|helpers?)\b/, want: ['warm', 'neutral'] },
  ...
  /**
   * Ball-family lesson words (dock cards / oddOneOut options). When present,
   * flats that paint a ball as chromeMotif read as a second unlabeled ball.
   * Soccer/pitch lessons likewise must not land picnic-blanket chrome.
   */
  function topicHasBallFamily(topicWords) {
    const text = ' ' + String(
      Array.isArray(topicWords) ? topicWords.join(' ') : (topicWords || '')
    ).toLowerCase() + ' ';
    return /\b(ball|basketballs?|soccer[\s-]?balls?|footballs?|tennis[\s-]?balls?)\b/.test(text);
  }

  function topicIsSoccerPitch(topicWords) {
    const text = ' ' + String(
      Array.isArray(topicWords) ? topicWords.join(' ') : (topicWords || '')
    ).toLowerCase() + ' ';
    return /\b(soccer|football|goalkeeper|kickoff|pitch|fifa|striker|goalie)\b/.test(text);
  }

  function flatChromeMotifs(entry) {
    const raw = entry && entry.chromeMotifs;
    if (!Array.isArray(raw)) return [];
    return raw.map((m) => String(m || '').trim().toLowerCase()).filter(Boolean);
  }

  /** Motifs that conflict with the current topic (ball chrome, picnic-on-pitch, …). */
  function conflictMotifsForTopic(topicWords) {
    const out = [];
    if (topicHasBallFamily(topicWords)) out.push('ball');
    if (topicIsSoccerPitch(topicWords)) out.push('picnic');
    return out;
  }

  /** Drop flats whose painted chrome conflicts with topic dock words / sport register. */
  function filterChromeConflicts(keys, m, topicWords) {
    if (!keys || !keys.length) return keys || [];
    const avoid = conflictMotifsForTopic(topicWords);
    if (!avoid.length) return keys;
    const filtered = keys.filter((k) => {
      const motifs = flatChromeMotifs(m.flats[k]);
      return !avoid.some((a) => motifs.includes(a));
    });
    // Keep the set usable if every panel declares a conflicting motif.
    return filtered.length >= 2 ? filtered : keys;
  }
  ...
  function pickFlat(m, index, seed, reason, moods, topicWords, lockedSet, pin) {
    ...
    if (wantSet) {
      let setKeys = all
        ...
        .sort();
      setKeys = filterChromeConflicts(setKeys, m, topicWords || seed);
      if (setKeys.length >= 2) {
        let key;
        ...
        if (pin === 'open') key = setKeys[0];
          ...
          midPool = filterChromeConflicts(midPool, m, topicWords || seed).slice(0, 2);
          if (!midPool.length) midPool = setKeys.slice(0, 2);
          key = midPool[(flatOffset(seed, midPool.length) + (index || 0)) % midPool.length];
        }
        const avoid = conflictMotifsForTopic(topicWords || seed);
        const avoided = avoid.length > 0
          && !avoid.some((a) => flatChromeMotifs(m.flats[key] || {}).includes(a))
          && all.some((k) => {
            if (m.flats[k].set !== wantSet) return false;
            return avoid.some((a) => flatChromeMotifs(m.flats[k]).includes(a));
          });
        return {
          type: 'flat',
          name: key,
          ...
          reason: reason + ` · set:${wantSet}` + (pin ? ` · pin:${pin}` : '')
            + (avoided ? ` · avoid:${avoid.join('+')}-chrome` : ''),
        };
      }
    }
  ...
  /**
   * Page-type → background density (Structure ↔ Background contract).
   * quiet   — mid chrome / vocab / reading: house panel when a place set is locked
   * expressive — cover / story: full place wash OK
   * work    — activity / king stage: place-set work surface (not random house)
   * close   — wrap: place-set end panel
   */
  const PAGE_DENSITY = {
    title: 'expressive',
    cover: 'expressive',
  ...
  function densityClassFor(section) {
    const tags = (section && section.tags) || [];
    for (let i = 0; i < tags.length; i++) {
      const t = String(tags[i] || '').toLowerCase();
      if (PAGE_DENSITY[t]) return PAGE_DENSITY[t];
      if (/^story/.test(t)) return 'expressive';
    }
    const pt = String((section && section.pageType) || '').toLowerCase();
    if (PAGE_DENSITY[pt]) return PAGE_DENSITY[pt];
    if (section && section.preferFlat) return 'quiet';
    return 'expressive';
  }

  /** Title / story / activity / wrap keep the place wash; drill pages may go house. */
  function isPlaceBookend(section) {
    const d = densityClassFor(section);
    return d === 'expressive' || d === 'work' || d === 'close';
  }
  ...
  async function pickFor(section, opts = {}) {
    ...
    const pin = opts.forcePin
      || (secTags.includes('title') ? 'open' : (secTags.includes('wrap') ? 'close' : null));
  ...
  async function planFor(sections, opts = {}) {
    ...
    const brief = opts.topicBrief
      || (opts.lesson && window.TopicIdentity && window.TopicIdentity.ensureBrief
        ? window.TopicIdentity.ensureBrief(opts.lesson)
        : null)
      || null;
    const briefPrefer = brief && window.TopicIdentity && window.TopicIdentity.preferredBgSet
      ? window.TopicIdentity.preferredBgSet(brief)
      : null;
    const setMatch = briefPrefer || setFor(topicWords);
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      // Density table: quiet mid chrome → one house panel; bookends keep place.
      // forcePin open on quiet → always house-a so the mid band does not explode.
      const density = densityClassFor(sec);
      const chromeNeutral = !!(density === 'quiet' && lockedSet !== DEFAULT_SET);
      let sectionSet = chromeNeutral ? DEFAULT_SET : lockedSet;
      // Topic Identity: avoid parent-only sets on non-env pages when prefer set exists.
      if (
        brief
        && !chromeNeutral
        && window.TopicIdentity
        && typeof window.TopicIdentity.scoreAsset === 'function'
      ) {
        ...
        if (!scored.ok && briefPrefer) sectionSet = briefPrefer;
      }
      const forcePin = chromeNeutral
        ? 'open'
        : (density === 'close' ? 'close' : undefined);
      const p = await pickFor(sec, {
        ...opts,
        index: flatCount,
        moods,
        topicWords,
        lockedSet: sectionSet,
        forcePin,
      });
      if (p && typeof p === 'object') p.density = density;
  ...
  window.SceneBackgrounds = {
    ...
    densityClassFor, isPlaceBookend, PAGE_DENSITY,
