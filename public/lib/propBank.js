...
(function () {
  ...
  /**
   * Title charm displays ~400px — soft hero-sheet splices that barely clear
   * MIN_DOCK_SRC still look mushy there. Require a higher short-side floor.
   */
  const MIN_TITLE_CHARM_SRC = 240;
  ...
  function senseNeedsFor(word) {
    ...
    const key = slug(word);
    let entry = policy.senses[key];
    // Plurals share the singular sense lock (games/games → video-controller art).
    if ((!entry || !Array.isArray(entry.needs) || !entry.needs.length)
      && key.length > 3 && key.endsWith('s') && !key.endsWith('ss')) {
      entry = policy.senses[key.slice(0, -1)];
    }
  ...
  const PROP_ALIASES = {
    ...
    piata: 'party-pinata',
    pinata: 'party-pinata',
    // Kitchen helpers — densify kitchen-whisk / kitchen-apron are dockSafe:false
    // white-plates; pin sharp bakery/cooking cutouts so New Words stay pictured.
    whisk: 'bake-whisk',
    apron: 'bake-apron',
    // spatula/blender densify keys stay dockSafe:false (C6) until Manus object sheet;
    // aliases kept so resolve still prefers them over random kit-* when they clear.
    spatula: 'kitchen-spatula',
    blender: 'kitchen-blender',
    grater: 'kitchen-grater',
    timer: 'kitchen-timer',
    spoon: 'wooden-spoon',
    bowl: 'mixing-bowl',
    // Playground — title/wrap tableau need alpha cutouts, not vocab-pack white plates.
    slide: 'park-slide',
    swing: 'park-swing-set',
    seesaw: 'park-seesaw',
    sandbox: 'park-sandbox',
    tent: 'hike-tent',
    campfire: 'camp-campfire',
    ...
    // Bare "smoke" is volcanic/air plume — never pin the fire-station detector
    // (reads as a loading spinner at dock size). Detector only for explicit words.
    'smoke-detector': 'fire-smoke-detector',
    smokedetector: 'fire-smoke-detector',
  ...
  const MATTE_HINTS = [
    ...
    // Food / market ESL — win over incidental "trip" in titles like
    // "Fruit Market Bus Trip" so matte apple/bus cutouts stay in the pool.
    'fruit', 'food', 'market', 'grocery', 'vegetable', 'farm', 'supermarket',
  ];
  ...
  // Bare "trip" is too broad (bus trip / field trip) — keep travel/journey/voyage.
  const GLOSSY_HINTS = [
    'adventure', 'quest', 'pirate', 'treasure', 'magic', 'wizard',
    'explorer', 'explore', 'jungle',
    'island', 'safari', 'journey', 'voyage',
    'travel', 'holiday', 'vacation', 'airport', 'flight', 'flying',
    'tourist', 'suitcase',
  ];
  ...
  function index(raw) {
    ...
    for (const key of Object.keys(props).sort()) {
      ...
      const prop = {
        ...
        // Optional packs[] = multi-kit membership (e.g. bath-sink bathroom+kitchen).
        pack: row.pack
          ? String(row.pack).toLowerCase()
          : (Array.isArray(row.packs) && row.packs[0] ? String(row.packs[0]).toLowerCase() : null),
        packs: Array.isArray(row.packs) && row.packs.length
          ? [...new Set(row.packs.map((p) => String(p).toLowerCase()).filter(Boolean))]
          : (row.pack ? [String(row.pack).toLowerCase()] : []),
        ...
        // false = residual white plate / bad densify key — keep in bank, ban docks.
        dockSafe: row.dockSafe === false ? false : true,
  ...
  function byWord(pool, q) {
    ...
    // Prefer dock-safe members: densify -v2 / kit-* white-plates must not win
    // the rotate slot and then fail VocabArt isDockSharp (Kitchen Helpers:
    // spatula/blender text-only while grater/timer pictured).
    const pick = (hit) => {
      if (!hit) return null;
      if (hit.key === key && isVariantProp(hit)) return hit;
      const band = variantBand(pool, hit);
      if (band.length <= 1) return hit;
      const safe = band.filter((p) => p && p.dockSafe !== false);
      const use = safe.length ? safe : band;
      ...
      return rotatePick(use, (seed ? seed + '|' : '') + key, 0);
    };

    let hit = find(key);
    // Manus gap imports often bank as s60-<word>; bare ESL words must still resolve.
    if (!hit && key && !key.startsWith('s60-')) {
      hit = find('s60-' + key);
    }
    let viaAlias = false;
    if (!hit) {
    ...
    // Policy aliases are deliberate (brush→paintbrush tool, etc.) — do not
    // reopen them into a *-token peer band (arch-brush would steal dental brush).
    if (viaAlias) return pick(hit);
    ...
    const pickThemedBand = (band, tight) => {
      ...
      // resolveTheme packs (when caller passes preferredPacks) beat incidental
      // seed tokens — tent→circus-tent on a circus lesson, not camping tent.
      const preferred = new Set(
        (q && q.preferredPacks ? q.preferredPacks : [])
          .map((p) => slug(p))
          .filter(Boolean)
      );
      const sportBallQuery = rawWord && /^(balls?)$/.test(slug(rawWord));
      const basketballCue = [...theme].some((t) =>
        /^(basketball|court|hoop|hoops|nba)$/.test(t)
      );
      const soccerCue = [...theme].some((t) =>
        /^(soccer|football|goalkeeper|pitch|fifa)$/.test(t)
      );
      const themeScore = (p) => {
        let s = 0;
        ...
        for (const pk of membershipPacks(p)) {
          if (theme.has(slug(pk))) s += 2;
          if (preferred.has(slug(pk))) s += 10;
        }
        // Suffix / exact identity beats pack-head-only (city-gym > gym-barbell
        // for query "gym"; circus-tent stays both suffix+pack for "tent").
        if (rawWord) {
          const tok = slug(rawWord);
          if (tok && (p.key === tok || p.key.endsWith('-' + tok))) s += 4;
        }
        // White-plate / failed densify cutouts: keep them out of the winner
        // even when preferredPacks would otherwise prefer kitchen-whisk over
        // bake-whisk (Kitchen Helpers New Words text-only miss).
        if (p.dockSafe === false) s -= 50;
        // Polyseme "ball": prefer the sport named in the lesson over yarn /
        // cotton / generic *-ball tag ties (story caption "…the ball").
        if (sportBallQuery) {
          const k = String(p.key || '');
          if (basketballCue && /basketball/.test(k)) s += 8;
          else if (soccerCue && /soccer/.test(k)) s += 8;
          else if ((basketballCue || soccerCue) && /yarn|cotton|disco|foil|circus/.test(k)) s -= 12;
        }
        return s;
      };
      band.sort(
    ...
    // Head-noun / pack-suffix / pack-head identity peers.
    const identityPrefixed = (token) =>
      pool.filter((p) => !isNeverProp(rawWord, p) && identityHit(p, token));

    // Exact key (tent) must not skip themed peers (circus-tent, hike-tent).
    // Merge into a band and let theme / preferredPacks pick.
    const earlyPeers = identityPrefixed(key);
    if (hit || earlyPeers.length) {
      const band = [];
      const seen = new Set();
      if (hit) {
        band.push(hit);
      ...
      for (const p of earlyPeers) {
        if (seen.has(p.key)) continue;
        seen.add(p.key);
        band.push(p);
      }
      if (band.length > 1) return pickThemedBand(band, true);
      if (band.length === 1) return pick(band[0]);
    }
    ...
    if (merged.length > 1) {
      const tight = merged.filter((p) =>
        tokens.some((t) => {
          if (p.key === t || p.key.endsWith('-' + t)) return true;
          // Compound sport balls (basketball) are tight for query "ball" even
          // though the key does not end with the hyphen suffix "-ball".
          if (
            (t === 'ball' || t === 'balls')
            && /(?:basket|soccer|tennis|base|volley)ball/.test(String(p.key || '').replace(/-/g, ''))
          ) {
            return true;
          }
          return false;
  ...
  /**
   * Pack ids a prop belongs to (multi-kit via packs[], else singular pack).
   */
  function membershipPacks(prop) {
    if (!prop) return [];
    if (Array.isArray(prop.packs) && prop.packs.length) return prop.packs;
    return prop.pack ? [prop.pack] : [];
  }
   ...
   * Pack-head: word "circus" / "cafe" / "farm" qualifies keys in that pack
   * that start with "<token>-" (circus-tent, cafe-counter) — shared rule so
   * place nouns pick their kit without per-word aliases.
   */
  function identityHit(prop, token) {
    ...
    // Compound sport balls are one orthographic word (basketball), so they do
    // not end with "-ball" — still identity-match bare ESL "ball".
    if (token === 'ball' || token === 'balls') {
      const compact = String(prop.key || '').replace(/-/g, '');
      if (/(?:basket|soccer|tennis|base|volley)ball/.test(compact)) return true;
    }
    // Pack-head: token names the pack and the key is pack-prefixed.
    if (token.length >= 3 && prop.key && prop.key.startsWith(token + '-')) {
      const packs = membershipPacks(prop);
      if (packs.includes(token)) return true;
    }
    return false;
  }
  ...
  const KIT_STOP = new Set([
    ...
    // Recipe chrome — "Drag the players onto the stage" must not score every
    // hero tagged stage / steal packs via "players" (basketball analyze).
    'stage', 'stages', 'player', 'players',
  ]);
  ...
  const WEAK_KIT_TOKENS = new Set([
    ...
    'music',
    // Reward / celebrity "star" must not unlock the space kit via kenney-star_* tags
    // ("The Cat Ate My Homework" + star → spaceship cockpit title charm).
    'star', 'stars',
    // Bare "ball"/"hoop" must not Ready playground via playground-ball /
    // park-basketball-hoop suffix hits when the lesson is basketball/sports
    // (empty → sortBins beats a playground-slide king).
    'ball', 'balls', 'hoop', 'hoops',
  ]);
  ...
  /** Roles allowed on a kit dock (face dressParts + timers count). */
  function isKitDockRole(p) {
    const r = p && p.role;
    return !r || r === 'object' || r === 'tool' || r === 'dressPart'
      || r === 'playPart' || r === 'timer';
  }
  ...
  function isDockSharp(p) {
    if (!p) return false;
    // White-plate / failed densify cutouts stay in the bank for regen but
    // must never ship on roleplay docks (Sports Arena white boxes).
    if (p.dockSafe === false) return false;
    const w = p.srcW;
    const h = p.srcH;
  ...
  /** True when a prop is sharp enough to ship as a ~400px title-page charm. */
  function isTitleCharmSharp(p) {
    if (!p) return false;
    if (p.dockSafe === false) return false;
    const w = p.srcW;
    const h = p.srcH;
    if (!(w > 0) || !(h > 0)) return false;
    return Math.min(w, h) >= MIN_TITLE_CHARM_SRC;
  }
  ...
  function assessKit(lesson) {
    if (!bank) return null;
    // Theme lock: one lesson theme up front. none / empty packs → no ready kit
    // (sortBins / no title charm). Stops pack-tag Whac-A-Mole.
    const LT = window.LessonTraits;
    const theme = LT && typeof LT.resolveTheme === 'function' ? LT.resolveTheme(lesson) : null;
    if (theme && (theme.id === 'none' || !theme.packs || !theme.packs.length)) {
      return null;
    }
    const TI = window.TopicIdentity;
    const brief = TI && typeof TI.ensureBrief === 'function' ? TI.ensureBrief(lesson) : null;
    const allowedPacks = theme && theme.packs && theme.packs.length
      ? new Set(theme.packs)
      : null;
    ...
    for (const p of pool) {
      ...
      // as primary identity (farm-barn on beekeeping).
      if (brief && TI && typeof TI.scoreAsset === 'function') {
        ...
        if (scored.role === 'forbidden') continue;
        if (scored.role === 'secondary' && !scored.ok) continue;
      }
      for (const pack of packs) {
        if (allowedPacks && !allowedPacks.has(pack)) continue;
        // Decorative/character packs (gashapon toy apples, feeling faces) must not
        // win a king stage just because vocab words collide with prize keys
        // (Manus Y737: fruit-market → gashapon-machine after cafe/farm skip).
        if (isDecorativeProp(p) && !decoOK.has(pack)) continue;
        if (!byPack.has(pack)) byPack.set(pack, []);
        byPack.get(pack).push(p);
      }
    }

    // Theme-pinned heroes (wash-up → bath-sink, dental open-mouth, …) join the
    // allow-listed pack even when the PNG's primary pack tag differs — so ready
    // kits match heroKey without dual-pack Whac-A-Mole on cooking lessons.
    if (theme && theme.heroKey && allowedPacks && allowedPacks.size) {
      const pinned = get(theme.heroKey);
      if (pinned && isHeroSized(pinned)) {
        for (const pack of allowedPacks) {
          if (!byPack.has(pack)) byPack.set(pack, []);
          const list = byPack.get(pack);
          if (!list.some((p) => p.key === pinned.key)) list.push(pinned);
        }
      }
    }
    ...
    for (const [pack, members] of byPack) {
      ...
      // Prefer theme.heroKey when it is a member of this pack (dental/face/
      // trampoline pins must beat alphabetical / scale ties).
      let heroes = members.filter(isHeroSized).sort(
        (a, b) => (b.relativeScale || 0) - (a.relativeScale || 0) || a.key.localeCompare(b.key)
      );
      if (theme && theme.heroKey) {
        const pinned = members.find((p) => p.key === theme.heroKey && isHeroSized(p));
        if (pinned) heroes = [pinned, ...heroes.filter((h) => h.key !== pinned.key)];
      }
      const hero = heroes[0] || null;
      const dockPool = members
        .filter((p) => hero && p.key !== hero.key && isKitDockRole(p));
      ...
      // Theme-locked lessons already passed resolveTheme — pack allow-list is
      // enough; still require a hero + docks for ready.
      const themeOk = allowedPacks
        ? !!hero
        : (score >= 8 && (packHit || memberHits >= 2));
      // When theme-locked, prefer the pack's best hero even if token score is low
      // (title said "At the Cafe" — cafe pack is already the chosen theme).
      let adj = allowedPacks ? Math.max(score, 12) : score;
      // Dentist lessons share brush/clean/tooth tags with bathroom — demote bath
      // packs and boost dental so title charm is not a bathtub (Manus mfLN).
      // (Belt-and-suspenders when LessonTraits is cold / allowedPacks unset.)
      const dentalTok = tokens.some((t) => /^(dentist|dental|floss|cavity)$/.test(t));
      if (!allowedPacks && dentalTok && pack === 'bathroom') adj -= 24;
      if (!allowedPacks && dentalTok && (pack === 'dental' || pack === 'dentist')) adj += 12;
      if (!allowedPacks) {
        const produceTok = tokens.some((t) =>
          /^(apple|banana|carrot|tomato|lemon|grape|fruit|fruits|market|veggie|veggies|vegetable|vegetables|produce)$/.test(t)
        );
        const cafeCue = tokens.some((t) =>
          /^(cafe|cafes|coffee|bakery|pastry|latte|croissant|muffin|diner|restaurant)$/.test(t)
        );
        const farmCue = tokens.some((t) =>
          /^(farm|farms|barn|barns|tractor|scarecrow|hay|cow|chicken|pig|sheep)$/.test(t)
        );
        const spaceCue = tokens.some((t) =>
          /^(space|spaceship|spacecraft|rocket|astronaut|alien|planet|planets|nasa|orbit|galaxy|nebula|satellite|mars|lunar|moon)$/.test(t)
        );
        const schoolPetTok = tokens.some((t) =>
          /^(homework|school|classroom|teacher|student|cat|dog|pet|pets|kitten|puppy|milk)$/.test(t)
        );
        const sportTok = tokens.some((t) =>
          /^(sport|sports|basketball|soccer|football|tennis|baseball|volleyball|gym|athletic|athletics|team|teams|score|court|courts|hoop|hoops|coach|whistle|goalkeeper|kickoff|pitch)$/.test(t)
        );
        const playgroundCue = tokens.some((t) =>
          /^(playground|playgrounds|slide|slides|swing|swings|seesaw|sandbox|recess|monkey|climber|carousel)$/.test(t)
        );
        if (produceTok && !cafeCue && pack === 'cafe') continue;
        if (produceTok && !farmCue && pack === 'farm') continue;
        if (pack === 'space' && !spaceCue) continue;
        if (pack === 'farm' && schoolPetTok && !farmCue) continue;
        if (pack === 'cafe' && schoolPetTok && !cafeCue) continue;
        if (pack === 'playground' && sportTok && !playgroundCue) continue;
        // Belt-and-suspenders: music must not outscore sports when theme lock
        // is cold (Sports and Games → flute title charm).
        if (pack === 'music' && sportTok && !tokens.some((t) =>
          /^(music|compose|composer|orchestra|symphony|concert|classical|piano|violin|flute)$/.test(t)
        )) continue;
      }
      const ready = !!(hero && docks.length >= 6 && themeOk && adj >= 8);
      ...
      if (ready) {
        if (!best || adj > best.score || (adj === best.score && docks.length > best.dockCount)) {
          best = candidate;
        }
        ...
        if (
          !near
          || adj > near.score
          || (adj === near.score && docks.length > near.dockCount)
        ) {
          near = candidate;
        }
      }
