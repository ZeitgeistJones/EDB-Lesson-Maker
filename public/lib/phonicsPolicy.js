 ...
 *   Pre-A1 listen/say/trace only, 2–3 boxes, letter-sound exposure
...
(function (root, factory) {
  ...
  const LEVEL_RULES = {
    'PRE-A1': {
      ...
      scriptHint: 'Listen, point, say the sounds, then trace.',
    },
    A1: {
      minBoxes: 3,
      maxBoxes: 3,
  ...
  const FALLBACK_BANK = {
    'PRE-A1': [
      { word: 'up', graphemes: ['u', 'p'], emoji: '⬆️' },
      { word: 'in', graphemes: ['i', 'n'], emoji: '📥' },
      { word: 'on', graphemes: ['o', 'n'], emoji: '🔛' },
      { word: 'go', graphemes: ['g', 'o'], emoji: '🟢' },
  ...
  function normalize(lesson, meta) {
    ...
    // Manus Ehp2 (face) / S4DZ (castle): meta.phonics=on must NOT invent a pure
    // cat/dog page when no taught-vocab word is legal at this CEFR. Force only
    // means "don't omit for level alone (C1+)" — still require a vocab hit.
    // 1) Always admit bank rows that overlap lesson vocab (topic-tied CVC).
    // 2) Pure off-topic bank fillers only to pad AFTER a vocab hit exists.
    ...
    for (const row of bank) {
      ...
      if (!vocabSet.has(row.word)) continue;
      candidates.push({
        word: row.word,
        graphemes: row.graphemes.slice(),
        boxCount: row.graphemes.length,
        emoji: row.emoji || '🔤',
        topicRelevance: 'Decodable practice word from lesson vocab',
        source: 'vocab-bank',
      });
    }
    const hasTopicMaterial = candidates.some(
      (c) => c.source === 'vocab' || c.source === 'vocab-bank' || c.source === 'gemini'
    );
    if (hasTopicMaterial) {
      for (const row of bank) {
        if (candidates.some((c) => c.word === row.word)) continue;
    ...
    // Keep page when lesson/Gemini supplied a legal word. Pure bank-only (cat/dog
    // with no topic material) → omit, even if meta.phonics=on (Manus face/castle).
    const topicHits = candidates.filter(
      (c) => c.source === 'vocab' || c.source === 'vocab-bank' || c.source === 'gemini'
    );
    if (!topicHits.length) return null;
    if (!candidates.length) return null;

    let words = [];
    // Prefer a vocab-tied focus when available; else first legal Gemini word.
    const vocabHits = candidates.filter((c) => c.source === 'vocab' || c.source === 'vocab-bank');
    words.push((vocabHits[0] || topicHits[0]));
...
Do NOT include a phonics object (omit it). This lesson level/topic does not need a phonics page.`;
    ...
    const byLevel = {
      'PRE-A1': `Pre-A1 ONLY: optional listen/say/trace sound exposure for very short regular words. 2–3 boxes. No digraphs, blends, vowel teams, magic-e, multisyllabic words, or spelling-rule teaching.`,
...
- KNOWN-VOCABULARY-FIRST: prefer phonetically regular words drawn FROM the lesson vocabulary (same spelling). If vocab is irregular (castle, knight, could) OR illegal for this CEFR (magic-e / digraphs at A1), OMIT the phonics object entirely — do NOT substitute unrelated CVC fillers (cat/dog) that break topic fit.
