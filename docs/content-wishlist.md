...
| 2026-08-08 | "confused" 😕 still reads as neutral, not clearly puzzled (judges noted alongside shy) | feelings New Words | Soft: only shy fixed this pass; a distinct puzzled glyph/override for confused is next-loop fuel (avoid clashing with 😲/😳) | open |
| 2026-08-08 | Activity heroStage still leaves right-side/upper dead space; blank drop-face is the largest element by design (drop target) | feelings activity | Soft: king shrunk + faces enlarged this pass; a fuller stage composition (raise dock band / balance right) is uncheckable polish for a later loop | open |
