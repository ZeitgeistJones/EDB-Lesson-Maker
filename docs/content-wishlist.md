...
| 2026-08-08 | "time/clock" is low-value as a full 4×8 prop sheet — the theme is already well-covered (wall-clock, hourglass, stopwatch, metronome, kitchen-timer, pocket-watch, grandfather-clock, sun) so a 32-tile sheet re-treads those and pads with off-theme fillers (gears, coil spring); only ~13 genuinely-new props survived the merge | batch-run6 merge time sheet (weakest of 7) | Right-size thin themes to 4×4/16 (gptbrief Run-6 rule); don't brief clock/time as a full Variant C sheet — top up the handful of new pieces in-house instead | open |
| 2026-08-09 | Match dock / labeled art shipped Gemini emoji, `•`, or role-label placeholders when VocabIcons index was cold/failed (`{}` cached forever) or art missing | F13 bake null ladder | VocabArt.planFor pack→prop(+headNounOk)→curatedGlyph→none; loadIndex retryable; match dock drops unpaired words; story caption-only not 📖; H10/S72 | schema-fixed |
| 2026-08-09 | LATENT feelings-dock landmine: the feelings detector regex (`lessonTraits.RE.feelingsWords` / edbActivities) matches `excited` + `tired`, but roster filter dropped them when not on `ROLEPLAY_DOCK_FEELINGS` → short dock / S49. | fresheyes audit | Pack-backed taught feelings stay on dock even without feeling-* props; added feeling-excited/tired to roster | schema-fixed |
| 2026-08-09 | StoryArt client vs server cache keys diverge: client (`storyArt.js cacheKey`) hashes `{title,level,pages}`; server (`generate-story-art cacheKeyFor`) also folds in `model` + `PROMPT_VERSION`. Independent caches so no correctness bug, but after a server prompt-version bump the client keeps serving stale session art (Map + sessionStorage) until refresh. | fresheyes audit (unverified→confirmed 2026-08-09) | LOW. Cheap durable fix: add a version token to `STORAGE_PREFIX` (bump on prompt/model changes) so a version change invalidates the client cache too. Not worth churn until the story-art prompt actually changes. | open |
| 2026-08-09 | Importer exit-code conflation: `import-prop.mjs` uses `exit(1)` for usage errors AND the crash catch (line 1205), but `exitCode=1` for EXPECTED gate-blocks (1017/1023/1103). A caller reading only the exit code can't tell "some tiles gate-blocked" (normal in staging) from "script crashed". Not active — `import-sheet` reads per-tile status from `--results` JSON, not the exit code. | fresheyes audit (unverified→confirmed 2026-08-09) | LOW / document-don't-touch. A clean fix (distinct codes: 2=usage, 3=crash, 1=some-blocked) is a behavior change that could break any caller checking `==1`; leave until an automation actually needs the distinction. | open |
| 2026-08-09 | Perfect11 Call1 Manus ignored portrait 4×8 black-field brief — delivered 2048×2048 square 6×6 (36 cells). First import used `--grid=8x4` and mis-sliced (multi-prop tiles). carnival/library/airport were white-field (+ airport labels). | shift30 perfect11-call1 | Call2 brief hardened (portrait 4×8, #000, no text, no 6×6). Re-imported circus+post with `--grid=6x6 --force` (35+35); culled 94 carnival/lib/air mis-slices. Do NOT re-fire Call1. Wait credit OK for Call2. | prompt-fixed |
| 2026-08-10 | Story truncated mid-name ("…paintings. Ben") + comprehension asked why Anna chose choir while story had her at art booth | teacher PDF "All Kinds of School Clubs" | Track A: `StoryIntegrity` repair + generate prompt + Ready/Draft. Track B (judges): soft **S73** + dual-lens / Manus heuristics — silent repair ≠ review pass if JPGs still mismatch | schema-fixed |
| 2026-08-10 | New Words picture bin shipped blurry `gashapon-robot` (71×83 soft splice + mis-slice fringe) for vocab "robot" — MIN_DOCK_SRC already banned soft blobs from roleplay docks, but VocabArt.planFor tier-2 had no isDockSharp / decorativePacksFor gate | teacher PDF "All Kinds of School Clubs" | VocabArt rejects soft + off-topic decorative props; SAFE_EMOJI robot→🤖 until sharp pack art; wishlist sharp robot | schema-fixed |
| 2026-08-10 | Ready now needs ≥83% vocab art (was 50%) | BoardReadiness honesty / New Words | VOCAB_ART_FLOOR → 5/6; Draft when under floor | schema-fixed |
| 2026-08-10 | Board taught generate's first 6 even when overflow had picturable words → hollow / Draft art floor | producer adapt vs topic-drift | `VocabArt.adaptBoardVocabulary` reorders same-topic list (art-preferred six); activity with no hero→`sortBins` (not random dressUp); StoryArt stays opportunistic — not a Ready dependency | schema-fixed |
