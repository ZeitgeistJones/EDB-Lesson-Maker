...
| `home-warm` | curtain fold, lamp-glow blob, soft arch shadow, window sash fringe on `home-a`…`d` (picker open/mid/close). Legacy hill/leaf PNGs kept as `home-e`…`h` without `set` | outdoor meadow, clinic teal wall, formula leaf stickers |
...
| `soccer-fresh` | almost-plain turf haze, goal-post corner, pitch-line fringe, corner flag | picnic blanket, zoo gate, camp tent, indoor gym court, basketball chrome |
...
| `space-cool` | soft indigo–periwinkle wash; **title `-a` must still show sparse corner stars** (not blank lavender); crescent moon corner; soft ringed-planet edge; nebula fringe + sparse stars | dawn clouds, blossom petals, deep-night light-ink skies, furnished rooms, mid-frame rockets, **anonymous empty haze on `-a`** |
| `volcano-cool` | ash-rose haze, crater-rim watermark, ash-plume curl, ember glow fringe | picnic grass, zoo gate, outdoor meadow, furnished rooms |
...
| `outdoor-fresh` | park / zoo / camp / green-field family (not soccer pitch) | wired |
| `soccer-fresh` | soccer / pitch / football (goal · lines · corner flag) — **not** picnic/zoo/camp | wired (in-house Aug 2026) |
| `gym-cool` | gym / basketball / workout (indoor court washes) — **not** soccer | wired (in-house Aug 2026) |
| `board-house` | default + bare sports/tennis/baseball; also **mid drill pages** when a place set is locked (less is more) | wired |
...
| `aquarium-cool` | aquarium / fish tank / **sea animals / sharks / marine / underwater** | wired (in-house Aug 2026) — marine fauna before outdoor `\banimal\b` false friend |
...
| `space-cool` | space / planet / rocket / astronaut / galaxy | wired (in-house Aug 2026) — replaced borrowed starry-night / dawn-clouds / moon-petals |
| `volcano-cool` | volcano / lava / eruption / crater / ash | wired (in-house Aug 2026) — was false-friend `outdoor-fresh` picnic |
...
arch shadow, window sash fringe. Not outdoor meadow / leaf stickers. Picker
open/mid/close use sorted `a` / `b`–`c` / last — place-true glyphs on
`home-a`…`d`. Legacy hill/leaf cousins stay as `home-e`…`h` **without** `set`
so they do not steal the close pin.
...
Corner glyphs must be **postage-stamp stickers**, not full habitat / campsite
scenes (shift60 eye-QA rejected giraffe habitats and mid-frame tents).
...
### 4b. `gym-cool` — gym / basketball / workout
...
**Not soccer** — soccer / pitch lessons lock `soccer-fresh`. Bare "sports"
without basketball locks quiet `board-house`.
...
### 4c. `soccer-fresh` — soccer / pitch / football
Palette: pale turf green + cream sky. Corner motifs: almost-plain turf haze
(`-a`), goal-post + net watermark, pitch-line / arc fringe, corner flag.
Empty centre. **Not** park picnic / zoo gate / camp tent (`outdoor-fresh`).
**Not** indoor gym court (`gym-cool`). Panel `soccer-c` is `quiet: false`
(busy mid) and stays out of chrome rotation.

> [style lock] Quiet ESL soccer pitch washes. Shared pale turf + sky palette.
> Empty centre. Place-true pitch motifs only — goal, lines, corner flag.
> No picnic blanket, zoo gate, camp tent, basketball hoop, or ball chrome.
