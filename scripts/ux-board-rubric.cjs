...
const HARD = {
  ...
  H5: 'matchDock answer leak — only when drag-pad dock embeds answers on cards; inline picture-beside-word is OK',
...
const METRICS = {
  ...
  M11: {
    label: 'Background mid-band texture stddev (centre 40–70% height) — quiet flats stay empty',
    scope: 'page',
    worseWhen: 'higher',
    warn: 22,
    fail: 32,
    severity: 'soft',
    why: 'Busy wash under cards — style-lock centre emptiness / ChatGPT polish gap',
  },
};
...
const SOFT = {
  ...
  S73: 'Comprehension questions are grounded in story text (no choir-Q when story only has art booth); story pages end in complete sentences',
  ...
  S28: 'New Words shows picture beside each word (matchDockInline) or numbered drop pads (legacy matchDockPads)',
  ...
  // Off-topic prop leak (soccer bake 2026-08): decorative/character filler packs
  S71: 'No decorative/character filler prop (3D feeling-* faces, gashapon toy blobs) is docked on the activity or used as story art on a lesson whose topic does not invite that pack — a soccer beat must never show a 3D "worried" face or a gashapon cyclops-in-sunglasses',
};
...
const TIERS = {
  honesty: ['H5', 'H1', 'H10', 'S15', 'S8', 'M7', 'S19', 'S20', 'S26', 'S30', 'S31', 'S37', 'S51', 'S52', 'S57', 'S58', 'S59', 'S60', 'S63', 'S67', 'S69', 'S70', 'S71', 'S72', 'S73'],
  readable: ['H4', 'S1', 'S5', 'S11', 'S13', 'S9', 'M1', 'M2', 'M6', 'M11', 'S35', 'S50', 'S55', 'S56', 'S62'],
...
const PILLARS = {
  readable: ['S1', 'S5', 'S11', 'S13', 'M1', 'M2', 'M6', 'M11'],
  navigable: ['S4', 'S10', 'S14', 'M3', 'M8', 'H3'],
  accurateVocabArt: ['S15', 'S9', 'M7', 'H5', 'H10', 'S72'],
  backgroundVariability: ['S2', 'S6', 'S18', 'M5', 'H2'],
  appropriateBackgrounds: ['S3', 'S8', 'H1', 'M11'],
  funCharming: ['S7', 'S12', 'S16', 'S17', 'M4'],
};

/**
 * Visual-quality critical buckets (plan Step 15). Fail any of these → page
 * cannot pass a visual-run. Mapped to existing H/M/S codes + M11 mid-band.
 */
const CRITICAL_VISUAL = {
  readability: ['M1', 'M2', 'M6', 'M11'],
  interactionClarity: ['M3', 'M8', 'M10', 'H3'],
  clutter: ['M9', 'M2', 'M11'],
};
...
/**
 * Returns critical visual failures for a page/case metric bag.
 * Critical categories must score pass (not warn/fail) — plan Step 15.
 */
function criticalVisualFails(metrics, hardCodes) {
  const fails = [];
  const hard = new Set(hardCodes || []);
  for (const [bucket, codes] of Object.entries(CRITICAL_VISUAL)) {
    for (const code of codes) {
      if (code.startsWith('H')) {
        if (hard.has(code)) {
          fails.push({ bucket, code, grade: 'fail', value: true });
        }
        continue;
      }
      if (!metrics || metrics[code] == null) continue;
      const grade = gradeMetric(code, metrics[code]);
      if (grade === 'fail') {
        fails.push({ bucket, code, grade, value: metrics[code] });
      }
    }
  }
  return fails;
