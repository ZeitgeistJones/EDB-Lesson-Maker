...
// Force-on must NOT invent off-topic cat/dog when lesson vocab has no legal CVC
// (Manus face Ehp2 / castle S4DZ — cases.json still passes phonics:on).
const castleNoCvc = {
  title: 'Building Our Royal Castle',
  vocabulary: [
    { word: 'castle' }, { word: 'king' }, { word: 'queen' },
    { word: 'knight' }, { word: 'dragon' }, { word: 'tower' },
  ],
  phonics: null,
};
assert(
  PhonicsPolicy.normalize(castleNoCvc, { level: 'A1', phonics: 'on' }) == null,
  'castle with no legal A1 vocab CVC omits even when phonics=on'
);

// C1 omits unless forced (and still needs a vocab-tied legal word)
