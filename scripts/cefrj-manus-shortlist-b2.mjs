/**
 * Fast CEFR-J B2 Manus shortlist — same light keep/review/reject as B1.
 * Exact-dedupe against live pack. No scoring. No Manus send.
 *
 *   node scripts/cefrj-manus-shortlist-b2.mjs
 */
import fs from 'fs';
...
const OUT = path.join(ROOT, 'tmp/cefrj-manus/b2');
...
/** Same sets as scripts/cefrj-manus-shortlist-b1.mjs */
...
/** Never send to Manus — exact keys that trip safety and can wipe a whole sheet task. */
const MANUS_SAFETY_DENY = new Set(
...
function classify(pos, word) {
  if (MANUS_SAFETY_DENY.has(normalize(word))) return 'reject';
...
function main() {
  ...
  const summary = {
    ...
    level: LEVEL,
    note: 'B2 shortlist — same light scrub as B1. Live pack exact-dedupe. No Manus sent.',
    nouns: {},
    verbs: {},
    adjectives: {},
  };
  ...
  for (const pos of ['noun', 'verb', 'adjective']) {
    ...
    fs.writeFileSync(
      path.join(OUT, `manus-${plural}-b2.txt`),
      keep.map((r) => r.word).join('\n') + (keep.length ? '\n' : '')
    );
    fs.writeFileSync(
      path.join(OUT, `${plural}-review-b2.txt`),
      review.map((r) => r.word).join('\n') + (review.length ? '\n' : '')
    );
    fs.writeFileSync(
      path.join(OUT, `${plural}-rejected-b2.txt`),
      reject.map((r) => r.word).join('\n') + (reject.length ? '\n' : '')
    );

    summary[plural] = {
      cefrjB2Rows: csvRows.length,
  ...
  const md = `# CEFR-J B2 Manus shortlist

Generated: ${summary.generatedAt}

Same light picturability scrub as B1. Exact-deduped against live pack. **No Manus sent.**

## Nouns
- CEFR-J B2 rows: **${summary.nouns.cefrjB2Rows}**
...
## Verbs
- CEFR-J B2 rows: **${summary.verbs.cefrjB2Rows}**
...
## Adjectives
- CEFR-J B2 rows: **${summary.adjectives.cefrjB2Rows}**
...
## Files
- \`tmp/cefrj-manus/b2/manus-*-b2.txt\`
- \`tmp/cefrj-manus/b2/*-review-b2.txt\` / \`*-rejected-b2.txt\`
`;
