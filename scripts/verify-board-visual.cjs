...
/** Drop page-N.jpg at/above pageCount so Manus cannot attach stale wrap orphans. */
function clearOrphanPageJpgs(caseDir, pageCount) {
  if (!fs.existsSync(caseDir)) return;
  const keep = Math.max(0, Number(pageCount) || 0);
  for (const n of fs.readdirSync(caseDir)) {
    const m = /^page-(\d+)(?:-.+)?\.(jpe?g|png)$/i.exec(n);
    if (!m) continue;
    if (Number(m[1]) >= keep) {
      try { fs.unlinkSync(path.join(caseDir, n)); } catch (_) { /* ignore */ }
    }
  }
}
...
async function measureInPage({ lesson, meta, BOARD_W, BOARD_H, MAX_PAGES, MAX_UNLOCKED_IOU, forceBg }) {
  ...
  function measurePage(pageEl) {
    ...
    for (const node of pageEl.querySelectorAll('*')) {
      ...
      // Match-dock word cards are pad play surfaces (drag targets), not sparse text cards.
      const isMatchCard = node.dataset && node.dataset.matchCard === '1';
      if ((isColoringStage || isWriteInStage || isMatchCard) && area > pageArea * 0.08) {
        cards.push({
          rect: r,
          area,
    ...
    // Mid-band emptiness (M11): flats only — scenes are allowed more texture.
    let midBandSd = null;
    if (stats && bgImg && (bgImg.dataset.bgKind || 'flat') !== 'scene') {
      const mid = stats.region({
        x: BOARD_W * 0.15,
        y: BOARD_H * 0.4,
        w: BOARD_W * 0.7,
        h: BOARD_H * 0.3,
      });
      if (mid) midBandSd = Number(mid.stddev.toFixed(1));
    }
  ...
  async function answerLeakCheck(boardPlan, rendered) {
    ...
    // Inline New Words (picture beside word on the card) is the product layout —
    // icons inside cards are intentional, not a drag-match answer leak (H5).
    const newWordsPage = (boardPlan.pages || []).find((pg) => pg.pageKey === 'newWords');
    const notes = (newWordsPage && newWordsPage.notes) || [];
    if (notes.some((n) => /matchDockInline/i.test(String(n)))) {
      return { ok: true, skipped: true, reason: 'matchDockInline' };
    }
...
function metricFlagsFor(caseId, result) {
  ...
  for (const p of result.pages) {
    ...
    push('M11', p.metrics.M11, 'page', p.pageKey, p.index);
  }
...
function regressionsFor(caseId, result, baselineCase) {
  ...
  for (const p of result.pages) {
    ...
    for (const code of ['M1', 'M2', 'M3', 'M6', 'M8', 'M9', 'M10', 'M11']) {
...
async function main() {
  ...
  try {
    ...
    for (const c of cases) {
      ...
      // Wipe orphans before write — phonics-omit / shorter decks leave stale
      // page-N.jpg that Manus pickImages still attaches as duplicate Wrap Ups.
      clearOrphanPageJpgs(caseDir, result.pageCount);
      ...
      // Place bookends may use a matching scene (music → classical terrace).
      // Mid chrome (warm/vocab/frames/…) must stay quiet flats.
      const isPlaceBookendKey = (key) => /^(title|activity|wrap|story)/i.test(String(key || ''));
      ...
      // Quiet flats under chrome + EDB. Place bookend scenes must match topic.
      if (!result.titlePick) {
        fail('H2', 'title pick missing');
      } else if (result.titlePick.type === 'scene') {
        if (!expectName.test(result.titlePick.name)) {
          fail('H1', `title scene ${result.titlePick.name} off-topic (want ~/${c.expectScene}/)`);
        }
      } else if (result.titlePick.type !== 'flat') {
        fail('H2', `title pick expected flat or place scene, got ${JSON.stringify(result.titlePick)}`);
      }
      if (!result.vocabPick || result.vocabPick.type !== 'flat') {
        fail('H2', `vocab pick expected flat, got ${JSON.stringify(result.vocabPick)}`);
      }
      if (!activityPick) {
        fail('H2', 'activity pick missing');
      } else if (activityPick.type === 'scene') {
        if (!expectName.test(activityPick.name)) {
          fail('H1', `activity scene ${activityPick.name} off-topic (want ~/${c.expectScene}/)`);
        }
      } else if (activityPick.type !== 'flat') {
        fail('H2', `activity pick expected quiet flat or place scene, got ${JSON.stringify(activityPick)}`);
      }
      if (flatCount < 3) {
      ...
      (result.pageKeys || []).forEach((key, i) => {
        ...
        if (!pick) return;
        if (pick.type === 'scene') {
          if (!isPlaceBookendKey(key)) {
            fail('H2', `page ${key} mid chrome must be quiet flat, got scene:${pick.name}`);
          } else if (!expectName.test(pick.name)) {
            fail('H1', `page ${key} scene ${pick.name} off-topic (want ~/${c.expectScene}/)`);
          }
          return;
        }
        if (pick.type !== 'flat') {
