...
async function main() {
  ...
  if (!dry) {
    ...
    // Replay just this run's rows onto a fresh read under the manifest lock
    // (manifest-lock.mjs) so a concurrent import's keys survive.
    if (merged > 0) {
      await mutateManifest(MANIFEST, (m) => {
        for (const k of mergedKeys) m.props[k] = manifest.props[k];
      });
    }
  }
