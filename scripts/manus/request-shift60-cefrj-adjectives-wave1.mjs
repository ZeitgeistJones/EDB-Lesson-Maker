/**
 * Shift60 — CEFR-J A1/A2 adjectives wave1 via Manus.
 * Source: tmp/cefrj-manus/manus-adjectives-a1-a2.txt (approved keep list; no re-score).
 * Exact-dedupe once with verifiedPackHit immediately before send; no re-filter/rank.
 * White 3×3 still-life sheets; Perfect-11 (≤11 sheets/task); ≤3 createTask in flight.
 * Anti-iconey LOOK (rich kid-illustration / concrete prop scenes — not emoji/UI glyphs).
 *
 *   node scripts/manus/request-shift60-cefrj-adjectives-wave1.mjs           # dry-run
 *   node scripts/manus/request-shift60-cefrj-adjectives-wave1.mjs --send
 *   node scripts/manus/request-shift60-cefrj-adjectives-wave1.mjs --send --task=1
 */
import fs from 'fs';
...
const ADJ_PATH = path.join(ROOT, 'tmp', 'cefrj-manus', 'manus-adjectives-a1-a2.txt');
...
const OUT_ROOT = path.join(ROOT, 'tmp', 'manus-shift60-cefrj-adjectives-wave1');
const CELLS = 9;
...
/**
 * Read approved A1/A2 adjective queue in order. Exact-dedupe vs current pack only.
 * No re-filter / re-rank / B1–B2 expansion.
 */
function loadAndDedupe() {
  if (!fs.existsSync(ADJ_PATH)) {
    throw new Error(`Missing ${ADJ_PATH}`);
  }
  const index = JSON.parse(fs.readFileSync(INDEX_PATH, 'utf8'));
  const whitelist = loadWhitelist();
  const raw = [];
  const seen = new Set();
  for (const line of fs.readFileSync(ADJ_PATH, 'utf8').split(/\r?\n/)) {
...
function buildSheets(rows) {
  ...
  for (let i = 0; i < rows.length; i += CELLS) {
    ...
    sheets.push({
      id: `S${n}`,
      theme: `cefrj-adjectives-${String(n).padStart(2, '0')}`,
      title: `CEFRJ ADJECTIVES ${n}`,
...
This is CEFR-J A1/A2 adjectives wave1 task ${taskNo}/${taskCount} (Shift60; kind cefrj-adjectives-wave1). Keep working until EVERY listed sheet PNG exists (Perfect-${n} multi-call — one image call per sheet, all in THIS task).

Filename each PNG with the theme slug in the name (e.g. esl_cefrj_adjectives_sheet_01_cefrj-adjectives-01.png). Prefer one zip of all PNGs plus CDN links in chat.
...
async function fireTask(task, taskCount, meta) {
  ...
  const created = await createTask({
    title: `ESL white vocab 3×3: Shift60 CEFR-J adjectives wave1 t${task.taskNo}/${taskCount} (${task.sheets.length} sheets)`,
    message: brief,
    agent_profile: profile,
    force_skills: force,
  ...
  JSON.stringify(
    {
      ...
      source: 'tmp/cefrj-manus/manus-adjectives-a1-a2.txt',
      rawCount,
