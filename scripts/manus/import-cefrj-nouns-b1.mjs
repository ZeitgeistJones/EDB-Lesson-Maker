 ...
 * Discovers all taskN/run.json under tmp/manus-shift60-cefrj-nouns-b1/.
 *
 *   node scripts/manus/import-cefrj-nouns-b1.mjs
 *   node scripts/manus/import-cefrj-nouns-b1.mjs --task=1
 *   node scripts/manus/import-cefrj-nouns-b1.mjs --inbox=assets-inbox/manus-cefrj-nouns-b1
 */
import fs from 'fs';
...
const PLAN_ROOT = path.join(ROOT, 'tmp', 'manus-shift60-cefrj-nouns-b1');
const DEFAULT_INBOX = path.join(ROOT, 'assets-inbox', 'manus-cefrj-nouns-b1');
...
function scorePngForTheme(pngPath, theme, sheetId) {
  ...
  const numbered = themeSlug.match(/^(cefrj-nouns(?:-b1)?-)(\d+)$/);
  if (numbered) {
    const n = numbered[2];
