 ...
 *
 * Perfect-11: ONE Manus task / ONE createTask message lists all 11 sheets.
 * The 5-image cap is per generate_image tool call, not per task — Manus must
 * fire 3 generate_image calls (5+5+1) inside that same task. Never handcuff
 * with "exactly 5 sheets" or split Call 2/3 into new Cursor chats.
 */
export const ESL_ASSET_CURSOR_RULES = [
  ...
  '4. PERFECT-11 / MULTI-CALL — The 5-image limit is PER generate_image CALL, not per task. Put the FULL sheet list (up to 11) in ONE message/task. Manus MUST keep working: fire 5+5+1 (or fewer trailing) generate_image calls inside THIS task until every listed sheet PNG exists. Do NOT stop after the first batch. Do NOT open a second Manus task for the remainder. Do NOT say "this call is exactly N sheets" as a handcuff. Completion check: count sheet PNGs against the list before finishing.',
].join('\n');
...
async function parseApiResponse(method, route, res, text, allowStatuses) {
  let data;
  try {
...
async function api(method, route, body, { allowStatuses = [] } = {}) {
  ...
  const once = () => fetch(url, { method, headers, body: payload });
  let res = await once();
  let text = await res.text();
  // Rate-limit: wait 90s, one retry; if still 429, double wait then stop (no third call).
  if (res.status === 429) {
    const waitMs = 90_000;
    console.error(`Manus 429 on ${method} ${route}; waiting ${waitMs / 1000}s then one retry`);
    await sleep(waitMs);
    res = await once();
    text = await res.text();
    if (res.status === 429) {
      const doubled = waitMs * 2;
      console.error(`Manus 429 retry failed; backing off ${doubled / 1000}s with no extra call`);
      await sleep(doubled);
      throw new Error(`Manus ${method} ${route} failed (429) after one retry`);
    }
  }
  return parseApiResponse(method, route, res, text, allowStatuses);
}
...
/**
 * Follow-up into an existing task (Perfect-N multi-call remainder, rate-limit recovery).
 * See https://open.manus.ai/docs/v2/task.sendMessage
 */
export async function sendMessage(taskId, opts) {
  const {
    message,
    agent_profile,
    structured_output_schema,
  ...
  if (agent_profile != null && String(agent_profile).trim()) {
    body.agent_profile = String(agent_profile).trim();
  }
  if (structured_output_schema) body.structured_output_schema = structured_output_schema;
  if (clear_connectors === true) body.clear_connectors = true;
  return api('POST', '/task.sendMessage', body);
}
