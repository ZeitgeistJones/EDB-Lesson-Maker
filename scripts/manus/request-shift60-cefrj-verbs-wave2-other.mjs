/**
 * Shift60 — CEFR-J verbs wave2 OTHER (Group2) via Manus.
 * Source: tmp/cefrj-manus/manus-verbs-wave2-other.txt
 ...
 * Import as picturable-verbs wave=12. Themes continue after primary: cefrj-verbs-31+.
 *
 *   node scripts/manus/request-shift60-cefrj-verbs-wave2-other.mjs           # dry-run
 *   node scripts/manus/request-shift60-cefrj-verbs-wave2-other.mjs --send
 *   node scripts/manus/request-shift60-cefrj-verbs-wave2-other.mjs --send --task=1
 */
import fs from 'fs';
...
const OTHER_PATH = path.join(ROOT, 'tmp', 'cefrj-manus', 'manus-verbs-wave2-other.txt');
...
const OUT_ROOT = path.join(ROOT, 'tmp', 'manus-shift60-picturable-verbs-wave12');
const CELLS = 9;
const SHEETS_PER_TASK = 11;
const MAX_TASKS = 3;
const WAVE = 12;
/** Continue theme numbering after wave2 primary sheets cefrj-verbs-05..30 */
const THEME_START = 31;
...
/**
 * Read other queue in order. Exact-dedupe vs current pack only — drop verified hits.
 * No re-filter / re-rank / SKIP list.
 */
function loadAndDedupe() {
  if (!fs.existsSync(OTHER_PATH)) {
    throw new Error(`Missing ${OTHER_PATH}`);
  }
  const index = JSON.parse(fs.readFileSync(INDEX_PATH, 'utf8'));
  const whitelist = loadWhitelist();
  const raw = [];
  const seen = new Set();
  for (const line of fs.readFileSync(OTHER_PATH, 'utf8').split(/\r?\n/)) {
...
This is CEFR-J A1/A2 verbs wave2 other Group2 task ${taskNo}/${taskCount} (Shift60; import wave ${WAVE}; kind cefrj-verbs-wave2-other). Keep working until EVERY listed sheet PNG exists (Perfect-${n} multi-call — one image call per sheet, all in THIS task).

Filename each PNG with the theme slug in the name (e.g. esl_cefrj_verbs_sheet_31_cefrj-verbs-31.png). Prefer one zip of all PNGs plus CDN links in chat.
...
async function fireTask(task, taskCount, meta) {
  ...
  const created = await createTask({
    title: `ESL white vocab 3×3: Shift60 CEFR-J verbs wave2 other t${task.taskNo}/${taskCount} (${task.sheets.length} sheets)`,
    message: brief,
    agent_profile: profile,
    force_skills: force,
