/**
 * Tiny mop for two wave3 glance-fail pairs only: doll-cradle, mason-jar.
 * Do not redo anything else from wave3.
 *
 *   node scripts/manus/request-hero-targets-wave3-mop.mjs --fire
 *   node scripts/manus/request-hero-targets-wave3-mop.mjs --poll-only
 */
import fs from 'fs';
...
const OUT_DIR = path.join(STOCKPILE, 'wave3-mop');
const SHEET_DIR = path.join(OUT_DIR, 'sheets');
const RUN_JSON = path.join(OUT_DIR, 'run.json');
const WAVE_ID = 'wave3-mop';

const PAIRS = [
  {
    slug: 'doll-cradle',
    closed:
      'pastel wooden doll cradle, 3/4 view. SOLID hood/canopy fully DOWN covering the entire bed so the mattress is COMPLETELY hidden — reads as a closed covered bassinet. No doll. Hood must hide the hollow.',
    open:
      'SAME cradle, SAME viewpoint/scale/colors: hood flipped all the way BACK behind the headboard. Empty mid-tone walnut mattress hollow clearly visible. Still no doll. Closed vs open must be obvious at a glance (covered bed vs uncovered empty well).',
  },
  {
    slug: 'mason-jar',
    closed:
      'teal-tinted glass mason jar, 3/4 view, chunky bright COPPER screw-lid ON with a tall round knob. Lid is large and unmistakable. Empty but sealed.',
    open:
      'SAME jar, SAME viewpoint/scale: lid COMPLETELY GONE, wide open mouth, empty teal-glass interior (mid-tone, not white). No jam. Difference must be obvious: lid-on vs lid-off.',
  },
];

const BRIEF = withEslAssetGeneratorBrief(`TASK: Produce **ONE black-field 2×2 contact sheet** (2 rows × 2 columns = 4 cells) of MEDIUM HERO play-surface props.

This replaces TWO failed wave3 pairs only. Do not invent extra objects.
...
1. hero-doll-cradle-closed — ${PAIRS[0].closed}
2. hero-doll-cradle-open — ${PAIRS[0].open}
3. hero-mason-jar-closed — ${PAIRS[1].closed}
4. hero-mason-jar-open — ${PAIRS[1].open}
...
function upsertInventory(dump) {
  ...
  const items = PAIRS.map((p) => ({
    ...
    sheet_id: 'sheet1-2x2-mop',
    obvious_failure: false,
    replaces_wave: 'wave3',
  }));
  inv.waves[WAVE_ID] = {
    kind: 'hero-targets-wave3-mop',
    title: 'ESL hero targets wave3 mop — doll-cradle + mason-jar only',
  ...
  const w3 = inv.waves.wave3;
  if (w3 && Array.isArray(w3.items)) {
    for (const item of w3.items) {
      if (item.concept !== 'doll-cradle' && item.concept !== 'mason-jar') continue;
...
fs.writeFileSync(
  ...
  JSON.stringify({ wave: WAVE_ID, pairs: PAIRS.map((p) => p.slug), grid: '2x2' }, null, 2),
);
...
if (!pollOnly) {
  ...
  const created = await createTask({
    title: 'ESL hero targets wave3 mop — doll-cradle + mason-jar',
...
if (large.length < NEED_SHEETS) {
  ...
  await sendMessage(taskId, {
    ...
    message: withEslAssetGeneratorBrief(
      `Continue THIS task. You returned ${large.length} usable PNG sheet(s); we need exactly 1 black-field 2×2 sheet (doll-cradle closed/open, mason-jar lid-on/lid-off). Fire generate_image now. Do not restart. Do not add extra objects.`,
    ),
  });
