/**
 * B2 final vertical visual stockpile harvest.
 * Stockpile only — no import / keying / renderer wiring.
 *
 *   node scripts/manus/request-b2-harvest.mjs --write-ledgers
 *   node scripts/manus/request-b2-harvest.mjs --wave=1 --fire
 *   node scripts/manus/request-b2-harvest.mjs --wave=1 --poll-only
 *
 * Sheets: harvested/manus-b2-stockpile/<wave-id>/sheets/
 * Inventory: harvested/manus-b2-stockpile/inventory.json
 * Tracked copy: docs/b2-stockpile-inventory.json
 */
import fs from 'fs';
...
function sheetBlock(sheet, index) {
  ...
  const emptyNote = sheet.cells.length < 9
    ? `\nINCOMPLETE SHEET: only cells 1-${sheet.cells.length} have art. Leave cells ${sheet.cells.length + 1}-9 pure black empty. No fillers.`
    : '';
  return `SHEET ${index} — ${sheet.title} (${sheet.format}, one B2 concept per cell):\n${lines.join('\n')}\nKeys: ${sheet.cells.map((c) => c.key).join(',')}${emptyNote}`;
}

function waveFailureLock(wave) {
  if (wave.id === 'wave-1-p0-core') {
    return `WAVE 1 FAILURE LOCK:
- Do not make dashboards, worksheets, debate posters, ROI matrices, or labeled pro/con cards.
- These are relation mini-scenes: claim/support weight, trade-off tension, two views, two-source synthesis, idea hierarchy, and multi-constraint compromise.
- B1 already covers simple reason/evidence/viewpoint/source. B2 must show weighing or answering back.`;
  }
  if (wave.id === 'wave-2-p1-interaction-stance') {
    return `WAVE 2 FAILURE LOCK:
- Keep negotiation young-learner: group project, club plan, wrong/missing equipment, activity choice.
- Do not draw high-stakes diplomacy, legal disputes, debate stages, or text-heavy presentation slides.
- Conversation/floor visuals must be relation scenes, not speech-bubble worksheets.`;
  }
  return `WAVE 3 FAILURE LOCK:
- Conservative finish only. Audience/register, revision, narrative extension, and grammar-semantic relation tokens.
- No posters, grammar charts, paragraph text, irony/bias, long essays, or C1 rhetoric.`;
}

function buildBrief(wave, sheets) {
  return withEslAssetGeneratorBrief(`TASK: Produce **${sheets.length} B2 final vertical black-field PNG contact sheet(s)** for ClassIn ESL.

This is B2: SHAPES, WEIGHS, AND ANSWERS BACK. It is the product ceiling. C1/C2 are future only.
...
- NO readable text, fake writing, labels, connector words, letters, numbers, prices, times, dates, source names, paragraph text, reasons/opinions/checklists, dialogue, UI text, menus, or signs.
- Black field means the whole PNG canvas; no white worksheet/page backgrounds, no grey table backgrounds, no classroom handout look.
- Young-learner translation only: school, clubs, sports, trips, group projects, wrong/missing equipment, activity plans.
- Do not create A2/B1 assets again. If a brief asks for B2, show weighing, relation tension, multiple constraints, answer-back, or source/view contrast.
- Do not create C1/C2: no sophisticated argument/rhetoric, rebuttal chains, irony/bias, academic research, 3+ complex sources, diplomacy, high-stakes negotiation, long essays, mixed conditionals, or abstract policy.
...
function emptyInv() {
  return {
    spec: 'b2-final-vertical-cultivation',
    updated_at: null,
    running_total: {
      proposals_reviewed: GENERATE.length + REUSE_EXISTING.length + CODE_LATER.length + DEFER_C1.length,
...
function recomputeTotals(inv) {
  ...
  inv.running_total = {
    ...
    generate_pass: waves.reduce((n, w) => n + (w.items || []).filter((it) => it.qa_status === 'pass').length, 0),
    generate_hold: waves.reduce((n, w) => n + (w.items || []).filter((it) => it.qa_status === 'hold').length, 0),
    recovered_locally: waves.reduce((n, w) => n + (w.items || []).filter((it) => it.recovered_locally).length, 0),
    regenerated: waves.reduce((n, w) => n + (w.items || []).filter((it) => it.regenerated).length, 0),
  };
}
  ...
  for (const s of sheets) {
    for (const cell of s.cells) {
      items.push({
        ...
        obvious_fail: false,
        qa_status: (dump.saved || []).some((x) => x.bytes > 80_000) ? 'pass' : null,
        qa_note: (dump.saved || []).some((x) => x.bytes > 80_000)
          ? 'Narrow QA pending visual cell review; raw B2 stockpile sheet banked for future import/keying.'
          : null,
      });
    }
  }
...
const dump = {
  ...
  kind: 'b2-final-vertical-cultivation',
  wave: wave.id,
  phase: wave.phase,
  family: wave.family,
  sheet_dir: SHEET_DIR,
...
if (large.length < NEED_SHEETS) {
  ...
  await sendMessage(taskId, {
    ...
    message: withEslAssetGeneratorBrief(
      `Continue THIS task. You returned ${large.length} usable PNG sheet(s); we need exactly ${NEED_SHEETS} B2 black-field sheets listed in the original brief. Do not restart. Do not add text. Do not change the key list. Preserve B2 only: no C1/C2, no dashboards, no worksheets, no labels.`,
    ),
  });
