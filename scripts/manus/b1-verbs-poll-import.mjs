/**
 * Poll/import CEFR-J B1 verbs (import wave=13).
 * Per-task import as soon as a task is ready. Optionally fill ≤3 createTask slots.
 *
 *   node scripts/manus/b1-verbs-poll-import.mjs
 *   node scripts/manus/b1-verbs-poll-import.mjs --once
 *   node scripts/manus/b1-verbs-poll-import.mjs --orchestrate --maxMinutes=180
 *   node scripts/manus/b1-verbs-poll-import.mjs --task=1
 */
import fs from 'fs';
...
const INBOX = path.join(ROOT, 'assets-inbox', 'manus-cefrj-verbs-b1');
const STATE_PATH = path.join(ROOT, 'tmp', 'cefrj-manus', 'b1-verbs-loop-state.json');
const LOCK_PATH = path.join(ROOT, 'tmp', 'cefrj-manus', 'b1-verbs-import.lock');
const WAVE = 13;
...
function acquireLock() {
  ...
  try {
    ...
    fs.writeFileSync(
      LOCK_PATH,
      JSON.stringify({ at: new Date().toISOString(), pid: process.pid, owner: 'b1-verbs-poll-import' }, null, 2)
    );
    return true;
  } catch {
    return false;
...
function fillSlots(state) {
  ...
  const args = [
    'scripts/manus/request-shift60-cefrj-verbs-b1.mjs',
    '--send',
    '--fill-slots',
    '--maxInFlight=3',
  ];
...
async function tick(state) {
  ...
  fs.writeFileSync(
    path.join(ROOT, 'tmp', 'cefrj-manus', 'b1-verbs-poll-snapshot.json'),
    JSON.stringify(
      {
