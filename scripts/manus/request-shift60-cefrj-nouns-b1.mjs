/**
 * Shift60 — CEFR-J B1 nouns via Manus.
 * Source: tmp/cefrj-manus/b1/manus-nouns-b1.txt
 ...
 * Anti-iconey LOOK.
 *
 *   node scripts/manus/request-shift60-cefrj-nouns-b1.mjs
 *   node scripts/manus/request-shift60-cefrj-nouns-b1.mjs --send
 *   node scripts/manus/request-shift60-cefrj-nouns-b1.mjs --send --fill-slots --done-tasks=1,2
 */
import fs from 'fs';
...
const QUEUE_PATH = path.join(ROOT, 'tmp', 'cefrj-manus', 'b1', 'manus-nouns-b1.txt');
...
function buildSheets(rows) {
  ...
  for (let i = 0; i < rows.length; i += CELLS) {
    ...
    sheets.push({
      id: `S${n}`,
      theme: `cefrj-nouns-b1-${String(n).padStart(2, '0')}`,
      title: `CEFRJ NOUNS B1 ${n}`,
...
function buildBrief(sheets, taskNo, taskCount) {
  ...
  return withEslAssetGeneratorBrief(`TASK: White-background ESL vocab 3×3 NOUN sheets. Each cell = one clear picturable OBJECT / clear still-life. Skip abstracts / logos / text when a physical prop or scene works.
...
This is CEFR-J B1 nouns task ${taskNo}/${taskCount} (Shift60; kind cefrj-nouns-b1). Keep working until EVERY listed sheet PNG exists (Perfect-${n} multi-call — one image call per sheet, all in THIS task).

Filename each PNG with the theme slug in the name (e.g. esl_cefrj_adjectives_b1_sheet_01_cefrj-nouns-b1-01.png). Prefer one zip of all PNGs plus CDN links in chat.
...
async function fireTask(task, taskCount, meta) {
  ...
  const created = await createTask({
    title: `ESL white vocab 3×3: Shift60 CEFR-J B1 nouns t${task.taskNo}/${taskCount} (${task.sheets.length} sheets)`,
    message: brief,
    agent_profile: profile,
    force_skills: force,
