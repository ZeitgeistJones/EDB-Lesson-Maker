/**
 * Poll/import CEFR-J verbs wave2 other / Group2 (import wave=12).
 ...
 *
 *   node scripts/manus/wave2-other-verbs-poll-import.mjs
 *   node scripts/manus/wave2-other-verbs-poll-import.mjs --once
 *   node scripts/manus/wave2-other-verbs-poll-import.mjs --maxMinutes=90
 *   node scripts/manus/wave2-other-verbs-poll-import.mjs --task=1
 */
import fs from 'fs';
...
const INBOX = path.join(ROOT, 'assets-inbox', 'manus-cefrj-verbs-w2-other');
const STATE_PATH = path.join(ROOT, 'tmp', 'cefrj-manus', 'wave2-other-verbs-loop-state.json');
const LOCK_PATH = path.join(ROOT, 'tmp', 'cefrj-manus', 'wave2-other-verbs-import.lock');
const WAVE = 12;
...
function acquireLock() {
  ...
  try {
    ...
    fs.writeFileSync(
      LOCK_PATH,
      JSON.stringify(
        { at: new Date().toISOString(), pid: process.pid, owner: 'wave2-other-verbs-poll-import' },
        null,
        2
      )
    );
...
async function tick(state) {
  ...
  fs.writeFileSync(
    path.join(ROOT, 'tmp', 'cefrj-manus', 'wave2-other-verbs-poll-snapshot.json'),
    JSON.stringify(
      {
