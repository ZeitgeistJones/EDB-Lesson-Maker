/**
 * Poll/import CEFR-J A1/A2 adjectives wave1.
 ...
 *
 *   node scripts/manus/adjectives-wave1-poll-import.mjs
 *   node scripts/manus/adjectives-wave1-poll-import.mjs --once
 *   node scripts/manus/adjectives-wave1-poll-import.mjs --maxMinutes=90
 *   node scripts/manus/adjectives-wave1-poll-import.mjs --task=1
 */
import fs from 'fs';
...
const maxMinutes = Number(
  (process.argv.find((a) => a.startsWith('--maxMinutes=')) || '--maxMinutes=120').slice(13)
);
...
const INBOX = path.join(ROOT, 'assets-inbox', 'manus-cefrj-adjectives-w1');
const STATE_PATH = path.join(ROOT, 'tmp', 'cefrj-manus', 'adjectives-wave1-loop-state.json');
const LOCK_PATH = path.join(ROOT, 'tmp', 'cefrj-manus', 'adjectives-wave1-import.lock');
...
function acquireLock() {
  ...
  try {
    ...
    fs.writeFileSync(
      LOCK_PATH,
      JSON.stringify(
        { at: new Date().toISOString(), pid: process.pid, owner: 'adjectives-wave1-poll-import' },
        null,
        2
      )
    );
...
function stageByPlanOrder(task) {
  const taskInbox = path.join(INBOX, `task${task.taskNo}`);
  fs.mkdirSync(taskInbox, { recursive: true });
  ...
  for (let i = 0; i < n; i++) {
    ...
    const destTask = path.join(taskInbox, `esl_${theme}.png`);
    const destFlat = path.join(INBOX, `esl_${theme}.png`);
    fs.copyFileSync(pngs[i].full, destTask);
    fs.copyFileSync(pngs[i].full, destFlat);
...
function importTaskThemes(task, themes) {
  const r = spawnSync(
    process.execPath,
    [
      'scripts/manus/import-cefrj-adjectives-wave1.mjs',
      `--task=${task.taskNo}`,
...
async function tick(state) {
  ...
  for (const task of tasks) {
    ...
    try {
      console.log(`Importing adjectives wave1 (task ${task.taskNo})…`);
      ...
      if (result.ok) {
        ...
        console.error(
          JSON.stringify({ phase: 'import-failed', taskNo: task.taskNo, status: result.status })
        );
      }
    } finally {
      releaseLock();
  ...
  fs.writeFileSync(
    path.join(ROOT, 'tmp', 'cefrj-manus', 'adjectives-wave1-poll-snapshot.json'),
    JSON.stringify(
      {
...
while (true) {
  ...
  if (allDone) {
    console.log(
      JSON.stringify({ phase: 'all-imported', rows, importedThemes: state.importedThemes })
    );
    process.exit(0);
  }
  if (ONCE) {
