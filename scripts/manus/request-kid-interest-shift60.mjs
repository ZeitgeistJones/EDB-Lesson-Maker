...
export const WAVES = {
  ...
  /** ——— AGGRO WAVE6 (contact sheets only; majority discovered-new) ——— */

  'ki-geology-dig': wave('ki-geology-dig', {
    lane: 'other-discovered',
    bucket: 'other-discovered',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI geology dig club NEW (3×3 ×3)',
    why: 'Discovered rockhound/dig kit not in original prompt',
    sheets: [
      sh('S1', 'dig tools 3x3', 'black-contact-3x3', [
        cell('geo', 'rock-hammer', `${CUTOUT} Rock hammer.`),
        cell('geo', 'chisel', `${CUTOUT} Chisel.`),
        cell('geo', 'safety-goggles-geo', `${CUTOUT} Safety goggles.`),
        cell('geo', 'hand-lens', `${CUTOUT} Hand lens.`),
        cell('geo', 'sample-bags', `${CUTOUT} Sample bags blank.`),
        cell('geo', 'geo-pick', `${CUTOUT} Geology pick.`),
        cell('geo', 'sieve', `${CUTOUT} Sieve screen.`),
        cell('geo', 'brush-soft', `${CUTOUT} Soft dig brush.`),
        cell('geo', 'hard-hat', `${CUTOUT} Hard hat.`),
      ]),
      sh('S2', 'rocks minerals 3x3', 'black-contact-3x3', [
        cell('geo', 'quartz', `${CUTOUT} Quartz crystal cluster.`),
        cell('geo', 'amethyst', `${CUTOUT} Amethyst geode half.`),
        cell('geo', 'pyrite', `${CUTOUT} Pyrite cube specimen.`),
        cell('geo', 'obsidian', `${CUTOUT} Obsidian chunk.`),
        cell('geo', 'fossil-trilobite', `${CUTOUT} Trilobite fossil.`),
        cell('geo', 'fossil-shell', `${CUTOUT} Shell fossil.`),
        cell('geo', 'lava-rock', `${CUTOUT} Lava rock.`),
        cell('geo', 'sandstone', `${CUTOUT} Sandstone slab.`),
        cell('geo', 'mineral-tray', `${CUTOUT} Mineral specimen tray.`),
      ]),
      sh('S3', 'lab display 3x3', 'black-contact-3x3', [
        cell('geo', 'display-dome', `${CUTOUT} Glass display dome.`),
        cell('geo', 'label-cards-blank', `${CUTOUT} Specimen label cards ZERO text.`),
        cell('geo', 'uv-lamp', `${CUTOUT} UV lamp.`),
        cell('geo', 'streak-plate', `${CUTOUT} Streak plate.`),
        cell('geo', 'hardness-kit', `${CUTOUT} Hardness test kit.`),
        cell('geo', 'map-topo-blank', `${CUTOUT} Topo map blank (no text).`),
        cell('geo', 'sample-case', `${CUTOUT} Rock sample case.`),
        cell('geo', 'loupe-geo', `${CUTOUT} Jeweler loupe.`),
        cell('geo', 'trophy-geo', `${CUTOUT} Geology-club trophy blank.`),
      ]),
    ],
  }),

  'ki-birding-nature': wave('ki-birding-nature', {
    lane: 'other-discovered',
    bucket: 'other-discovered',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI birding + nature journal NEW (3×3 ×3)',
    why: 'Discovered hobby — birdwatching not in original prompt',
    sheets: [
      sh('S1', 'birding gear 3x3', 'black-contact-3x3', [
        cell('bird', 'binoculars-bird', `${CUTOUT} Birding binoculars.`),
        cell('bird', 'spotting-scope-bird', `${CUTOUT} Spotting scope on tripod.`),
        cell('bird', 'field-guide-blank', `${CUTOUT} Field guide book blank cover.`),
        cell('bird', 'bird-journal', `${CUTOUT} Nature journal blank.`),
        cell('bird', 'pencil-set', `${CUTOUT} Pencil set.`),
        cell('bird', 'checklist-blank', `${CUTOUT} Checklist clipboard ZERO letters.`),
        cell('bird', 'bird-call', `${CUTOUT} Bird call whistle.`),
        cell('bird', 'camouflage-hat', `${CUTOUT} Soft camo hat (generic pattern).`),
        cell('bird', 'daypack', `${CUTOUT} Daypack.`),
      ]),
      sh('S2', 'birds still-life 3x3', 'black-contact-3x3', [
        cell('bird', 'sparrow', `${CUTOUT} Sparrow still-life.`),
        cell('bird', 'robin', `${CUTOUT} Robin still-life.`),
        cell('bird', 'bluejay', `${CUTOUT} Blue jay still-life.`),
        cell('bird', 'owl', `${CUTOUT} Friendly owl still-life.`),
        cell('bird', 'hummingbird', `${CUTOUT} Hummingbird still-life.`),
        cell('bird', 'duck', `${CUTOUT} Duck still-life.`),
        cell('bird', 'nest', `${CUTOUT} Bird nest with eggs.`),
        cell('bird', 'feeder', `${CUTOUT} Bird feeder.`),
        cell('bird', 'birdbath', `${CUTOUT} Birdbath.`),
      ]),
      sh('S3', 'habitat kit 3x3', 'black-contact-3x3', [
        cell('bird', 'birdhouse-kit', `${CUTOUT} Birdhouse.`),
        cell('bird', 'seed-bag-blank', `${CUTOUT} Birdseed bag blank.`),
        cell('bird', 'suet-cage', `${CUTOUT} Suet cage.`),
        cell('bird', 'trail-cam-blank', `${CUTOUT} Trail camera blank screen (no logos).`),
        cell('bird', 'hide-tent', `${CUTOUT} Small bird hide tent.`),
        cell('bird', 'walking-stick', `${CUTOUT} Walking stick.`),
        cell('bird', 'magnifier-leaf', `${CUTOUT} Magnifier over leaf.`),
        cell('bird', 'insect-viewer', `${CUTOUT} Bug viewer box.`),
        cell('bird', 'trophy-nature', `${CUTOUT} Nature-club trophy blank.`),
      ]),
    ],
  }),

  'ki-pets-habitats-deep': wave('ki-pets-habitats-deep', {
    lane: 'pets',
    bucket: 'pets',
    in_prompt_named: true,
    title: 'KI pet habitats depth (3×3 ×3)',
    why: 'Pets still thin on habitats — tanks/cages/playpens',
    sheets: [
      sh('S1', 'habitats 3x3', 'black-contact-3x3', [
        cell('phab', 'hamster-cage', `${CUTOUT} Hamster cage with wheel.`),
        cell('phab', 'rabbit-hutch', `${CUTOUT} Rabbit hutch.`),
        cell('phab', 'guinea-pig-pen', `${CUTOUT} Guinea pig playpen.`),
        cell('phab', 'cat-tree', `${CUTOUT} Cat tree tower.`),
        cell('phab', 'dog-crate', `${CUTOUT} Soft dog crate.`),
        cell('phab', 'bird-aviary', `${CUTOUT} Small bird aviary.`),
        cell('phab', 'turtle-tank', `${CUTOUT} Turtle tank.`),
        cell('phab', 'fish-tank-long', `${CUTOUT} Long aquarium (blank glass, no logos).`),
        cell('phab', 'terrarium-lizard', `${CUTOUT} Lizard terrarium.`),
      ]),
      sh('S2', 'habitat gear 3x3', 'black-contact-3x3', [
        cell('phab', 'hamster-wheel', `${CUTOUT} Hamster wheel standalone.`),
        cell('phab', 'water-bottle-cage', `${CUTOUT} Cage water bottle.`),
        cell('phab', 'hay-rack', `${CUTOUT} Hay rack.`),
        cell('phab', 'chew-toys', `${CUTOUT} Pet chew toy set.`),
        cell('phab', 'tunnel-tubes', `${CUTOUT} Play tunnel tubes.`),
        cell('phab', 'heat-lamp', `${CUTOUT} Terrarium heat lamp.`),
        cell('phab', 'filter-pump', `${CUTOUT} Aquarium filter.`),
        cell('phab', 'gravel-scoop', `${CUTOUT} Gravel scoop.`),
        cell('phab', 'hide-cave', `${CUTOUT} Pet hide cave.`),
      ]),
      sh('S3', 'play care 3x3', 'black-contact-3x3', [
        cell('phab', 'cat-wand', `${CUTOUT} Cat wand toy.`),
        cell('phab', 'laser-pointer', `${CUTOUT} Pet laser pointer (toy).`),
        cell('phab', 'treat-ball', `${CUTOUT} Treat-dispensing ball.`),
        cell('phab', 'grooming-table', `${CUTOUT} Mini grooming table.`),
        cell('phab', 'nail-file-pet', `${CUTOUT} Pet nail file.`),
        cell('phab', 'travel-bowl', `${CUTOUT} Collapsible travel bowl.`),
        cell('phab', 'poop-bag-dispenser', `${CUTOUT} Waste-bag dispenser blank.`),
        cell('phab', 'carrier-soft', `${CUTOUT} Soft pet carrier.`),
        cell('phab', 'trophy-pet', `${CUTOUT} Pet-show trophy blank.`),
      ]),
    ],
  }),

  'ki-music-percussion': wave('ki-music-percussion', {
    lane: 'music',
    bucket: 'music',
    in_prompt_named: true,
    title: 'KI percussion + world drums (3×3 ×3)',
    why: 'Music still thin — percussion depth beyond kit/xylophone',
    sheets: [
      sh('S1', 'drums 3x3', 'black-contact-3x3', [
        cell('perc', 'snare', `${CUTOUT} Snare drum.`),
        cell('perc', 'bass-drum', `${CUTOUT} Bass drum.`),
        cell('perc', 'tom-tom', `${CUTOUT} Tom-tom.`),
        cell('perc', 'bongos', `${CUTOUT} Bongos.`),
        cell('perc', 'conga', `${CUTOUT} Conga drum.`),
        cell('perc', 'djembe', `${CUTOUT} Djembe.`),
        cell('perc', 'cajon', `${CUTOUT} Cajón box drum.`),
        cell('perc', 'steel-pan', `${CUTOUT} Steel pan drum.`),
        cell('perc', 'handpan', `${CUTOUT} Handpan.`),
      ]),
      sh('S2', 'small perc 3x3', 'black-contact-3x3', [
        cell('perc', 'maracas-pair', `${CUTOUT} Maracas pair.`),
        cell('perc', 'shaker-egg', `${CUTOUT} Egg shakers.`),
        cell('perc', 'tambourine-perc', `${CUTOUT} Tambourine.`),
        cell('perc', 'triangle', `${CUTOUT} Triangle + beater.`),
        cell('perc', 'cowbell', `${CUTOUT} Cowbell.`),
        cell('perc', 'woodblock', `${CUTOUT} Woodblock.`),
        cell('perc', 'claves', `${CUTOUT} Claves pair.`),
        cell('perc', 'castanets', `${CUTOUT} Castanets.`),
        cell('perc', 'rainstick', `${CUTOUT} Rainstick.`),
      ]),
      sh('S3', 'mallets kit 3x3', 'black-contact-3x3', [
        cell('perc', 'mallets', `${CUTOUT} Mallet pair.`),
        cell('perc', 'drum-pad-practice', `${CUTOUT} Practice drum pad.`),
        cell('perc', 'cymbal', `${CUTOUT} Cymbal on stand.`),
        cell('perc', 'hi-hat', `${CUTOUT} Hi-hat pair.`),
        cell('perc', 'glockenspiel', `${CUTOUT} Glockenspiel (no letter labels).`),
        cell('perc', 'chimes', `${CUTOUT} Bar chimes.`),
        cell('perc', 'drum-bag', `${CUTOUT} Drum bag.`),
        cell('perc', 'metronome-perc', `${CUTOUT} Metronome.`),
        cell('perc', 'trophy-band', `${CUTOUT} Band trophy blank.`),
      ]),
    ],
  }),

  'ki-parkour-ninja': wave('ki-parkour-ninja', {
    lane: 'sports',
    bucket: 'sports',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI parkour + ninja course NEW (3×3 ×3)',
    why: 'Discovered active sport — ninja/parkour course gear',
    sheets: [
      sh('S1', 'course modules 3x3', 'black-contact-3x3', [
        cell('ninja', 'warped-wall', `${CUTOUT} Warped wall module.`),
        cell('ninja', 'salmon-ladder', `${CUTOUT} Salmon ladder rails.`),
        cell('ninja', 'monkey-bars', `${CUTOUT} Monkey bars segment.`),
        cell('ninja', 'cargo-net', `${CUTOUT} Cargo net.`),
        cell('ninja', 'balance-log', `${CUTOUT} Balance log.`),
        cell('ninja', 'swinging-steps', `${CUTOUT} Swinging step pods.`),
        cell('ninja', 'rope-climb', `${CUTOUT} Climbing rope.`),
        cell('ninja', 'vault-box-ninja', `${CUTOUT} Vault box.`),
        cell('ninja', 'landing-mat', `${CUTOUT} Landing mat.`),
      ]),
      sh('S2', 'grips holds 3x3', 'black-contact-3x3', [
        cell('ninja', 'hangboard', `${CUTOUT} Hangboard.`),
        cell('ninja', 'rings-ninja', `${CUTOUT} Gym rings.`),
        cell('ninja', 'pegboard', `${CUTOUT} Pegboard climb.`),
        cell('ninja', 'cliffhanger', `${CUTOUT} Cliffhanger ledges.`),
        cell('ninja', 'spinning-log', `${CUTOUT} Spinning log.`),
        cell('ninja', 'unstable-bridge', `${CUTOUT} Unstable bridge.`),
        cell('ninja', 'tire-obstacle', `${CUTOUT} Tire obstacle.`),
        cell('ninja', 'wall-traverse', `${CUTOUT} Traverse wall holds.`),
        cell('ninja', 'finish-bell', `${CUTOUT} Finish bell.`),
      ]),
      sh('S3', 'athlete kit 3x3', 'black-contact-3x3', [
        cell('ninja', 'grip-socks', `${CUTOUT} Grip socks pair.`),
        cell('ninja', 'chalk-bag-ninja', `${CUTOUT} Chalk bag.`),
        cell('ninja', 'wrist-wraps', `${CUTOUT} Wrist wraps.`),
        cell('ninja', 'kneepads-ninja', `${CUTOUT} Knee pads.`),
        cell('ninja', 'timer-gate', `${CUTOUT} Timing gate blank.`),
        cell('ninja', 'water-bottle', `${CUTOUT} Water bottle.`),
        cell('ninja', 'towel', `${CUTOUT} Towel.`),
        cell('ninja', 'medal-ninja', `${CUTOUT} Ninja medal blank.`),
        cell('ninja', 'trophy-ninja', `${CUTOUT} Ninja-course trophy blank.`),
      ]),
    ],
  }),

  /** ——— AGGRO WAVE7 (keep queue hot; discovered-new) ——— */

  'ki-comics-maker': wave('ki-comics-maker', {
    lane: 'other-discovered',
    bucket: 'other-discovered',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI comics + zine maker NEW (3×3 ×3)',
    why: 'Discovered creator hobby — comics/zine kit not in original prompt',
    sheets: [
      sh('S1', 'draw kit 3x3', 'black-contact-3x3', [
        cell('comic', 'sketchbook-comic', `${CUTOUT} Sketchbook blank.`),
        cell('comic', 'ink-pens', `${CUTOUT} Ink pen set.`),
        cell('comic', 'brush-pens', `${CUTOUT} Brush pens.`),
        cell('comic', 'markers-comic', `${CUTOUT} Marker set.`),
        cell('comic', 'ruler-triangle', `${CUTOUT} Triangle ruler blank.`),
        cell('comic', 'lightbox', `${CUTOUT} Lightbox blank.`),
        cell('comic', 'ink-bottle', `${CUTOUT} Ink bottle.`),
        cell('comic', 'eraser-kneaded', `${CUTOUT} Kneaded eraser.`),
        cell('comic', 'pencil-case', `${CUTOUT} Pencil case.`),
      ]),
      sh('S2', 'panels props 3x3', 'black-contact-3x3', [
        cell('comic', 'panel-grid-blank', `${CUTOUT} Comic panel grid blank (no letters).`),
        cell('comic', 'speech-bubble-blank', `${CUTOUT} Speech bubble shape ZERO letters.`),
        cell('comic', 'thought-bubble-blank', `${CUTOUT} Thought bubble ZERO letters.`),
        cell('comic', 'action-burst-blank', `${CUTOUT} Action burst shape ZERO letters.`),
        cell('comic', 'staple-zine', `${CUTOUT} Stapled mini zine blank.`),
        cell('comic', 'printer-comic', `${CUTOUT} Printer blank screen.`),
        cell('comic', 'tablet-draw-blank', `${CUTOUT} Drawing tablet blank screen.`),
        cell('comic', 'stylus', `${CUTOUT} Stylus.`),
        cell('comic', 'reference-board', `${CUTOUT} Reference cork board blank.`),
      ]),
      sh('S3', 'character props 3x3', 'black-contact-3x3', [
        cell('comic', 'hero-cape-prop', `${CUTOUT} Cape prop.`),
        cell('comic', 'mask-blank-comic', `${CUTOUT} Blank eye mask (generic, not IP).`),
        cell('comic', 'prop-sword-soft', `${CUTOUT} Soft prop sword.`),
        cell('comic', 'toy-camera', `${CUTOUT} Toy camera.`),
        cell('comic', 'model-stand', `${CUTOUT} Figure stand blank.`),
        cell('comic', 'portfolio-case', `${CUTOUT} Portfolio case.`),
        cell('comic', 'badge-con-blank', `${CUTOUT} Convention badge blank.`),
        cell('comic', 'table-cloth', `${CUTOUT} Artist alley tablecloth.`),
        cell('comic', 'trophy-comic', `${CUTOUT} Comics-club trophy blank.`),
      ]),
    ],
  }),

  'ki-kite-festival': wave('ki-kite-festival', {
    lane: 'other-discovered',
    bucket: 'other-discovered',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI kite festival NEW (3×3 ×3)',
    why: 'Discovered outdoor festival hobby — kite forms + reels',
    sheets: [
      sh('S1', 'kite forms 3x3', 'black-contact-3x3', [
        cell('kite', 'diamond-kite', `${CUTOUT} Diamond kite.`),
        cell('kite', 'delta-kite', `${CUTOUT} Delta kite.`),
        cell('kite', 'box-kite', `${CUTOUT} Box kite.`),
        cell('kite', 'dragon-kite', `${CUTOUT} Friendly dragon kite (generic, not IP).`),
        cell('kite', 'bird-kite', `${CUTOUT} Bird-shaped kite.`),
        cell('kite', 'stunt-dual', `${CUTOUT} Dual-line stunt kite.`),
        cell('kite', 'parafoil', `${CUTOUT} Parafoil kite.`),
        cell('kite', 'mini-kite', `${CUTOUT} Mini kite.`),
        cell('kite', 'kite-tail', `${CUTOUT} Long kite tail.`),
      ]),
      sh('S2', 'flight gear 3x3', 'black-contact-3x3', [
        cell('kite', 'kite-reel', `${CUTOUT} Kite string reel.`),
        cell('kite', 'string-winder', `${CUTOUT} String winder.`),
        cell('kite', 'gloves-kite', `${CUTOUT} Flying gloves.`),
        cell('kite', 'spike-anchor', `${CUTOUT} Ground spike anchor.`),
        cell('kite', 'wind-meter-blank', `${CUTOUT} Wind meter blank dial.`),
        cell('kite', 'repair-tape', `${CUTOUT} Repair tape roll.`),
        cell('kite', 'spare-spars', `${CUTOUT} Spare spar sticks.`),
        cell('kite', 'kite-bag', `${CUTOUT} Kite bag.`),
        cell('kite', 'picnic-blanket', `${CUTOUT} Picnic blanket.`),
      ]),
      sh('S3', 'festival props 3x3', 'black-contact-3x3', [
        cell('kite', 'windsock', `${CUTOUT} Windsock.`),
        cell('kite', 'banner-blank', `${CUTOUT} Festival banner ZERO letters.`),
        cell('kite', 'flag-blank', `${CUTOUT} Flag blank (no emblems).`),
        cell('kite', 'balloon-cluster', `${CUTOUT} Balloon cluster.`),
        cell('kite', 'lawn-chair', `${CUTOUT} Lawn chair.`),
        cell('kite', 'cooler', `${CUTOUT} Cooler.`),
        cell('kite', 'megaphone-blank', `${CUTOUT} Megaphone blank.`),
        cell('kite', 'medal-kite', `${CUTOUT} Kite medal blank.`),
        cell('kite', 'trophy-kite', `${CUTOUT} Kite-festival trophy blank.`),
      ]),
    ],
  }),
  ...
  'ki-model-trains': wave('ki-model-trains', {
    lane: 'other-discovered',
    bucket: 'other-discovered',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI model trains club NEW (3×3 ×3)',
    why: 'Discovered hobby — model rail not in original prompt',
    sheets: [
      sh('S1', 'trains 3x3', 'black-contact-3x3', [
        cell('train', 'loco-steam', `${CUTOUT} Model steam locomotive (generic, no logos).`),
        cell('train', 'loco-diesel', `${CUTOUT} Model diesel loco blank.`),
        cell('train', 'passenger-car', `${CUTOUT} Passenger car.`),
        cell('train', 'freight-boxcar', `${CUTOUT} Boxcar.`),
        cell('train', 'tank-car', `${CUTOUT} Tank car.`),
        cell('train', 'caboose', `${CUTOUT} Caboose.`),
        cell('train', 'tram', `${CUTOUT} Model tram.`),
        cell('train', 'bullet-train', `${CUTOUT} Bullet-train model blank.`),
        cell('train', 'handcar', `${CUTOUT} Handcar.`),
      ]),
      sh('S2', 'layout gear 3x3', 'black-contact-3x3', [
        cell('train', 'track-curve', `${CUTOUT} Curved track section.`),
        cell('train', 'track-straight', `${CUTOUT} Straight track section.`),
        cell('train', 'switch-track', `${CUTOUT} Switch/turnout.`),
        cell('train', 'bridge-model', `${CUTOUT} Model bridge.`),
        cell('train', 'tunnel-portal', `${CUTOUT} Tunnel portal.`),
        cell('train', 'station-blank', `${CUTOUT} Station building blank signs.`),
        cell('train', 'signal-light', `${CUTOUT} Signal light.`),
        cell('train', 'controller-blank', `${CUTOUT} Train controller blank (no logos/UI text).`),
        cell('train', 'power-pack', `${CUTOUT} Power pack blank.`),
      ]),
      sh('S3', 'scenery 3x3', 'black-contact-3x3', [
        cell('train', 'tree-model', `${CUTOUT} Model tree.`),
        cell('train', 'building-kit', `${CUTOUT} Building kit box blank.`),
        cell('train', 'people-figures-blank', `${CUTOUT} Tiny blank figure set (no faces detail).`),
        cell('train', 'car-model', `${CUTOUT} Tiny car model blank.`),
        cell('train', 'bench-platform', `${CUTOUT} Platform bench.`),
        cell('train', 'water-tower', `${CUTOUT} Water tower.`),
        cell('train', 'crossing-gate', `${CUTOUT} Crossing gate.`),
        cell('train', 'display-shelf', `${CUTOUT} Display shelf.`),
        cell('train', 'trophy-train', `${CUTOUT} Model-rail trophy blank.`),
      ]),
    ],
  }),

  'ki-yo-yo-club': wave('ki-yo-yo-club', {
    lane: 'other-discovered',
    bucket: 'other-discovered',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI yo-yo + skill toys NEW (3×3 ×3)',
    why: 'Discovered skill-toy club beyond one yo-yo',
    sheets: [
      sh('S1', 'yo-yos 3x3', 'black-contact-3x3', [
        cell('yoyo', 'yoyo-classic', `${CUTOUT} Classic yo-yo.`),
        cell('yoyo', 'yoyo-butterfly', `${CUTOUT} Butterfly yo-yo.`),
        cell('yoyo', 'yoyo-metal', `${CUTOUT} Metal yo-yo blank.`),
        cell('yoyo', 'yoyo-beginner', `${CUTOUT} Beginner yo-yo.`),
        cell('yoyo', 'yoyo-glow', `${CUTOUT} Glow yo-yo.`),
        cell('yoyo', 'string-pack', `${CUTOUT} Yo-yo string pack blank.`),
        cell('yoyo', 'bearing', `${CUTOUT} Oversized bearing (board-scale).`),
        cell('yoyo', 'response-pads', `${CUTOUT} Response pads.`),
        cell('yoyo', 'yoyo-holster', `${CUTOUT} Yo-yo holster.`),
      ]),
      sh('S2', 'skill toys 3x3', 'black-contact-3x3', [
        cell('yoyo', 'kendama', `${CUTOUT} Kendama.`),
        cell('yoyo', 'diabolo-skill', `${CUTOUT} Diabolo + sticks.`),
        cell('yoyo', 'spin-tops', `${CUTOUT} Spinning tops set.`),
        cell('yoyo', 'juggling-balls-skill', `${CUTOUT} Juggling balls.`),
        cell('yoyo', 'chinese-yo', `${CUTOUT} Chinese yo / reel.`),
        cell('yoyo', 'fidget-spinner-blank', `${CUTOUT} Generic spinner blank (no logos).`),
        cell('yoyo', 'fingerboard-skill', `${CUTOUT} Fingerboard.`),
        cell('yoyo', 'pen-spin', `${CUTOUT} Pen for spinning (blank).`),
        cell('yoyo', 'cardistry-deck-blank', `${CUTOUT} Blank-faced card deck.`),
      ]),
      sh('S3', 'contest kit 3x3', 'black-contact-3x3', [
        cell('yoyo', 'contest-mat', `${CUTOUT} Contest mat.`),
        cell('yoyo', 'timer-yoyo', `${CUTOUT} Timer blank.`),
        cell('yoyo', 'score-cards-blank', `${CUTOUT} Score cards ZERO writing.`),
        cell('yoyo', 'banner-blank', `${CUTOUT} Club banner ZERO letters.`),
        cell('yoyo', 'case-yoyo', `${CUTOUT} Yo-yo case.`),
        cell('yoyo', 'oil-bottle', `${CUTOUT} Tiny oil bottle blank.`),
        cell('yoyo', 'tool-kit', `${CUTOUT} Mini tool kit.`),
        cell('yoyo', 'medal-yoyo', `${CUTOUT} Yo-yo medal blank.`),
        cell('yoyo', 'trophy-yoyo', `${CUTOUT} Yo-yo club trophy blank.`),
      ]),
    ],
  }),

  /** ——— AGGRO WAVE9 (majority discovered-new) ——— */

  'ki-camping-outdoors': wave('ki-camping-outdoors', {
    lane: 'other-discovered',
    bucket: 'other-discovered',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI camping outdoors kit NEW (3×3 ×3)',
    why: 'Discovered outdoors hobby — camping kit depth not in original prompt',
    sheets: [
      sh('S1', 'shelter sleep 3x3', 'black-contact-3x3', [
        cell('camp', 'tent-dome', `${CUTOUT} Dome tent.`),
        cell('camp', 'tent-tunnel', `${CUTOUT} Tunnel tent.`),
        cell('camp', 'sleeping-bag-camp', `${CUTOUT} Sleeping bag.`),
        cell('camp', 'sleeping-pad', `${CUTOUT} Sleeping pad.`),
        cell('camp', 'camp-pillow', `${CUTOUT} Camp pillow.`),
        cell('camp', 'hammock-camp', `${CUTOUT} Camping hammock.`),
        cell('camp', 'tarp', `${CUTOUT} Tarp shelter.`),
        cell('camp', 'guyline-stakes', `${CUTOUT} Guyline + stakes set.`),
        cell('camp', 'lantern', `${CUTOUT} Camp lantern.`),
      ]),
      sh('S2', 'cook fire 3x3', 'black-contact-3x3', [
        cell('camp', 'camp-stove', `${CUTOUT} Camp stove.`),
        cell('camp', 'fuel-canister', `${CUTOUT} Fuel canister blank.`),
        cell('camp', 'cook-pot', `${CUTOUT} Cook pot.`),
        cell('camp', 'mess-kit', `${CUTOUT} Mess kit.`),
        cell('camp', 'thermos-camp', `${CUTOUT} Thermos.`),
        cell('camp', 'fire-pit-ring', `${CUTOUT} Fire pit ring.`),
        cell('camp', 'marshmallow-stick', `${CUTOUT} Marshmallow roasting stick.`),
        cell('camp', 'cooler-camp', `${CUTOUT} Cooler.`),
        cell('camp', 'water-jug', `${CUTOUT} Water jug.`),
      ]),
      sh('S3', 'trail pack 3x3', 'black-contact-3x3', [
        cell('camp', 'backpack-camp', `${CUTOUT} Hiking backpack.`),
        cell('camp', 'trekking-poles-camp', `${CUTOUT} Trekking poles.`),
        cell('camp', 'compass-camp', `${CUTOUT} Compass blank dial.`),
        cell('camp', 'map-case-blank', `${CUTOUT} Map case blank.`),
        cell('camp', 'multi-tool-camp', `${CUTOUT} Multi-tool blank.`),
        cell('camp', 'first-aid-pouch', `${CUTOUT} First-aid pouch.`),
        cell('camp', 'headlamp-camp', `${CUTOUT} Headlamp.`),
        cell('camp', 'binoculars-camp', `${CUTOUT} Binoculars.`),
        cell('camp', 'trophy-camp', `${CUTOUT} Camping-club trophy blank.`),
      ]),
    ],
  }),

  'ki-fishing-tackle': wave('ki-fishing-tackle', {
    lane: 'sports',
    bucket: 'sports',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI fishing tackle club NEW (3×3 ×3)',
    why: 'Discovered — fishing kit beyond one rod',
    sheets: [
      sh('S1', 'rods reels 3x3', 'black-contact-3x3', [
        cell('fish', 'spinning-rod', `${CUTOUT} Spinning rod + reel.`),
        cell('fish', 'fly-rod', `${CUTOUT} Fly rod.`),
        cell('fish', 'ice-rod', `${CUTOUT} Ice fishing rod.`),
        cell('fish', 'reel-spare', `${CUTOUT} Spare reel.`),
        cell('fish', 'rod-holder', `${CUTOUT} Rod holder.`),
        cell('fish', 'landing-net', `${CUTOUT} Landing net.`),
        cell('fish', 'tackle-box', `${CUTOUT} Tackle box open.`),
        cell('fish', 'lure-tray', `${CUTOUT} Lure tray (abstract shapes, no brands).`),
        cell('fish', 'bobber-set', `${CUTOUT} Bobber set.`),
      ]),
      sh('S2', 'shore kit 3x3', 'black-contact-3x3', [
        cell('fish', 'bait-bucket', `${CUTOUT} Bait bucket.`),
        cell('fish', 'keepnet', `${CUTOUT} Keepnet.`),
        cell('fish', 'fish-ruler-blank', `${CUTOUT} Fish ruler blank (no numbers).`),
        cell('fish', 'pliers', `${CUTOUT} Fishing pliers.`),
        cell('fish', 'line-spool', `${CUTOUT} Line spool blank.`),
        cell('fish', 'hook-case', `${CUTOUT} Hook case blank.`),
        cell('fish', 'vest-fish', `${CUTOUT} Fishing vest.`),
        cell('fish', 'waders', `${CUTOUT} Waders.`),
        cell('fish', 'hat-fish', `${CUTOUT} Fishing hat.`),
      ]),
      sh('S3', 'catch still-life 3x3', 'black-contact-3x3', [
        cell('fish', 'trout', `${CUTOUT} Trout still-life.`),
        cell('fish', 'bass', `${CUTOUT} Bass still-life.`),
        cell('fish', 'goldfish-pond', `${CUTOUT} Pond goldfish.`),
        cell('fish', 'crab', `${CUTOUT} Crab.`),
        cell('fish', 'clam', `${CUTOUT} Clam.`),
        cell('fish', 'ice-auger', `${CUTOUT} Ice auger.`),
        cell('fish', 'tip-up', `${CUTOUT} Ice tip-up.`),
        cell('fish', 'cooler-fish', `${CUTOUT} Fish cooler.`),
        cell('fish', 'trophy-fish', `${CUTOUT} Fishing trophy blank.`),
      ]),
    ],
  }),

  'ki-baking-cafe': wave('ki-baking-cafe', {
    lane: 'hobbies',
    bucket: 'hobbies',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI baking cafe club NEW (3×3 ×3)',
    why: 'Discovered — bakery cafe props beyond basic cook kit',
    sheets: [
      sh('S1', 'bake tools 3x3', 'black-contact-3x3', [
        cell('bake', 'stand-mixer', `${CUTOUT} Stand mixer blank (no logos).`),
        cell('bake', 'hand-mixer', `${CUTOUT} Hand mixer.`),
        cell('bake', 'piping-bag', `${CUTOUT} Piping bag + tips.`),
        cell('bake', 'cake-tin', `${CUTOUT} Cake tin.`),
        cell('bake', 'loaf-pan', `${CUTOUT} Loaf pan.`),
        cell('bake', 'cookie-sheet', `${CUTOUT} Cookie sheet.`),
        cell('bake', 'rolling-pin-bake', `${CUTOUT} Rolling pin.`),
        cell('bake', 'pastry-brush', `${CUTOUT} Pastry brush.`),
        cell('bake', 'sifter', `${CUTOUT} Flour sifter.`),
      ]),
      sh('S2', 'pastries 3x3', 'black-contact-3x3', [
        cell('bake', 'croissant', `${CUTOUT} Croissant.`),
        cell('bake', 'donut', `${CUTOUT} Donut.`),
        cell('bake', 'muffin', `${CUTOUT} Muffin.`),
        cell('bake', 'baguette', `${CUTOUT} Baguette.`),
        cell('bake', 'pie', `${CUTOUT} Pie.`),
        cell('bake', 'layer-cake', `${CUTOUT} Layer cake blank (no text).`),
        cell('bake', 'macaron-tower', `${CUTOUT} Macaron tower.`),
        cell('bake', 'cinnamon-roll', `${CUTOUT} Cinnamon roll.`),
        cell('bake', 'pretzel', `${CUTOUT} Soft pretzel.`),
      ]),
      sh('S3', 'cafe props 3x3', 'black-contact-3x3', [
        cell('bake', 'display-case', `${CUTOUT} Pastry display case.`),
        cell('bake', 'coffee-pot', `${CUTOUT} Coffee pot.`),
        cell('bake', 'espresso-blank', `${CUTOUT} Espresso machine blank (no logos/UI).`),
        cell('bake', 'menu-board-blank', `${CUTOUT} Menu board ZERO letters.`),
        cell('bake', 'cake-stand-bake', `${CUTOUT} Cake stand.`),
        cell('bake', 'apron-bake', `${CUTOUT} Baker apron blank.`),
        cell('bake', 'chef-hat', `${CUTOUT} Chef hat.`),
        cell('bake', 'order-bell', `${CUTOUT} Order bell.`),
        cell('bake', 'trophy-bake', `${CUTOUT} Baking-club trophy blank.`),
      ]),
    ],
  }),

  'ki-skate-street-deep': wave('ki-skate-street-deep', {
    lane: 'vehicles-leisure',
    bucket: 'vehicles-leisure',
    in_prompt_named: true,
    title: 'KI skate street depth (3×3 ×3)',
    why: 'Vehicles/boards still thin — street skate parts + obstacles',
    sheets: [
      sh('S1', 'decks trucks 3x3', 'black-contact-3x3', [
        cell('sk8', 'deck-blank', `${CUTOUT} Skateboard deck blank (no logos).`),
        cell('sk8', 'longboard-deck', `${CUTOUT} Longboard deck blank.`),
        cell('sk8', 'cruiser-deck', `${CUTOUT} Cruiser deck.`),
        cell('sk8', 'trucks', `${CUTOUT} Skate trucks pair.`),
        cell('sk8', 'wheels-set', `${CUTOUT} Wheel set.`),
        cell('sk8', 'bearings-pack', `${CUTOUT} Bearings pack blank.`),
        cell('sk8', 'grip-tape', `${CUTOUT} Grip tape sheet.`),
        cell('sk8', 'skate-tool', `${CUTOUT} Skate tool.`),
        cell('sk8', 'complete-board', `${CUTOUT} Complete skateboard blank.`),
      ]),
      sh('S2', 'obstacles 3x3', 'black-contact-3x3', [
        cell('sk8', 'manual-pad', `${CUTOUT} Manual pad.`),
        cell('sk8', 'kicker-ramp', `${CUTOUT} Kicker ramp.`),
        cell('sk8', 'rail-flatbar', `${CUTOUT} Flatbar rail.`),
        cell('sk8', 'pyramid', `${CUTOUT} Skate pyramid.`),
        cell('sk8', 'stairs-set', `${CUTOUT} Mini stairs set.`),
        cell('sk8', 'ledge-skate', `${CUTOUT} Skate ledge.`),
        cell('sk8', 'bowl-section', `${CUTOUT} Bowl section module.`),
        cell('sk8', 'spine', `${CUTOUT} Spine mini-ramp.`),
        cell('sk8', 'bank-to-wall', `${CUTOUT} Bank-to-wall module.`),
      ]),
      sh('S3', 'street kit 3x3', 'black-contact-3x3', [
        cell('sk8', 'helmet-skate', `${CUTOUT} Skate helmet.`),
        cell('sk8', 'pads-set', `${CUTOUT} Pad set.`),
        cell('sk8', 'skate-backpack', `${CUTOUT} Skate backpack.`),
        cell('sk8', 'wax', `${CUTOUT} Skate wax.`),
        cell('sk8', 'water-bottle-sk8', `${CUTOUT} Water bottle.`),
        cell('sk8', 'beanie', `${CUTOUT} Beanie.`),
        cell('sk8', 'hoodie', `${CUTOUT} Hoodie on hanger blank.`),
        cell('sk8', 'medal-skate', `${CUTOUT} Skate medal blank.`),
        cell('sk8', 'trophy-skate', `${CUTOUT} Skate trophy blank.`),
      ]),
    ],
  }),

  'ki-radio-ham': wave('ki-radio-ham', {
    lane: 'other-discovered',
    bucket: 'other-discovered',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI radio + ham club NEW (3×3 ×3)',
    why: 'Discovered tech club — ham/radio kit not in prompt',
    sheets: [
      sh('S1', 'radios 3x3', 'black-contact-3x3', [
        cell('radio', 'handheld-radio', `${CUTOUT} Handheld radio blank screen.`),
        cell('radio', 'base-station', `${CUTOUT} Base station radio blank.`),
        cell('radio', 'walkie-pair-radio', `${CUTOUT} Walkie-talkie pair.`),
        cell('radio', 'shortwave', `${CUTOUT} Shortwave radio blank.`),
        cell('radio', 'cb-mic', `${CUTOUT} Radio mic.`),
        cell('radio', 'headset-radio', `${CUTOUT} Radio headset.`),
        cell('radio', 'morse-key', `${CUTOUT} Morse key.`),
        cell('radio', 'swr-meter-blank', `${CUTOUT} Meter blank dial.`),
        cell('radio', 'power-supply', `${CUTOUT} Power supply blank.`),
      ]),
      sh('S2', 'antennas 3x3', 'black-contact-3x3', [
        cell('radio', 'antenna-mast', `${CUTOUT} Antenna mast.`),
        cell('radio', 'yagi', `${CUTOUT} Yagi antenna.`),
        cell('radio', 'dipole', `${CUTOUT} Dipole wire antenna.`),
        cell('radio', 'magnetic-mount', `${CUTOUT} Mag-mount antenna.`),
        cell('radio', 'coax-coil', `${CUTOUT} Coax cable coil.`),
        cell('radio', 'balun', `${CUTOUT} Balun box blank.`),
        cell('radio', 'ground-plane', `${CUTOUT} Ground-plane antenna.`),
        cell('radio', 'portable-mast', `${CUTOUT} Portable mast kit.`),
        cell('radio', 'antenna-analyzer-blank', `${CUTOUT} Analyzer blank screen.`),
      ]),
      sh('S3', 'club desk 3x3', 'black-contact-3x3', [
        cell('radio', 'logbook-blank', `${CUTOUT} Logbook blank.`),
        cell('radio', 'qsl-card-blank', `${CUTOUT} QSL card blank (no text).`),
        cell('radio', 'world-map-blank', `${CUTOUT} World map blank.`),
        cell('radio', 'desk-lamp-radio', `${CUTOUT} Desk lamp.`),
        cell('radio', 'clock-utc-blank', `${CUTOUT} Clock blank face.`),
        cell('radio', 'badge-radio', `${CUTOUT} Club badge blank.`),
        cell('radio', 'go-box', `${CUTOUT} Radio go-box.`),
        cell('radio', 'solar-panel-small', `${CUTOUT} Small solar panel.`),
        cell('radio', 'trophy-radio', `${CUTOUT} Radio-club trophy blank.`),
      ]),
    ],
  }),

  /** ——— AGGRO WAVE10 (keep runway; majority NEW) ——— */

  'ki-ice-hockey-club': wave('ki-ice-hockey-club', {
    lane: 'sports',
    bucket: 'sports',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI ice hockey club NEW (3×3 ×3)',
    why: 'Discovered hockey depth',
    sheets: [
      sh('S1', 'hockey gear 3x3', 'black-contact-3x3', [
        cell('hockey', 'stick', `${CUTOUT} Hockey stick.`),
        cell('hockey', 'puck', `${CUTOUT} Hockey puck.`),
        cell('hockey', 'helmet-cage', `${CUTOUT} Hockey helmet with cage.`),
        cell('hockey', 'gloves', `${CUTOUT} Hockey gloves.`),
        cell('hockey', 'skates-ice', `${CUTOUT} Ice hockey skates.`),
        cell('hockey', 'shin-pads', `${CUTOUT} Shin pads.`),
        cell('hockey', 'goalie-mask', `${CUTOUT} Goalie mask.`),
        cell('hockey', 'goalie-pad', `${CUTOUT} Goalie leg pad.`),
        cell('hockey', 'net-goal', `${CUTOUT} Hockey goal net.`),
      ]),
      sh('S2', 'rink kit 3x3', 'black-contact-3x3', [
        cell('hockey', 'puck-bag', `${CUTOUT} Puck bag.`),
        cell('hockey', 'tape-roll', `${CUTOUT} Stick tape.`),
        cell('hockey', 'jersey-blank', `${CUTOUT} Jersey blank (no numbers/logos).`),
        cell('hockey', 'cone-drill', `${CUTOUT} Drill cone.`),
        cell('hockey', 'shooting-pad', `${CUTOUT} Shooting pad.`),
        cell('hockey', 'stick-rack', `${CUTOUT} Stick rack.`),
        cell('hockey', 'ball-street', `${CUTOUT} Street-hockey ball.`),
        cell('hockey', 'net-folding', `${CUTOUT} Folding street net.`),
        cell('hockey', 'trophy-hockey', `${CUTOUT} Hockey trophy blank.`),
      ]),
    ],
  }),

  'ki-escape-room-props': wave('ki-escape-room-props', {
    lane: 'other-discovered',
    bucket: 'other-discovered',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI escape-room mystery props NEW (3×3 ×3)',
    why: 'Discovered mystery/puzzle props',
    sheets: [
      sh('S1', 'locks clues 3x3', 'black-contact-3x3', [
        cell('esc', 'padlock', `${CUTOUT} Padlock.`),
        cell('esc', 'combo-lock-blank', `${CUTOUT} Combination lock blank dial.`),
        cell('esc', 'key-set', `${CUTOUT} Old key set.`),
        cell('esc', 'locked-box', `${CUTOUT} Locked puzzle box.`),
        cell('esc', 'cipher-disk-blank', `${CUTOUT} Cipher disk ZERO letters.`),
        cell('esc', 'magnifier-clue', `${CUTOUT} Magnifying glass.`),
        cell('esc', 'flashlight-uv', `${CUTOUT} UV flashlight.`),
        cell('esc', 'note-blank', `${CUTOUT} Folded note ZERO writing.`),
        cell('esc', 'map-fragment-blank', `${CUTOUT} Map fragment blank.`),
      ]),
      sh('S2', 'room props 3x3', 'black-contact-3x3', [
        cell('esc', 'hourglass', `${CUTOUT} Hourglass.`),
        cell('esc', 'safe-box', `${CUTOUT} Small safe blank.`),
        cell('esc', 'book-hollow', `${CUTOUT} Hollow book blank.`),
        cell('esc', 'painting-frame', `${CUTOUT} Empty picture frame.`),
        cell('esc', 'drawer-unit', `${CUTOUT} Small drawer unit.`),
        cell('esc', 'lantern', `${CUTOUT} Lantern.`),
        cell('esc', 'rope-coil', `${CUTOUT} Rope coil.`),
        cell('esc', 'walkie-pair', `${CUTOUT} Walkie-talkie pair.`),
        cell('esc', 'trophy-escape', `${CUTOUT} Escape trophy blank.`),
      ]),
    ],
  }),

  'ki-vinyl-dj-deep': wave('ki-vinyl-dj-deep', {
    lane: 'music',
    bucket: 'music',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI vinyl DJ deep NEW (3×3 ×2)',
    why: 'Discovered DJ/vinyl depth',
    sheets: [
      sh('S1', 'dj kit 3x3', 'black-contact-3x3', [
        cell('dj', 'turntable-pair', `${CUTOUT} Pair of turntables blank labels.`),
        cell('dj', 'mixer-dj', `${CUTOUT} DJ mixer blank.`),
        cell('dj', 'headphones-dj', `${CUTOUT} DJ headphones.`),
        cell('dj', 'vinyl-crate', `${CUTOUT} Vinyl crate.`),
        cell('dj', 'slipmats', `${CUTOUT} Slipmats blank.`),
        cell('dj', 'controller-pad-dj', `${CUTOUT} Pad controller blank.`),
        cell('dj', 'laptop-dj-blank', `${CUTOUT} Laptop blank screen.`),
        cell('dj', 'speaker-pair', `${CUTOUT} Speaker pair blank.`),
        cell('dj', 'flight-case', `${CUTOUT} Flight case.`),
      ]),
      sh('S2', 'booth 3x3', 'black-contact-3x3', [
        cell('dj', 'booth-stand', `${CUTOUT} DJ booth stand.`),
        cell('dj', 'light-bar', `${CUTOUT} Light bar.`),
        cell('dj', 'smoke-machine', `${CUTOUT} Fog machine.`),
        cell('dj', 'cable-snake', `${CUTOUT} Cable snake.`),
        cell('dj', 'usb-stick', `${CUTOUT} USB stick blank.`),
        cell('dj', 'setlist-blank', `${CUTOUT} Setlist clipboard ZERO letters.`),
        cell('dj', 'trophy-dj', `${CUTOUT} DJ trophy blank.`),
        cell('dj', 'cartridge', `${CUTOUT} Cartridge headshell.`),
        cell('dj', 'crossfader', `${CUTOUT} Crossfader module.`),
      ]),
    ],
  }),

  'ki-martial-dojo': wave('ki-martial-dojo', {
    lane: 'sports',
    bucket: 'sports',
    in_prompt_named: true,
    title: 'KI martial dojo kit (3×3 ×2)',
    why: 'Martial thin — dojo kit',
    sheets: [
      sh('S1', 'uniforms gear 3x3', 'black-contact-3x3', [
        cell('dojo', 'gi-folded', `${CUTOUT} Folded gi.`),
        cell('dojo', 'belt-stack', `${CUTOUT} Belt stack blank.`),
        cell('dojo', 'gloves-spar', `${CUTOUT} Sparring gloves.`),
        cell('dojo', 'headgear', `${CUTOUT} Headgear.`),
        cell('dojo', 'shin-guards', `${CUTOUT} Shin guards.`),
        cell('dojo', 'focus-mitts', `${CUTOUT} Focus mitts.`),
        cell('dojo', 'kick-shield', `${CUTOUT} Kick shield.`),
        cell('dojo', 'mat-tile', `${CUTOUT} Puzzle mat tile.`),
        cell('dojo', 'trophy-dojo', `${CUTOUT} Dojo trophy blank.`),
      ]),
      sh('S2', 'safe weapons 3x3', 'black-contact-3x3', [
        cell('dojo', 'bo-staff-safe', `${CUTOUT} Soft practice staff.`),
        cell('dojo', 'nunchaku-foam', `${CUTOUT} Foam nunchaku.`),
        cell('dojo', 'breaking-board', `${CUTOUT} Breaking board.`),
        cell('dojo', 'target-pad', `${CUTOUT} Wall target pad.`),
        cell('dojo', 'jump-rope', `${CUTOUT} Jump rope.`),
        cell('dojo', 'agility-ladder', `${CUTOUT} Agility ladder.`),
        cell('dojo', 'timer', `${CUTOUT} Round timer blank.`),
        cell('dojo', 'banner-blank', `${CUTOUT} Banner ZERO letters.`),
        cell('dojo', 'duffel', `${CUTOUT} Duffel bag.`),
      ]),
    ],
  }),

  'ow-wave30-clubs': wave('ow-wave30-clubs', {
    stockpile: 'overview-worlds',
    lane: 'overview',
    bucket: 'leisure',
    ...
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'OW wave30 — rink + bakery street + escape loft (FULL-PAGE ×3)',
    why: 'NEW overviews for hockey/pastry/escape',
    sheets: [
      sh('S1', 'ice rink overview', 'full-page-overview', [
        owCell('ice-rink-complex', `${OVERVIEW} Ice rink complex: rink oval, bleachers, lobby plaza, skate-rental blank, path. 5 zones. No people/text.`),
      ]),
      sh('S2', 'bakery street overview', 'full-page-overview', [
        owCell('bakery-street', `${OVERVIEW} Bakery street: pastry shop blank, outdoor tables, mill edge, market cart, path. 5 zones. No people/text. NEW.`),
      ]),
      sh('S3', 'escape loft overview', 'full-page-overview', [
        owCell('escape-loft', `${OVERVIEW} Mystery loft: puzzle rooms, clue table, locked door wall, exit stair, open floor. 5 zones. No people/text. NEW.`),
      ]),
    ],
  }),

  'ow-wave31-outdoors': wave('ow-wave31-outdoors', {
    stockpile: 'overview-worlds',
    lane: 'overview',
    bucket: 'nature-adventure',
    ...
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'OW wave31 — skate night + bird marsh + dig quarry (FULL-PAGE ×3)',
    why: 'NEW outdoor overviews',
    sheets: [
      sh('S1', 'skate plaza night', 'full-page-overview', [
        owCell('skate-plaza-night', `${OVERVIEW} Night skate plaza: bowl, street pad, rail zone, lamp path, bench. Soft lights. 5 zones. No graffiti letters. No people.`),
      ]),
      sh('S2', 'bird marsh overview', 'full-page-overview', [
        owCell('bird-marsh', `${OVERVIEW} Bird marsh: boardwalk, hide shelter, reed pond, lookout, trail. 5 zones. No people/text. NEW.`),
      ]),
      sh('S3', 'dig quarry overview', 'full-page-overview', [
        owCell('dig-quarry', `${OVERVIEW} Fossil dig quarry: dig pits, tool shed, display tent blank, path, picnic. 5 zones. No people/text. NEW.`),
      ]),
    ],
  }),

  /** ——— AGGRO WAVE11 (majority discovered-new) ——— */

  'ki-pottery-studio': wave('ki-pottery-studio', {
    lane: 'crafts-maker',
    bucket: 'crafts-maker',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI pottery studio NEW (3×3 ×3)',
    why: 'Discovered maker hobby — pottery beyond one wheel',
    sheets: [
      sh('S1', 'wheel clay 3x3', 'black-contact-3x3', [
        cell('pot', 'pottery-wheel', `${CUTOUT} Pottery wheel with clay mound.`),
        cell('pot', 'clay-block', `${CUTOUT} Clay block.`),
        cell('pot', 'clay-tools', `${CUTOUT} Clay tool set.`),
        cell('pot', 'wire-cutter', `${CUTOUT} Clay wire cutter.`),
        cell('pot', 'sponge', `${CUTOUT} Pottery sponge.`),
        cell('pot', 'ribs', `${CUTOUT} Pottery ribs set.`),
        cell('pot', 'banding-wheel', `${CUTOUT} Banding wheel.`),
        cell('pot', 'slab-roller', `${CUTOUT} Slab roller.`),
        cell('pot', 'apron-pot', `${CUTOUT} Potter apron blank.`),
      ]),
      sh('S2', 'forms kiln 3x3', 'black-contact-3x3', [
        cell('pot', 'vase', `${CUTOUT} Ceramic vase.`),
        cell('pot', 'bowl-pot', `${CUTOUT} Ceramic bowl.`),
        cell('pot', 'mug', `${CUTOUT} Ceramic mug blank.`),
        cell('pot', 'plate', `${CUTOUT} Ceramic plate.`),
        cell('pot', 'teapot', `${CUTOUT} Teapot.`),
        cell('pot', 'kiln', `${CUTOUT} Small kiln blank.`),
        cell('pot', 'glaze-jars', `${CUTOUT} Glaze jars blank.`),
        cell('pot', 'bisque-ware', `${CUTOUT} Bisque ware stack.`),
        cell('pot', 'drying-rack', `${CUTOUT} Drying rack.`),
      ]),
      sh('S3', 'studio kit 3x3', 'black-contact-3x3', [
        cell('pot', 'work-table', `${CUTOUT} Pottery work table.`),
        cell('pot', 'water-bucket', `${CUTOUT} Water bucket.`),
        cell('pot', 'batts', `${CUTOUT} Wheel batts stack.`),
        cell('pot', 'calipers', `${CUTOUT} Calipers blank.`),
        cell('pot', 'stamp-set-blank', `${CUTOUT} Clay stamp set blank (no letters).`),
        cell('pot', 'slip-trailer', `${CUTOUT} Slip trailer bottle.`),
        cell('pot', 'shelf-ware', `${CUTOUT} Ware shelf.`),
        cell('pot', 'medal-pot', `${CUTOUT} Pottery medal blank.`),
        cell('pot', 'trophy-pot', `${CUTOUT} Pottery-club trophy blank.`),
      ]),
    ],
  }),

  'ki-beekeeping-club': wave('ki-beekeeping-club', {
    lane: 'other-discovered',
    bucket: 'other-discovered',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI beekeeping club NEW (3×3 ×3)',
    why: 'Discovered nature hobby — beekeeping kit not in prompt',
    sheets: [
      sh('S1', 'hive gear 3x3', 'black-contact-3x3', [
        cell('bee', 'hive-box', `${CUTOUT} Beehive box.`),
        cell('bee', 'frames', `${CUTOUT} Hive frames.`),
        cell('bee', 'smoker', `${CUTOUT} Bee smoker.`),
        cell('bee', 'hive-tool', `${CUTOUT} Hive tool.`),
        cell('bee', 'bee-brush', `${CUTOUT} Bee brush.`),
        cell('bee', 'queen-cage', `${CUTOUT} Queen cage.`),
        cell('bee', 'feeder-jar', `${CUTOUT} Hive feeder jar.`),
        cell('bee', 'entrance-reducer', `${CUTOUT} Entrance reducer.`),
        cell('bee', 'stand-hive', `${CUTOUT} Hive stand.`),
      ]),
      sh('S2', 'suit honey 3x3', 'black-contact-3x3', [
        cell('bee', 'bee-suit', `${CUTOUT} Bee suit on hanger.`),
        cell('bee', 'veil-hat', `${CUTOUT} Veil hat.`),
        cell('bee', 'gloves-bee', `${CUTOUT} Bee gloves.`),
        cell('bee', 'honey-jar', `${CUTOUT} Honey jar.`),
        cell('bee', 'honeycomb', `${CUTOUT} Honeycomb frame.`),
        cell('bee', 'extractor', `${CUTOUT} Honey extractor.`),
        cell('bee', 'uncapping-knife', `${CUTOUT} Uncapping knife.`),
        cell('bee', 'bee-friendly-flowers', `${CUTOUT} Flower pot cluster.`),
        cell('bee', 'trophy-bee', `${CUTOUT} Bee-club trophy blank.`),
      ]),
      sh('S3', 'garden pollinate 3x3', 'black-contact-3x3', [
        cell('bee', 'bee-hotel', `${CUTOUT} Bee hotel.`),
        cell('bee', 'observation-hive', `${CUTOUT} Observation hive panel.`),
        cell('bee', 'magnifier-bee', `${CUTOUT} Magnifier.`),
        cell('bee', 'field-notebook-bee', `${CUTOUT} Field notebook blank.`),
        cell('bee', 'pollen-trap', `${CUTOUT} Pollen trap.`),
        cell('bee', 'wax-block', `${CUTOUT} Beeswax block.`),
        cell('bee', 'candle-mold', `${CUTOUT} Candle mold.`),
        cell('bee', 'sign-blank-bee', `${CUTOUT} Garden sign blank.`),
        cell('bee', 'medal-bee', `${CUTOUT} Bee medal blank.`),
      ]),
    ],
  }),

  'ki-candle-soap': wave('ki-candle-soap', {
    lane: 'crafts-maker',
    bucket: 'crafts-maker',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI candle + soap craft NEW (3×3 ×3)',
    why: 'Discovered craft — candle/soap making not in prompt',
    sheets: [
      sh('S1', 'candle kit 3x3', 'black-contact-3x3', [
        cell('cndl', 'wax-block', `${CUTOUT} Wax block.`),
        cell('cndl', 'wick-spool', `${CUTOUT} Wick spool.`),
        cell('cndl', 'pouring-pot', `${CUTOUT} Pouring pot.`),
        cell('cndl', 'thermometer', `${CUTOUT} Thermometer blank.`),
        cell('cndl', 'mold-pillar', `${CUTOUT} Pillar candle mold.`),
        cell('cndl', 'mold-jar', `${CUTOUT} Jar candle.`),
        cell('cndl', 'scent-oils', `${CUTOUT} Scent oil bottles blank.`),
        cell('cndl', 'dye-chips', `${CUTOUT} Dye chips.`),
        cell('cndl', 'wick-holder', `${CUTOUT} Wick holder.`),
      ]),
      sh('S2', 'soap kit 3x3', 'black-contact-3x3', [
        cell('cndl', 'soap-mold', `${CUTOUT} Soap mold tray.`),
        cell('cndl', 'soap-bars', `${CUTOUT} Soap bar stack.`),
        cell('cndl', 'mixing-bowl-soap', `${CUTOUT} Mixing bowl.`),
        cell('cndl', 'stick-blender', `${CUTOUT} Stick blender blank.`),
        cell('cndl', 'scale-soap', `${CUTOUT} Scale blank.`),
        cell('cndl', 'glycerin-block', `${CUTOUT} Glycerin block.`),
        cell('cndl', 'cutter-wire', `${CUTOUT} Soap cutter.`),
        cell('cndl', 'stamp-soap-blank', `${CUTOUT} Soap stamp blank (no letters).`),
        cell('cndl', 'drying-rack-soap', `${CUTOUT} Drying rack.`),
      ]),
      sh('S3', 'finish pack 3x3', 'black-contact-3x3', [
        cell('cndl', 'gift-box', `${CUTOUT} Gift box.`),
        cell('cndl', 'tissue-paper', `${CUTOUT} Tissue paper stack.`),
        cell('cndl', 'ribbon-spool', `${CUTOUT} Ribbon spool.`),
        cell('cndl', 'label-blank', `${CUTOUT} Label sheet ZERO letters.`),
        cell('cndl', 'apron-craft', `${CUTOUT} Craft apron.`),
        cell('cndl', 'gloves-craft', `${CUTOUT} Craft gloves.`),
        cell('cndl', 'candle-set', `${CUTOUT} Finished candle set.`),
        cell('cndl', 'soap-set', `${CUTOUT} Finished soap set.`),
        cell('cndl', 'trophy-craft', `${CUTOUT} Craft-club trophy blank.`),
      ]),
    ],
  }),

  'ki-brick-build': wave('ki-brick-build', {
    lane: 'crafts-maker',
    bucket: 'crafts-maker',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI brick build club NEW (3×3 ×3)',
    why: 'Discovered builder hobby — generic bricks (NO brand logos)',
    sheets: [
      sh('S1', 'bricks parts 3x3', 'black-contact-3x3', [
        cell('brick', 'brick-stack', `${CUTOUT} Stack of interlocking toy bricks (generic, no logos).`),
        cell('brick', 'plate-base', `${CUTOUT} Base plate.`),
        cell('brick', 'wheels-axles', `${CUTOUT} Wheel + axle set.`),
        cell('brick', 'hinge-parts', `${CUTOUT} Hinge parts.`),
        cell('brick', 'window-parts', `${CUTOUT} Window frame parts.`),
        cell('brick', 'door-parts', `${CUTOUT} Door parts.`),
        cell('brick', 'slope-bricks', `${CUTOUT} Slope bricks.`),
        cell('brick', 'technic-beam', `${CUTOUT} Beam pieces generic.`),
        cell('brick', 'minifig-blank', `${CUTOUT} Blank figure (generic, NOT licensed character).`),
      ]),
      sh('S2', 'builds 3x3', 'black-contact-3x3', [
        cell('brick', 'house-build', `${CUTOUT} Small brick house.`),
        cell('brick', 'car-build', `${CUTOUT} Brick car.`),
        cell('brick', 'spaceship-build', `${CUTOUT} Brick spaceship generic.`),
        cell('brick', 'robot-build', `${CUTOUT} Brick robot generic.`),
        cell('brick', 'castle-build', `${CUTOUT} Brick castle.`),
        cell('brick', 'bridge-build', `${CUTOUT} Brick bridge.`),
        cell('brick', 'crane-build', `${CUTOUT} Brick crane.`),
        cell('brick', 'animal-build', `${CUTOUT} Brick animal generic.`),
        cell('brick', 'city-block', `${CUTOUT} City block module.`),
      ]),
      sh('S3', 'club kit 3x3', 'black-contact-3x3', [
        cell('brick', 'sort-tray', `${CUTOUT} Sorting tray.`),
        cell('brick', 'storage-bin', `${CUTOUT} Storage bin.`),
        cell('brick', 'instruction-blank', `${CUTOUT} Instruction booklet blank (no text).`),
        cell('brick', 'build-mat', `${CUTOUT} Build mat.`),
        cell('brick', 'separator-tool', `${CUTOUT} Brick separator tool.`),
        cell('brick', 'display-case-brick', `${CUTOUT} Display case.`),
        cell('brick', 'timer-build', `${CUTOUT} Build timer blank.`),
        cell('brick', 'medal-build', `${CUTOUT} Build medal blank.`),
        cell('brick', 'trophy-build', `${CUTOUT} Build-club trophy blank.`),
      ]),
    ],
  }),

  /** ——— AGGRO WAVE12 (majority NEW) ——— */

  'ki-archery-range': wave('ki-archery-range', {
    lane: 'sports',
    bucket: 'sports',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI archery range NEW (3×3 ×3)',
    why: 'Discovered sport depth — archery beyond one bow',
    sheets: [
      sh('S1', 'bows arrows 3x3', 'black-contact-3x3', [
        cell('arch', 'recurve-bow', `${CUTOUT} Recurve bow.`),
        cell('arch', 'longbow', `${CUTOUT} Longbow.`),
        cell('arch', 'compound-bow', `${CUTOUT} Compound bow (unbranded).`),
        cell('arch', 'arrow-set', `${CUTOUT} Arrow set.`),
        cell('arch', 'quiver', `${CUTOUT} Quiver.`),
        cell('arch', 'arm-guard', `${CUTOUT} Arm guard.`),
        cell('arch', 'finger-tab', `${CUTOUT} Finger tab.`),
        cell('arch', 'chest-guard', `${CUTOUT} Chest guard.`),
        cell('arch', 'bow-stand', `${CUTOUT} Bow stand.`),
      ]),
      sh('S2', 'targets range 3x3', 'black-contact-3x3', [
        cell('arch', 'target-face', `${CUTOUT} Target face (rings as shapes, no numbers).`),
        cell('arch', 'target-butt', `${CUTOUT} Target butt.`),
        cell('arch', '3d-animal-blank', `${CUTOUT} Generic 3D animal target (not IP).`),
        cell('arch', 'bag-target', `${CUTOUT} Bag target.`),
        cell('arch', 'range-flag', `${CUTOUT} Range flag blank.`),
        cell('arch', 'sight-blank', `${CUTOUT} Bow sight blank.`),
        cell('arch', 'stabilizer', `${CUTOUT} Stabilizer.`),
        cell('arch', 'release-aid', `${CUTOUT} Release aid.`),
        cell('arch', 'arrow-puller', `${CUTOUT} Arrow puller.`),
      ]),
      sh('S3', 'club kit 3x3', 'black-contact-3x3', [
        cell('arch', 'bow-case', `${CUTOUT} Bow case.`),
        cell('arch', 'scorecard-blank', `${CUTOUT} Scorecard ZERO writing.`),
        cell('arch', 'binoculars-arch', `${CUTOUT} Binoculars.`),
        cell('arch', 'stool', `${CUTOUT} Range stool.`),
        cell('arch', 'water-bottle-arch', `${CUTOUT} Water bottle.`),
        cell('arch', 'hat-arch', `${CUTOUT} Cap.`),
        cell('arch', 'medal-arch', `${CUTOUT} Archery medal blank.`),
        cell('arch', 'banner-arch-blank', `${CUTOUT} Banner ZERO letters.`),
        cell('arch', 'trophy-arch', `${CUTOUT} Archery trophy blank.`),
      ]),
    ],
  }),

  'ki-sewing-fashion-deep': wave('ki-sewing-fashion-deep', {
    lane: 'crafts-maker',
    bucket: 'crafts-maker',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI sewing fashion deep NEW (3×3 ×3)',
    why: 'Discovered — sewing depth beyond fashion studio wave',
    sheets: [
      sh('S1', 'machine sew 3x3', 'black-contact-3x3', [
        cell('sew', 'sewing-machine', `${CUTOUT} Sewing machine blank (no logos).`),
        cell('sew', 'serger', `${CUTOUT} Serger blank.`),
        cell('sew', 'iron-sew', `${CUTOUT} Iron.`),
        cell('sew', 'ironing-board', `${CUTOUT} Ironing board.`),
        cell('sew', 'pressing-ham', `${CUTOUT} Pressing ham.`),
        cell('sew', 'seam-ripper', `${CUTOUT} Seam ripper.`),
        cell('sew', 'rotary-cutter', `${CUTOUT} Rotary cutter.`),
        cell('sew', 'cutting-mat-sew', `${CUTOUT} Cutting mat blank.`),
        cell('sew', 'pattern-weights', `${CUTOUT} Pattern weights.`),
      ]),
      sh('S2', 'notions 3x3', 'black-contact-3x3', [
        cell('sew', 'thread-rack', `${CUTOUT} Thread rack.`),
        cell('sew', 'pin-cushion-sew', `${CUTOUT} Pin cushion.`),
        cell('sew', 'scissors-shears', `${CUTOUT} Fabric shears.`),
        cell('sew', 'measuring-tape-sew', `${CUTOUT} Measuring tape blank.`),
        cell('sew', 'zipper-pack', `${CUTOUT} Zipper pack.`),
        cell('sew', 'button-jar-sew', `${CUTOUT} Button jar.`),
        cell('sew', 'elastic-spool', `${CUTOUT} Elastic spool.`),
        cell('sew', 'bias-tape', `${CUTOUT} Bias tape.`),
        cell('sew', 'thimble', `${CUTOUT} Thimble.`),
      ]),
      sh('S3', 'projects 3x3', 'black-contact-3x3', [
        cell('sew', 'pillow-cover', `${CUTOUT} Pillow cover.`),
        cell('sew', 'tote-bag', `${CUTOUT} Tote bag blank.`),
        cell('sew', 'scarf-sew', `${CUTOUT} Scarf.`),
        cell('sew', 'plush-blank', `${CUTOUT} Simple plush (generic, not IP).`),
        cell('sew', 'quilt-square', `${CUTOUT} Quilt square.`),
        cell('sew', 'dress-form-mini', `${CUTOUT} Mini dress form.`),
        cell('sew', 'project-bag', `${CUTOUT} Project bag.`),
        cell('sew', 'medal-sew', `${CUTOUT} Sewing medal blank.`),
        cell('sew', 'trophy-sew', `${CUTOUT} Sewing-club trophy blank.`),
      ]),
    ],
  }),

  'ki-gardening-tools': wave('ki-gardening-tools', {
    lane: 'other-discovered',
    bucket: 'other-discovered',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI gardening tools NEW (3×3 ×3)',
    why: 'Discovered — garden tools depth beyond birding/grow waves',
    sheets: [
      sh('S1', 'hand tools 3x3', 'black-contact-3x3', [
        cell('gard', 'trowel', `${CUTOUT} Garden trowel.`),
        cell('gard', 'hand-fork', `${CUTOUT} Hand fork.`),
        cell('gard', 'pruners', `${CUTOUT} Pruners.`),
        cell('gard', 'loppers', `${CUTOUT} Loppers.`),
        cell('gard', 'hoe', `${CUTOUT} Hoe.`),
        cell('gard', 'rake', `${CUTOUT} Rake.`),
        cell('gard', 'spade', `${CUTOUT} Spade.`),
        cell('gard', 'watering-can-gard', `${CUTOUT} Watering can.`),
        cell('gard', 'hose-nozzle', `${CUTOUT} Hose nozzle.`),
      ]),
      sh('S2', 'grow gear 3x3', 'black-contact-3x3', [
        cell('gard', 'seed-tray', `${CUTOUT} Seed tray.`),
        cell('gard', 'pots-stack', `${CUTOUT} Pot stack.`),
        cell('gard', 'grow-bag', `${CUTOUT} Grow bag.`),
        cell('gard', 'compost-bin-gard', `${CUTOUT} Compost bin.`),
        cell('gard', 'wheelbarrow', `${CUTOUT} Wheelbarrow.`),
        cell('gard', 'garden-gloves', `${CUTOUT} Garden gloves.`),
        cell('gard', 'knee-pad', `${CUTOUT} Kneeler pad.`),
        cell('gard', 'plant-labels-blank', `${CUTOUT} Plant labels ZERO letters.`),
        cell('gard', 'rain-gauge-blank', `${CUTOUT} Rain gauge blank.`),
      ]),
      sh('S3', 'harvest 3x3', 'black-contact-3x3', [
        cell('gard', 'tomato', `${CUTOUT} Tomato.`),
        cell('gard', 'carrot', `${CUTOUT} Carrot bunch.`),
        cell('gard', 'lettuce', `${CUTOUT} Lettuce.`),
        cell('gard', 'pumpkin-gard', `${CUTOUT} Pumpkin.`),
        cell('gard', 'sunflower-gard', `${CUTOUT} Sunflower.`),
        cell('gard', 'harvest-basket', `${CUTOUT} Harvest basket.`),
        cell('gard', 'scarecrow', `${CUTOUT} Friendly scarecrow (no face detail).`),
        cell('gard', 'bird-bath-gard', `${CUTOUT} Bird bath.`),
        cell('gard', 'trophy-gard', `${CUTOUT} Garden-club trophy blank.`),
      ]),
    ],
  }),
  ...
  'ki-surfing-beach': wave('ki-surfing-beach', {
    lane: 'sports',
    bucket: 'sports',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI surfing beach club NEW (3×3 ×3)',
    why: 'Discovered water sport — surf kit depth',
    sheets: [
      sh('S1', 'boards 3x3', 'black-contact-3x3', [
        cell('surf', 'shortboard', `${CUTOUT} Shortboard surfboard blank.`),
        cell('surf', 'longboard-surf', `${CUTOUT} Longboard surfboard.`),
        cell('surf', 'bodyboard-surf', `${CUTOUT} Bodyboard.`),
        cell('surf', 'foil-board', `${CUTOUT} Foil board generic.`),
        cell('surf', 'skimboard', `${CUTOUT} Skimboard.`),
        cell('surf', 'fin-set', `${CUTOUT} Fin set.`),
        cell('surf', 'leash', `${CUTOUT} Surf leash.`),
        cell('surf', 'wax-comb', `${CUTOUT} Wax + comb.`),
        cell('surf', 'board-bag', `${CUTOUT} Board bag.`),
      ]),
      sh('S2', 'kit 3x3', 'black-contact-3x3', [
        cell('surf', 'wetsuit-surf', `${CUTOUT} Wetsuit.`),
        cell('surf', 'rash-guard', `${CUTOUT} Rash guard.`),
        cell('surf', 'booties-surf', `${CUTOUT} Surf booties.`),
        cell('surf', 'gloves-surf', `${CUTOUT} Surf gloves.`),
        cell('surf', 'reef-booties', `${CUTOUT} Reef booties.`),
        cell('surf', 'sunscreen-bottle', `${CUTOUT} Sunscreen bottle blank.`),
        cell('surf', 'beach-towel', `${CUTOUT} Beach towel.`),
        cell('surf', 'dry-bag', `${CUTOUT} Dry bag.`),
        cell('surf', 'trophy-surf', `${CUTOUT} Surf trophy blank.`),
      ]),
      sh('S3', 'beach 3x3', 'black-contact-3x3', [
        cell('surf', 'beach-umbrella', `${CUTOUT} Beach umbrella.`),
        cell('surf', 'beach-chair', `${CUTOUT} Beach chair.`),
        cell('surf', 'cooler-surf', `${CUTOUT} Cooler.`),
        cell('surf', 'sand-bucket', `${CUTOUT} Sand bucket.`),
        cell('surf', 'frisbee-surf', `${CUTOUT} Frisbee.`),
        cell('surf', 'volleyball-beach', `${CUTOUT} Beach volleyball.`),
        cell('surf', 'lifeguard-float', `${CUTOUT} Lifeguard float.`),
        cell('surf', 'flag-surf-blank', `${CUTOUT} Beach flag blank.`),
        cell('surf', 'medal-surf', `${CUTOUT} Surf medal blank.`),
      ]),
    ],
  }),

  'ki-rock-climbing': wave('ki-rock-climbing', {
    lane: 'sports',
    bucket: 'sports',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI rock climbing gym NEW (3×3 ×3)',
    why: 'Discovered — climbing gym kit depth',
    sheets: [
      sh('S1', 'harness rope 3x3', 'black-contact-3x3', [
        cell('climb', 'harness', `${CUTOUT} Climbing harness.`),
        cell('climb', 'rope-climb', `${CUTOUT} Climbing rope coil.`),
        cell('climb', 'belay-device', `${CUTOUT} Belay device.`),
        cell('climb', 'carabiners', `${CUTOUT} Carabiner set.`),
        cell('climb', 'quickdraws', `${CUTOUT} Quickdraws.`),
        cell('climb', 'chalk-bag-climb', `${CUTOUT} Chalk bag.`),
        cell('climb', 'shoes-climb', `${CUTOUT} Climbing shoes.`),
        cell('climb', 'helmet-climb', `${CUTOUT} Climbing helmet.`),
        cell('climb', 'crash-pad', `${CUTOUT} Crash pad.`),
      ]),
      sh('S2', 'wall holds 3x3', 'black-contact-3x3', [
        cell('climb', 'holds-jugs', `${CUTOUT} Jug holds cluster.`),
        cell('climb', 'holds-crimps', `${CUTOUT} Crimp holds.`),
        cell('climb', 'holds-slopers', `${CUTOUT} Sloper holds.`),
        cell('climb', 'volume-feature', `${CUTOUT} Volume feature.`),
        cell('climb', 'auto-belay', `${CUTOUT} Auto-belay unit blank.`),
        cell('climb', 'rope-bag', `${CUTOUT} Rope bag.`),
        cell('climb', 'brush-hold', `${CUTOUT} Hold brush.`),
        cell('climb', 'tape-fingers', `${CUTOUT} Finger tape.`),
        cell('climb', 'trophy-climb', `${CUTOUT} Climbing trophy blank.`),
      ]),
      sh('S3', 'boulder 3x3', 'black-contact-3x3', [
        cell('climb', 'boulder-mat', `${CUTOUT} Boulder mat.`),
        cell('climb', 'campus-board', `${CUTOUT} Campus board.`),
        cell('climb', 'hangboard-climb', `${CUTOUT} Hangboard.`),
        cell('climb', 'training-rings', `${CUTOUT} Training rings.`),
        cell('climb', 'timer-climb', `${CUTOUT} Timer blank.`),
        cell('climb', 'water-bottle-climb', `${CUTOUT} Water bottle.`),
        cell('climb', 'chalk-bucket', `${CUTOUT} Chalk bucket.`),
        cell('climb', 'medal-climb', `${CUTOUT} Climb medal blank.`),
        cell('climb', 'banner-climb-blank', `${CUTOUT} Banner ZERO letters.`),
      ]),
    ],
  }),

  'ki-film-animation': wave('ki-film-animation', {
    lane: 'other-discovered',
    bucket: 'other-discovered',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI film animation club NEW (3×3 ×3)',
    why: 'Discovered creator hobby — stop-motion/film kit',
    sheets: [
      sh('S1', 'camera light 3x3', 'black-contact-3x3', [
        cell('film', 'camera-film', `${CUTOUT} Film camera blank.`),
        cell('film', 'tripod-film', `${CUTOUT} Tripod.`),
        cell('film', 'slider', `${CUTOUT} Camera slider.`),
        cell('film', 'softbox-film', `${CUTOUT} Softbox.`),
        cell('film', 'led-panel', `${CUTOUT} LED light panel blank.`),
        cell('film', 'clapper-blank', `${CUTOUT} Clapperboard ZERO letters.`),
        cell('film', 'mic-boom-film', `${CUTOUT} Boom mic.`),
        cell('film', 'headphones-film', `${CUTOUT} Headphones.`),
        cell('film', 'monitor-blank', `${CUTOUT} Field monitor blank screen.`),
      ]),
      sh('S2', 'stop motion 3x3', 'black-contact-3x3', [
        cell('film', 'armature', `${CUTOUT} Stop-motion armature.`),
        cell('film', 'clay-figure', `${CUTOUT} Clay figure generic.`),
        cell('film', 'set-box', `${CUTOUT} Mini set box.`),
        cell('film', 'backdrop-paper', `${CUTOUT} Backdrop paper roll.`),
        cell('film', 'turntable-anim', `${CUTOUT} Animation turntable.`),
        cell('film', 'onion-skin-tablet', `${CUTOUT} Tablet blank screen.`),
        cell('film', 'stylus-anim', `${CUTOUT} Stylus.`),
        cell('film', 'light-box-anim', `${CUTOUT} Light box.`),
        cell('film', 'trophy-film', `${CUTOUT} Film-club trophy blank.`),
      ]),
      sh('S3', 'edit kit 3x3', 'black-contact-3x3', [
        cell('film', 'laptop-edit-blank', `${CUTOUT} Laptop blank screen.`),
        cell('film', 'external-drive', `${CUTOUT} External drive blank.`),
        cell('film', 'sd-case', `${CUTOUT} SD card case.`),
        cell('film', 'storyboard-blank', `${CUTOUT} Storyboard sheet ZERO drawings/text.`),
        cell('film', 'color-chart', `${CUTOUT} Color chart.`),
        cell('film', 'lens-set', `${CUTOUT} Lens set.`),
        cell('film', 'gimbal-film', `${CUTOUT} Gimbal.`),
        cell('film', 'medal-film', `${CUTOUT} Film medal blank.`),
        cell('film', 'popcorn-bowl-film', `${CUTOUT} Popcorn bowl.`),
      ]),
    ],
  }),

  /** ——— AGGRO WAVE14 (majority discovered-new) ——— */

  'ki-fencing-club': wave('ki-fencing-club', {
    lane: 'sports',
    bucket: 'sports',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI fencing club NEW (3×3 ×3)',
    why: 'Discovered Olympic-adjacent sport kit not in original prompt',
    sheets: [
      sh('S1', 'weapons 3x3', 'black-contact-3x3', [
        cell('fence', 'foil-blade', `${CUTOUT} Fencing foil (generic).`),
        cell('fence', 'epee-blade', `${CUTOUT} Fencing epee.`),
        cell('fence', 'sabre-blade', `${CUTOUT} Fencing sabre.`),
        cell('fence', 'mask-fence', `${CUTOUT} Fencing mask.`),
        cell('fence', 'glove-fence', `${CUTOUT} Fencing glove.`),
        cell('fence', 'lamé-jacket', `${CUTOUT} Lamé jacket blank.`),
        cell('fence', 'plastron', `${CUTOUT} Plastron underarm protector.`),
        cell('fence', 'breeches-fence', `${CUTOUT} Fencing breeches.`),
        cell('fence', 'socks-fence', `${CUTOUT} Knee socks rolled.`),
      ]),
      sh('S2', 'piste kit 3x3', 'black-contact-3x3', [
        cell('fence', 'piste-strip', `${CUTOUT} Piste strip roll.`),
        cell('fence', 'scoring-box-blank', `${CUTOUT} Scoring box blank screen (no digits).`),
        cell('fence', 'body-cord', `${CUTOUT} Body cord reel.`),
        cell('fence', 'reel-wire', `${CUTOUT} Floor reel.`),
        cell('fence', 'chest-protector', `${CUTOUT} Chest protector.`),
        cell('fence', 'bag-fence', `${CUTOUT} Fencing bag.`),
        cell('fence', 'shoe-fence', `${CUTOUT} Fencing shoe.`),
        cell('fence', 'water-bottle-fence', `${CUTOUT} Water bottle.`),
        cell('fence', 'trophy-fence', `${CUTOUT} Fencing trophy blank.`),
      ]),
      sh('S3', 'club extras 3x3', 'black-contact-3x3', [
        cell('fence', 'target-pad', `${CUTOUT} Wall target pad.`),
        cell('fence', 'dummy-arm', `${CUTOUT} Practice dummy arm.`),
        cell('fence', 'timer-fence', `${CUTOUT} Timer blank.`),
        cell('fence', 'medal-fence', `${CUTOUT} Fencing medal blank.`),
        cell('fence', 'banner-fence-blank', `${CUTOUT} Club banner ZERO letters.`),
        cell('fence', 'bench-fence', `${CUTOUT} Bench.`),
        cell('fence', 'tool-kit-fence', `${CUTOUT} Blade tool kit.`),
        cell('fence', 'grip-tape', `${CUTOUT} Grip tape roll.`),
        cell('fence', 'score-cards-fence', `${CUTOUT} Score cards ZERO writing.`),
      ]),
    ],
  }),

  'ki-kayak-paddle': wave('ki-kayak-paddle', {
    lane: 'sports',
    bucket: 'sports',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI kayak paddle club NEW (3×3 ×3)',
    why: 'Discovered paddle-sport depth beyond surf/scuba',
    sheets: [
      sh('S1', 'hulls 3x3', 'black-contact-3x3', [
        cell('kayak', 'kayak-sit-in', `${CUTOUT} Sit-in kayak.`),
        cell('kayak', 'kayak-sit-on', `${CUTOUT} Sit-on-top kayak.`),
        cell('kayak', 'canoe-open', `${CUTOUT} Open canoe.`),
        cell('kayak', 'paddleboard', `${CUTOUT} Stand-up paddleboard.`),
        cell('kayak', 'inflatable-kayak', `${CUTOUT} Inflatable kayak rolled.`),
        cell('kayak', 'outrigger', `${CUTOUT} Small outrigger canoe.`),
        cell('kayak', 'raft-pack', `${CUTOUT} Pack raft rolled.`),
        cell('kayak', 'spray-skirt', `${CUTOUT} Spray skirt.`),
        cell('kayak', 'hatch-cover', `${CUTOUT} Hatch cover.`),
      ]),
      sh('S2', 'paddles gear 3x3', 'black-contact-3x3', [
        cell('kayak', 'paddle-double', `${CUTOUT} Double-blade paddle.`),
        cell('kayak', 'paddle-single', `${CUTOUT} Single-blade canoe paddle.`),
        cell('kayak', 'pfd-vest', `${CUTOUT} PFD life vest.`),
        cell('kayak', 'helmet-kayak', `${CUTOUT} Whitewater helmet.`),
        cell('kayak', 'dry-bag-kayak', `${CUTOUT} Dry bag.`),
        cell('kayak', 'bilge-pump', `${CUTOUT} Bilge pump.`),
        cell('kayak', 'throw-rope', `${CUTOUT} Throw rope bag.`),
        cell('kayak', 'whistle-kayak', `${CUTOUT} Safety whistle.`),
        cell('kayak', 'trophy-kayak', `${CUTOUT} Paddle-club trophy blank.`),
      ]),
      sh('S3', 'shore kit 3x3', 'black-contact-3x3', [
        cell('kayak', 'roof-rack', `${CUTOUT} Roof rack bars.`),
        cell('kayak', 'cart-dolly', `${CUTOUT} Kayak cart dolly.`),
        cell('kayak', 'paddle-leash', `${CUTOUT} Paddle leash.`),
        cell('kayak', 'water-shoes', `${CUTOUT} Water shoes.`),
        cell('kayak', 'cooler-kayak', `${CUTOUT} Soft cooler.`),
        cell('kayak', 'map-case-blank', `${CUTOUT} Map case blank (no map marks).`),
        cell('kayak', 'compass-kayak', `${CUTOUT} Compass.`),
        cell('kayak', 'medal-kayak', `${CUTOUT} Paddle medal blank.`),
        cell('kayak', 'banner-kayak-blank', `${CUTOUT} Banner ZERO letters.`),
      ]),
    ],
  }),

  'ki-woodworking-shop': wave('ki-woodworking-shop', {
    lane: 'crafts-maker',
    bucket: 'crafts-maker',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI woodworking shop NEW (3×3 ×3)',
    why: 'Discovered maker craft — hand-tool woodshop depth',
    sheets: [
      sh('S1', 'hand tools 3x3', 'black-contact-3x3', [
        cell('wood', 'hand-saw', `${CUTOUT} Hand saw.`),
        cell('wood', 'hammer-claw', `${CUTOUT} Claw hammer.`),
        cell('wood', 'chisel-set', `${CUTOUT} Chisel set.`),
        cell('wood', 'hand-plane', `${CUTOUT} Hand plane.`),
        cell('wood', 'mallet-wood', `${CUTOUT} Wooden mallet.`),
        cell('wood', 'try-square', `${CUTOUT} Try square.`),
        cell('wood', 'clamp-bar', `${CUTOUT} Bar clamp.`),
        cell('wood', 'rasp-file', `${CUTOUT} Wood rasp.`),
        cell('wood', 'awl', `${CUTOUT} Awl.`),
      ]),
      sh('S2', 'bench stock 3x3', 'black-contact-3x3', [
        cell('wood', 'workbench', `${CUTOUT} Small workbench.`),
        cell('wood', 'vise-bench', `${CUTOUT} Bench vise.`),
        cell('wood', 'sawhorse', `${CUTOUT} Sawhorse.`),
        cell('wood', 'lumber-stack', `${CUTOUT} Lumber stack.`),
        cell('wood', 'plywood-sheet', `${CUTOUT} Plywood sheet.`),
        cell('wood', 'dowel-bundle', `${CUTOUT} Dowel bundle.`),
        cell('wood', 'sandpaper-pack', `${CUTOUT} Sandpaper pack blank.`),
        cell('wood', 'glue-bottle-wood', `${CUTOUT} Wood glue bottle blank.`),
        cell('wood', 'trophy-wood', `${CUTOUT} Woodshop trophy blank.`),
      ]),
      sh('S3', 'projects safety 3x3', 'black-contact-3x3', [
        cell('wood', 'birdhouse-blank', `${CUTOUT} Birdhouse (blank, no text).`),
        cell('wood', 'toolbox-wood', `${CUTOUT} Wooden toolbox.`),
        cell('wood', 'cutting-board', `${CUTOUT} Cutting board blank.`),
        cell('wood', 'goggles-wood', `${CUTOUT} Safety goggles.`),
        cell('wood', 'earmuffs-wood', `${CUTOUT} Earmuffs.`),
        cell('wood', 'apron-wood', `${CUTOUT} Shop apron.`),
        cell('wood', 'pencil-mark', `${CUTOUT} Carpenter pencil.`),
        cell('wood', 'tape-measure', `${CUTOUT} Tape measure blank face.`),
        cell('wood', 'medal-wood', `${CUTOUT} Woodshop medal blank.`),
      ]),
    ],
  }),

  'ki-weather-science': wave('ki-weather-science', {
    lane: 'other-discovered',
    bucket: 'other-discovered',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI weather science club NEW (3×3 ×3)',
    why: 'Discovered STEM club — meteorology kit depth',
    sheets: [
      sh('S1', 'instruments 3x3', 'black-contact-3x3', [
        cell('wx', 'thermometer-wx', `${CUTOUT} Outdoor thermometer blank scale.`),
        cell('wx', 'barometer', `${CUTOUT} Barometer blank face.`),
        cell('wx', 'anemometer', `${CUTOUT} Anemometer cups.`),
        cell('wx', 'rain-gauge', `${CUTOUT} Rain gauge.`),
        cell('wx', 'hygrometer', `${CUTOUT} Hygrometer blank.`),
        cell('wx', 'weather-vane', `${CUTOUT} Weather vane.`),
        cell('wx', 'psychrometer', `${CUTOUT} Sling psychrometer.`),
        cell('wx', 'uv-meter-blank', `${CUTOUT} UV meter blank screen.`),
        cell('wx', 'compass-wx', `${CUTOUT} Compass.`),
      ]),
      sh('S2', 'station kit 3x3', 'black-contact-3x3', [
        cell('wx', 'weather-station', `${CUTOUT} Home weather station blank screen.`),
        cell('wx', 'rain-boot', `${CUTOUT} Rain boot.`),
        cell('wx', 'umbrella-wx', `${CUTOUT} Umbrella.`),
        cell('wx', 'cloud-chart-blank', `${CUTOUT} Cloud chart card ZERO labels.`),
        cell('wx', 'notebook-wx-blank', `${CUTOUT} Field notebook ZERO writing.`),
        cell('wx', 'binoculars-wx', `${CUTOUT} Binoculars.`),
        cell('wx', 'radio-wx-blank', `${CUTOUT} Weather radio blank (no brands/text).`),
        cell('wx', 'balloon-wx', `${CUTOUT} Weather balloon.`),
        cell('wx', 'trophy-wx', `${CUTOUT} Weather-club trophy blank.`),
      ]),
      sh('S3', 'sky props 3x3', 'black-contact-3x3', [
        cell('wx', 'lightning-bolt-prop', `${CUTOUT} Stylized lightning bolt prop (toy).`),
        cell('wx', 'snowflake-prop', `${CUTOUT} Oversized snowflake prop.`),
        cell('wx', 'sun-prop', `${CUTOUT} Sun prop disk.`),
        cell('wx', 'cloud-prop', `${CUTOUT} Soft cloud prop.`),
        cell('wx', 'rainbow-arc-prop', `${CUTOUT} Rainbow arc prop.`),
        cell('wx', 'tornado-tube', `${CUTOUT} Tornado tube bottle.`),
        cell('wx', 'medal-wx', `${CUTOUT} Weather medal blank.`),
        cell('wx', 'banner-wx-blank', `${CUTOUT} Banner ZERO letters.`),
        cell('wx', 'map-case-wx', `${CUTOUT} Map case blank.`),
      ]),
    ],
  }),

  /** ——— AGGRO WAVE15 (majority discovered-new) ——— */

  'ki-gymnastics-floor': wave('ki-gymnastics-floor', {
    lane: 'sports',
    bucket: 'sports',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI gymnastics floor NEW (3×3 ×3)',
    why: 'Discovered gym sport kit not covered by martial/parkour',
    sheets: [
      sh('S1', 'apparatus 3x3', 'black-contact-3x3', [
        cell('gymn', 'balance-beam', `${CUTOUT} Balance beam.`),
        cell('gymn', 'vault-table', `${CUTOUT} Vault table.`),
        cell('gymn', 'uneven-bars', `${CUTOUT} Uneven bars.`),
        cell('gymn', 'parallel-bars', `${CUTOUT} Parallel bars.`),
        cell('gymn', 'rings-still', `${CUTOUT} Still rings.`),
        cell('gymn', 'pommel-horse', `${CUTOUT} Pommel horse.`),
        cell('gymn', 'floor-mat', `${CUTOUT} Floor exercise mat.`),
        cell('gymn', 'springboard', `${CUTOUT} Springboard.`),
        cell('gymn', 'crash-mat-gymn', `${CUTOUT} Thick crash mat.`),
      ]),
      sh('S2', 'training kit 3x3', 'black-contact-3x3', [
        cell('gymn', 'chalk-bowl', `${CUTOUT} Chalk bowl.`),
        cell('gymn', 'grips-gymn', `${CUTOUT} Hand grips.`),
        cell('gymn', 'leotard-blank', `${CUTOUT} Leotard blank (no logos).`),
        cell('gymn', 'shorts-gymn', `${CUTOUT} Gym shorts.`),
        cell('gymn', 'slippers-gymn', `${CUTOUT} Gym slippers.`),
        cell('gymn', 'foam-roller-gymn', `${CUTOUT} Foam roller.`),
        cell('gymn', 'resistance-band', `${CUTOUT} Resistance band.`),
        cell('gymn', 'water-bottle-gymn', `${CUTOUT} Water bottle.`),
        cell('gymn', 'trophy-gymn', `${CUTOUT} Gymnastics trophy blank.`),
      ]),
      sh('S3', 'club extras 3x3', 'black-contact-3x3', [
        cell('gymn', 'ribbon-rhythmic', `${CUTOUT} Rhythmic ribbon stick.`),
        cell('gymn', 'hoop-rhythmic', `${CUTOUT} Rhythmic hoop.`),
        cell('gymn', 'ball-rhythmic', `${CUTOUT} Rhythmic ball.`),
        cell('gymn', 'clubs-rhythmic', `${CUTOUT} Rhythmic clubs pair.`),
        cell('gymn', 'timer-gymn', `${CUTOUT} Timer blank.`),
        cell('gymn', 'medal-gymn', `${CUTOUT} Gymnastics medal blank.`),
        cell('gymn', 'banner-gymn-blank', `${CUTOUT} Banner ZERO letters.`),
        cell('gymn', 'bench-gymn', `${CUTOUT} Bench.`),
        cell('gymn', 'score-cards-gymn', `${CUTOUT} Score cards ZERO writing.`),
      ]),
    ],
  }),

  'ki-calligraphy-ink': wave('ki-calligraphy-ink', {
    lane: 'crafts-maker',
    bucket: 'crafts-maker',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI calligraphy ink club NEW (3×3 ×3)',
    why: 'Discovered fine-craft hobby — ink/brush lettering kit',
    sheets: [
      sh('S1', 'pens brushes 3x3', 'black-contact-3x3', [
        cell('callig', 'dip-pen', `${CUTOUT} Dip pen nib holder.`),
        cell('callig', 'nib-set', `${CUTOUT} Nib set.`),
        cell('callig', 'brush-pen', `${CUTOUT} Brush pen blank.`),
        cell('callig', 'fountain-pen-blank', `${CUTOUT} Fountain pen blank (no logos).`),
        cell('callig', 'ink-brush', `${CUTOUT} Ink brush.`),
        cell('callig', 'ruler-t-square', `${CUTOUT} T-square ruler.`),
        cell('callig', 'pencil-callig', `${CUTOUT} Soft pencil.`),
        cell('callig', 'eraser-kneaded', `${CUTOUT} Kneaded eraser.`),
        cell('callig', 'blade-scraper', `${CUTOUT} Scraping blade.`),
      ]),
      sh('S2', 'ink paper 3x3', 'black-contact-3x3', [
        cell('callig', 'ink-bottle', `${CUTOUT} Ink bottle blank label.`),
        cell('callig', 'ink-well', `${CUTOUT} Inkwell.`),
        cell('callig', 'ink-stone', `${CUTOUT} Ink stone.`),
        cell('callig', 'watercolor-pan', `${CUTOUT} Watercolor pan set.`),
        cell('callig', 'paper-pad-blank', `${CUTOUT} Paper pad ZERO writing.`),
        cell('callig', 'rice-paper-roll', `${CUTOUT} Rice paper roll.`),
        cell('callig', 'blotting-paper', `${CUTOUT} Blotting paper.`),
        cell('callig', 'wax-seal-blank', `${CUTOUT} Wax seal stamp blank.`),
        cell('callig', 'trophy-callig', `${CUTOUT} Calligraphy trophy blank.`),
      ]),
      sh('S3', 'desk kit 3x3', 'black-contact-3x3', [
        cell('callig', 'desk-slant', `${CUTOUT} Slant desk board.`),
        cell('callig', 'paperweight', `${CUTOUT} Paperweight.`),
        cell('callig', 'brush-rest', `${CUTOUT} Brush rest.`),
        cell('callig', 'water-dish', `${CUTOUT} Water dish.`),
        cell('callig', 'cloth-wipe', `${CUTOUT} Wipe cloth.`),
        cell('callig', 'portfolio-case', `${CUTOUT} Portfolio case.`),
        cell('callig', 'medal-callig', `${CUTOUT} Calligraphy medal blank.`),
        cell('callig', 'banner-callig-blank', `${CUTOUT} Banner ZERO letters.`),
        cell('callig', 'scroll-blank', `${CUTOUT} Blank scroll ZERO characters.`),
      ]),
    ],
  }),

  'ki-bowling-alley': wave('ki-bowling-alley', {
    lane: 'sports',
    bucket: 'sports',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI bowling alley club NEW (3×3 ×3)',
    why: 'Discovered recreational sport kit depth',
    sheets: [
      sh('S1', 'balls pins 3x3', 'black-contact-3x3', [
        cell('bowl', 'ball-bowling', `${CUTOUT} Bowling ball.`),
        cell('bowl', 'ball-lightweight', `${CUTOUT} Lightweight bowling ball.`),
        cell('bowl', 'pin-set', `${CUTOUT} Bowling pin.`),
        cell('bowl', 'pin-rack', `${CUTOUT} Pin rack.`),
        cell('bowl', 'bumper-guard', `${CUTOUT} Lane bumper.`),
        cell('bowl', 'shoe-bowl', `${CUTOUT} Bowling shoe.`),
        cell('bowl', 'wrist-support', `${CUTOUT} Wrist support.`),
        cell('bowl', 'towel-bowl', `${CUTOUT} Ball towel.`),
        cell('bowl', 'bag-bowl', `${CUTOUT} Bowling bag.`),
      ]),
      sh('S2', 'lane gear 3x3', 'black-contact-3x3', [
        cell('bowl', 'ball-return', `${CUTOUT} Ball return unit blank.`),
        cell('bowl', 'score-monitor-blank', `${CUTOUT} Score monitor blank screen.`),
        cell('bowl', 'lane-oil-bottle', `${CUTOUT} Lane oil bottle blank.`),
        cell('bowl', 'chalk-bag-bowl', `${CUTOUT} Chalk bag.`),
        cell('bowl', 'finger-tape-bowl', `${CUTOUT} Finger tape.`),
        cell('bowl', 'shoe-spray', `${CUTOUT} Shoe spray blank.`),
        cell('bowl', 'locker-key-blank', `${CUTOUT} Locker key blank tag.`),
        cell('bowl', 'water-bottle-bowl', `${CUTOUT} Water bottle.`),
        cell('bowl', 'trophy-bowl', `${CUTOUT} Bowling trophy blank.`),
      ]),
      sh('S3', 'party extras 3x3', 'black-contact-3x3', [
        cell('bowl', 'glow-pin', `${CUTOUT} Glow pin prop.`),
        cell('bowl', 'glow-ball', `${CUTOUT} Glow bowling ball.`),
        cell('bowl', 'party-hat-bowl', `${CUTOUT} Party hat.`),
        cell('bowl', 'pizza-slice-prop', `${CUTOUT} Pizza slice prop.`),
        cell('bowl', 'soda-cup-blank', `${CUTOUT} Soda cup blank.`),
        cell('bowl', 'medal-bowl', `${CUTOUT} Bowling medal blank.`),
        cell('bowl', 'banner-bowl-blank', `${CUTOUT} Banner ZERO letters.`),
        cell('bowl', 'score-sheet-blank', `${CUTOUT} Score sheet ZERO writing.`),
        cell('bowl', 'bench-bowl', `${CUTOUT} Bench.`),
      ]),
    ],
  }),

  'ki-metal-detecting': wave('ki-metal-detecting', {
    lane: 'other-discovered',
    bucket: 'other-discovered',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI metal detecting club NEW (3×3 ×3)',
    why: 'Discovered outdoor hobby — detector + finds kit',
    sheets: [
      sh('S1', 'detector kit 3x3', 'black-contact-3x3', [
        cell('detect', 'metal-detector', `${CUTOUT} Metal detector.`),
        cell('detect', 'pinpointer', `${CUTOUT} Pinpointer wand.`),
        cell('detect', 'headphones-detect', `${CUTOUT} Headphones.`),
        cell('detect', 'coil-cover', `${CUTOUT} Coil cover.`),
        cell('detect', 'digging-trowel', `${CUTOUT} Digging trowel.`),
        cell('detect', 'sand-scoop', `${CUTOUT} Sand scoop.`),
        cell('detect', 'finds-pouch', `${CUTOUT} Finds pouch.`),
        cell('detect', 'gloves-detect', `${CUTOUT} Work gloves.`),
        cell('detect', 'boots-detect', `${CUTOUT} Mud boots.`),
      ]),
      sh('S2', 'finds display 3x3', 'black-contact-3x3', [
        cell('detect', 'coin-generic', `${CUTOUT} Generic coin blank face.`),
        cell('detect', 'ring-plain', `${CUTOUT} Plain ring.`),
        cell('detect', 'button-old', `${CUTOUT} Old button.`),
        cell('detect', 'key-rusty', `${CUTOUT} Rusty key.`),
        cell('detect', 'nail-square', `${CUTOUT} Square nail.`),
        cell('detect', 'bottle-cap', `${CUTOUT} Bottle cap.`),
        cell('detect', 'display-case', `${CUTOUT} Finds display case.`),
        cell('detect', 'magnifier-detect', `${CUTOUT} Magnifier.`),
        cell('detect', 'trophy-detect', `${CUTOUT} Detecting-club trophy blank.`),
      ]),
      sh('S3', 'field extras 3x3', 'black-contact-3x3', [
        cell('detect', 'map-case-detect', `${CUTOUT} Map case blank.`),
        cell('detect', 'compass-detect', `${CUTOUT} Compass.`),
        cell('detect', 'water-bottle-detect', `${CUTOUT} Water bottle.`),
        cell('detect', 'hat-detect', `${CUTOUT} Sun hat.`),
        cell('detect', 'sunscreen-detect', `${CUTOUT} Sunscreen bottle blank.`),
        cell('detect', 'notebook-detect-blank', `${CUTOUT} Finds notebook ZERO writing.`),
        cell('detect', 'medal-detect', `${CUTOUT} Detecting medal blank.`),
        cell('detect', 'banner-detect-blank', `${CUTOUT} Banner ZERO letters.`),
        cell('detect', 'backpack-detect', `${CUTOUT} Daypack.`),
      ]),
    ],
  }),

  /** ——— AGGRO WAVE16 (majority discovered-new) ——— */

  'ki-lacrosse-field': wave('ki-lacrosse-field', {
    lane: 'sports',
    bucket: 'sports',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI lacrosse field NEW (3×3 ×3)',
    why: 'Discovered team sport kit not in original prompt',
    sheets: [
      sh('S1', 'sticks balls 3x3', 'black-contact-3x3', [
        cell('lax', 'stick-attack', `${CUTOUT} Lacrosse attack stick.`),
        cell('lax', 'stick-defense', `${CUTOUT} Lacrosse defense stick.`),
        cell('lax', 'stick-goalie', `${CUTOUT} Lacrosse goalie stick.`),
        cell('lax', 'ball-lax', `${CUTOUT} Lacrosse ball.`),
        cell('lax', 'helmet-lax', `${CUTOUT} Lacrosse helmet blank.`),
        cell('lax', 'gloves-lax', `${CUTOUT} Lacrosse gloves.`),
        cell('lax', 'pads-shoulder', `${CUTOUT} Shoulder pads.`),
        cell('lax', 'pads-arm', `${CUTOUT} Arm pads.`),
        cell('lax', 'cleats-lax', `${CUTOUT} Cleats.`),
      ]),
      sh('S2', 'goal field 3x3', 'black-contact-3x3', [
        cell('lax', 'goal-lax', `${CUTOUT} Lacrosse goal net.`),
        cell('lax', 'net-mesh', `${CUTOUT} Goal mesh panel.`),
        cell('lax', 'cone-drill', `${CUTOUT} Drill cone.`),
        cell('lax', 'rebounder', `${CUTOUT} Rebounder wall.`),
        cell('lax', 'ball-bucket', `${CUTOUT} Ball bucket.`),
        cell('lax', 'mouthguard', `${CUTOUT} Mouthguard case.`),
        cell('lax', 'water-bottle-lax', `${CUTOUT} Water bottle.`),
        cell('lax', 'bag-lax', `${CUTOUT} Equipment bag.`),
        cell('lax', 'trophy-lax', `${CUTOUT} Lacrosse trophy blank.`),
      ]),
      sh('S3', 'club extras 3x3', 'black-contact-3x3', [
        cell('lax', 'jersey-blank', `${CUTOUT} Jersey blank (no numbers/text).`),
        cell('lax', 'pinnie-blank', `${CUTOUT} Practice pinnie blank.`),
        cell('lax', 'timer-lax', `${CUTOUT} Timer blank.`),
        cell('lax', 'whistle-lax', `${CUTOUT} Whistle.`),
        cell('lax', 'medal-lax', `${CUTOUT} Lacrosse medal blank.`),
        cell('lax', 'banner-lax-blank', `${CUTOUT} Banner ZERO letters.`),
        cell('lax', 'bench-lax', `${CUTOUT} Bench.`),
        cell('lax', 'scoreboard-blank', `${CUTOUT} Scoreboard blank (no digits).`),
        cell('lax', 'first-aid-lax', `${CUTOUT} First-aid kit blank.`),
      ]),
    ],
  }),

  'ki-knitting-yarn': wave('ki-knitting-yarn', {
    lane: 'crafts-maker',
    bucket: 'crafts-maker',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI knitting yarn club NEW (3×3 ×3)',
    why: 'Discovered fiber craft beyond sewing fashion',
    sheets: [
      sh('S1', 'needles yarn 3x3', 'black-contact-3x3', [
        cell('knit', 'needles-straight', `${CUTOUT} Straight knitting needles.`),
        cell('knit', 'needles-circular', `${CUTOUT} Circular needles.`),
        cell('knit', 'hooks-crochet', `${CUTOUT} Crochet hooks set.`),
        cell('knit', 'yarn-ball', `${CUTOUT} Yarn ball.`),
        cell('knit', 'yarn-skein', `${CUTOUT} Yarn skein.`),
        cell('knit', 'yarn-bowl', `${CUTOUT} Yarn bowl.`),
        cell('knit', 'stitch-markers', `${CUTOUT} Stitch markers.`),
        cell('knit', 'row-counter-blank', `${CUTOUT} Row counter blank (no digits).`),
        cell('knit', 'tapestry-needle', `${CUTOUT} Tapestry needle.`),
      ]),
      sh('S2', 'projects 3x3', 'black-contact-3x3', [
        cell('knit', 'scarf-knit', `${CUTOUT} Knitted scarf.`),
        cell('knit', 'hat-knit', `${CUTOUT} Knitted hat.`),
        cell('knit', 'mittens-knit', `${CUTOUT} Mittens.`),
        cell('knit', 'sock-knit', `${CUTOUT} Knitted sock.`),
        cell('knit', 'blanket-square', `${CUTOUT} Blanket square.`),
        cell('knit', 'pom-pom', `${CUTOUT} Pom-pom.`),
        cell('knit', 'loom-knit', `${CUTOUT} Knitting loom.`),
        cell('knit', 'blocking-mats', `${CUTOUT} Blocking mats.`),
        cell('knit', 'trophy-knit', `${CUTOUT} Yarn-club trophy blank.`),
      ]),
      sh('S3', 'kit extras 3x3', 'black-contact-3x3', [
        cell('knit', 'project-bag', `${CUTOUT} Project bag.`),
        cell('knit', 'scissors-knit', `${CUTOUT} Small scissors.`),
        cell('knit', 'measuring-tape-knit', `${CUTOUT} Soft measuring tape blank.`),
        cell('knit', 'pattern-card-blank', `${CUTOUT} Pattern card ZERO writing.`),
        cell('knit', 'swift-yarn', `${CUTOUT} Yarn swift.`),
        cell('knit', 'ball-winder', `${CUTOUT} Ball winder.`),
        cell('knit', 'medal-knit', `${CUTOUT} Yarn medal blank.`),
        cell('knit', 'banner-knit-blank', `${CUTOUT} Banner ZERO letters.`),
        cell('knit', 'tea-mug-knit', `${CUTOUT} Tea mug blank.`),
      ]),
    ],
  }),

  'ki-sailing-dinghy': wave('ki-sailing-dinghy', {
    lane: 'sports',
    bucket: 'sports',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI sailing dinghy club NEW (3×3 ×3)',
    why: 'Discovered wind sport — dinghy sailing kit depth',
    sheets: [
      sh('S1', 'boats sails 3x3', 'black-contact-3x3', [
        cell('sail', 'dinghy-hull', `${CUTOUT} Small dinghy hull.`),
        cell('sail', 'sail-main', `${CUTOUT} Mainsail blank (no logos).`),
        cell('sail', 'sail-jib', `${CUTOUT} Jib sail blank.`),
        cell('sail', 'mast-sail', `${CUTOUT} Mast section.`),
        cell('sail', 'boom-sail', `${CUTOUT} Boom.`),
        cell('sail', 'rudder-tiller', `${CUTOUT} Rudder + tiller.`),
        cell('sail', 'centerboard', `${CUTOUT} Centerboard.`),
        cell('sail', 'spinnaker-blank', `${CUTOUT} Spinnaker blank.`),
        cell('sail', 'hull-trailer', `${CUTOUT} Boat trailer.`),
      ]),
      sh('S2', 'safety gear 3x3', 'black-contact-3x3', [
        cell('sail', 'pfd-sail', `${CUTOUT} PFD life vest.`),
        cell('sail', 'helmet-sail', `${CUTOUT} Sailing helmet.`),
        cell('sail', 'gloves-sail', `${CUTOUT} Sailing gloves.`),
        cell('sail', 'boots-sail', `${CUTOUT} Deck boots.`),
        cell('sail', 'whistle-sail', `${CUTOUT} Safety whistle.`),
        cell('sail', 'throw-float', `${CUTOUT} Throw float.`),
        cell('sail', 'dry-bag-sail', `${CUTOUT} Dry bag.`),
        cell('sail', 'compass-sail', `${CUTOUT} Compass.`),
        cell('sail', 'trophy-sail', `${CUTOUT} Sailing trophy blank.`),
      ]),
      sh('S3', 'dock kit 3x3', 'black-contact-3x3', [
        cell('sail', 'cleat-dock', `${CUTOUT} Dock cleat.`),
        cell('sail', 'rope-coil-sail', `${CUTOUT} Rope coil.`),
        cell('sail', 'winch-handle', `${CUTOUT} Winch handle.`),
        cell('sail', 'buoy-marker', `${CUTOUT} Marker buoy.`),
        cell('sail', 'paddle-backup', `${CUTOUT} Backup paddle.`),
        cell('sail', 'water-bottle-sail', `${CUTOUT} Water bottle.`),
        cell('sail', 'medal-sail', `${CUTOUT} Sailing medal blank.`),
        cell('sail', 'banner-sail-blank', `${CUTOUT} Banner ZERO letters.`),
        cell('sail', 'tool-kit-sail', `${CUTOUT} Rigging tool kit.`),
      ]),
    ],
  }),

  'ki-paleontology-dig': wave('ki-paleontology-dig', {
    lane: 'other-discovered',
    bucket: 'other-discovered',
    in_prompt_named: false,
    novelty: 'discovered-new',
    title: 'KI paleontology dig NEW (3×3 ×3)',
    why: 'Discovered STEM hobby — fossil dig beyond geology rocks',
    sheets: [
      sh('S1', 'dig tools 3x3', 'black-contact-3x3', [
        cell('paleo', 'rock-hammer', `${CUTOUT} Rock hammer.`),
        cell('paleo', 'chisel-paleo', `${CUTOUT} Paleo chisel.`),
        cell('paleo', 'brush-paleo', `${CUTOUT} Soft dig brush.`),
        cell('paleo', 'pick-small', `${CUTOUT} Small pick.`),
        cell('paleo', 'screen-sieve', `${CUTOUT} Sieve screen.`),
        cell('paleo', 'magnifier-paleo', `${CUTOUT} Magnifier.`),
        cell('paleo', 'goggles-paleo', `${CUTOUT} Safety goggles.`),
        cell('paleo', 'gloves-paleo', `${CUTOUT} Work gloves.`),
        cell('paleo', 'knee-pads-paleo', `${CUTOUT} Knee pads.`),
      ]),
      sh('S2', 'fossils finds 3x3', 'black-contact-3x3', [
        cell('paleo', 'fossil-ammonite', `${CUTOUT} Ammonite fossil.`),
        cell('paleo', 'fossil-trilobite', `${CUTOUT} Trilobite fossil.`),
        cell('paleo', 'fossil-tooth', `${CUTOUT} Fossil tooth.`),
        cell('paleo', 'fossil-leaf', `${CUTOUT} Leaf fossil.`),
        cell('paleo', 'bone-cast', `${CUTOUT} Bone cast generic.`),
        cell('paleo', 'matrix-block', `${CUTOUT} Rock matrix block.`),
        cell('paleo', 'display-stand', `${CUTOUT} Fossil display stand.`),
        cell('paleo', 'specimen-tray', `${CUTOUT} Specimen tray.`),
        cell('paleo', 'trophy-paleo', `${CUTOUT} Dig-club trophy blank.`),
      ]),
      sh('S3', 'camp lab 3x3', 'black-contact-3x3', [
        cell('paleo', 'field-notebook-blank', `${CUTOUT} Field notebook ZERO writing.`),
        cell('paleo', 'map-case-paleo', `${CUTOUT} Map case blank.`),
        cell('paleo', 'sample-bags', `${CUTOUT} Sample bags.`),
        cell('paleo', 'plaster-jacket', `${CUTOUT} Plaster jacket wrap.`),
        cell('paleo', 'water-bottle-paleo', `${CUTOUT} Water bottle.`),
        cell('paleo', 'hat-paleo', `${CUTOUT} Sun hat.`),
        cell('paleo', 'medal-paleo', `${CUTOUT} Dig medal blank.`),
        cell('paleo', 'banner-paleo-blank', `${CUTOUT} Banner ZERO letters.`),
        cell('paleo', 'backpack-paleo', `${CUTOUT} Field backpack.`),
      ]),
    ],
  }),
};
...
export const WAVE_ORDER = [
  ...
  // aggro wave6 — contact sheets; majority discovered-new
  'ki-geology-dig',
  'ki-birding-nature',
  'ki-parkour-ninja',
  'ki-pets-habitats-deep',
  'ki-music-percussion',
  // aggro wave7
  'ki-comics-maker',
  'ki-kite-festival',
  ...
  'ki-baking-cafe',
  'ki-radio-ham',
  'ki-skate-street-deep',
  // aggro wave10
  'ki-ice-hockey-club',
  'ki-escape-room-props',
  'ki-vinyl-dj-deep',
  'ow-wave30-clubs',
  'ki-martial-dojo',
  'ow-wave31-outdoors',
  ...
  'ki-candle-soap',
  'ki-brick-build',
  // aggro wave12 — majority discovered-new
  'ki-archery-range',
  'ki-sewing-fashion-deep',
  'ki-gardening-tools',
  // aggro wave13
  'ki-surfing-beach',
  'ki-rock-climbing',
