/**
 * Poll/import CEFR-J B1 adjectives.
 * Per-task import as soon as a task is ready. Optionally fill ≤3 createTask slots.
 *
 *   node scripts/manus/b1-adjectives-poll-import.mjs
 *   node scripts/manus/b1-adjectives-poll-import.mjs --once
 *   node scripts/manus/b1-adjectives-poll-import.mjs --orchestrate --maxMinutes=180
 *   node scripts/manus/b1-adjectives-poll-import.mjs --task=1
 */
import fs from 'fs';
...
const INBOX = path.join(ROOT, 'assets-inbox', 'manus-cefrj-adjectives-b1');
const STATE_PATH = path.join(ROOT, 'tmp', 'cefrj-manus', 'b1-adjectives-loop-state.json');
const LOCK_PATH = path.join(ROOT, 'tmp', 'cefrj-manus', 'b1-adjectives-import.lock');
...
function acquireLock() {
  ...
  try {
    ...
    fs.writeFileSync(
      LOCK_PATH,
      JSON.stringify({ at: new Date().toISOString(), pid: process.pid, owner: 'b1-adjectives-poll-import' }, null, 2)
    );
    return true;
  } catch {
    return false;
...
function importTaskThemes(task, themes) {
  const r = spawnSync(
    process.execPath,
    [
      'scripts/manus/import-cefrj-adjectives-b1.mjs',
      `--task=${task.taskNo}`,
...
function fillSlots(state) {
  ...
  const args = [
    'scripts/manus/request-shift60-cefrj-adjectives-b1.mjs',
    '--send',
    '--fill-slots',
    '--maxInFlight=3',
  ];
...
async function tick(state) {
  ...
  for (const task of tasks) {
    ...
    try {
      console.log(`Importing adjectives B1 (task ${task.taskNo})…`);
  ...
  fs.writeFileSync(
    path.join(ROOT, 'tmp', 'cefrj-manus', 'b1-adjectives-poll-snapshot.json'),
    JSON.stringify(
      {
