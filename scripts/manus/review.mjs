...
export function pickImages(dir, opts = {}) {
  ...
  // Prefer review.json pageCount when present — ignore stale page-N orphans
  // left after shorter rebakes (Manus UX fruit/face/dentist false duplicate wrap).
  let pageCap = Number(opts.pageCount);
  if (!(pageCap > 0)) {
    try {
      const reviewPath = path.join(dir, 'review.json');
      if (fs.existsSync(reviewPath)) {
        const review = JSON.parse(fs.readFileSync(reviewPath, 'utf8'));
        pageCap = Number(review.pageCount)
          || (Array.isArray(review.pageKeys) ? review.pageKeys.length : 0);
      }
    } catch {
      pageCap = 0;
    }
  ...
  const withinCap = (n) => {
    if (!(pageCap > 0)) return true;
    const m = /^page-(\d+)/i.exec(n);
    if (!m) return true;
    return Number(m[1]) < pageCap;
  };
  ...
  const byRole = (role) => {
    ...
    const hits = names.filter((n) => withinCap(n) && (re.test(n) || pageRe.test(n)));
  ...
  const storyHits = names
    .filter((n) => withinCap(n) && /^page-\d+-story\d+\.(jpe?g|png)$/i.test(n))
  ...
  const rest = names
    .filter((n) => withinCap(n) && !chosen.includes(n))
    .sort((a, b) => pageNum(a) - pageNum(b) || a.localeCompare(b, undefined, { numeric: true }));
  for (const n of rest) {
