/**
 * Contact-sheet preview for story-cast who matrix (21 = 7 poses × 3 emotions).
 *
 *   node scripts/manus/preview-story-cast-contact.mjs --who=leo
 *   node scripts/manus/preview-story-cast-contact.mjs --who=mia
 */
import fs from 'fs';
...
const who = String(arg('who', 'leo')).toLowerCase();
const OUT = path.join(ROOT, 'tmp', `manus-story-cast-${who}-pilot`, `${who}-cast-21-preview.jpg`);

const cells = [];
for (const emotion of EMOTIONS) {
  for (const pose of POSES) {
    const key = `cast-${who}-${pose}-${emotion}`;
...
const dataUrl = await page.evaluate(async () => {
  ...
  const titleWho = window.WHO.charAt(0).toUpperCase() + window.WHO.slice(1);
  ctx.fillText(`${titleWho} story cast pilot — 7 poses × 3 emotions (who:${window.WHO})`, 16, 34);
  ctx.font = '12px system-ui';
  ctx.fillStyle = '#aaa';
  ctx.fillText('pack: story-cast · hold = empty hands · viewer-right · black-field', 16, 50);
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      ctx.textAlign = 'center';
      ctx.fillText(cell.key.replace(`cast-${window.WHO}-`, ''), x + cellW / 2, y + cellH + 14);
      ctx.textAlign = 'left';
    }
  }
