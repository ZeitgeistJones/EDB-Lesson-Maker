/**
 * Aggressive S2 leftover quiet lesson-stage harvest.
 *   node scripts/manus/request-aggressive-s2-settings-leftover.mjs --wave=office --fire
 *   node scripts/manus/request-aggressive-s2-settings-leftover.mjs --wave=office --poll-only
 *
 * Does not remop parked space/volcano/castle/concert-hall.
...
import {
  ...
  PREFIX,
  PARKED_WAVE_IDS,
} from './aggressive-s2-settings-leftover-keys.mjs';
...
const STYLE_LOCK = `EDB SETTING DROP — quiet recognizable lesson-stage (${BOARD.width}×${BOARD.height}):

WHAT WE WANT:
- Everyday place a teacher names in 1 second (office / greenhouse / camp / etc.)
- Walls/floor/sky; furniture silhouettes at EDGES only
- Open center interaction floor (horizontal ~20%–80%, lower third) for dragging props
- Clear ground plane (groundY ${GROUND_Y_RANGE} px)
- Soft children's-book ClassIn ESL illustration — quiet stage, not cinematic wallpaper
- Generic empty setting — no story plot

HARD FAIL:
- Abstract quiet-flat wash + tiny corner glyph only (docs/bg-theme-sets.md anti-room flats)
- Furnished room with a cleared desk strip (busy interior as the hero)
- Decorative scenic poster / fantasy movie still / busy illustration / no usable floor
- People, faces, animals, readable text, letters, numbers, logos, key labels
- Black-field prop cutouts
- Cinematic lighting, volcano/castle/space drama
...
async function createTaskBackoff(payload) {
  let wait = RATE_WAIT_MS;
  try {
    return await createTask(payload);
  } catch (err) {
    if (!is429(err)) throw err;
    console.log(JSON.stringify({ phase: 'rate-limit-create', wait_ms: wait, retry: 1 }, null, 2));
    await sleep(wait);
    try {
      return await createTask(payload);
    } catch (err2) {
      if (!is429(err2)) throw err2;
      wait *= 2;
      console.log(JSON.stringify({ phase: 'rate-limit-create', wait_ms: wait, retry: 'backoff-stop' }, null, 2));
      await sleep(wait);
      throw err2;
    }
  }
...
function liveRun(runPath) {
  ...
  return Boolean(prev.task_id) && st !== 'stopped' && st !== 'error' && !prev.finished_at;
}

function anyS2Inflight(exceptId) {
  const hits = [];
  if (fs.existsSync(STOCKPILE)) {
    for (const name of fs.readdirSync(STOCKPILE)) {
      if (name === exceptId) continue;
      if (liveRun(path.join(STOCKPILE, name, 'run.json'))) hits.push(name);
    }
  }
...
async function runWave(waveKey) {
  if (!waveKey) throw new Error(`Need --wave=${WAVE_ORDER.join('|')}`);
  const { wave, setting } = resolveSetting(waveKey);
  if (PARKED_WAVE_IDS.includes(wave.id)) {
    console.error(JSON.stringify({ phase: 'refuse-new-fire', reason: 'parked-cinematic', id: wave.id }));
    process.exit(2);
  }
  const taken = readDedupedSlugs();
  if (taken.has(setting.slug)) {
    console.error(JSON.stringify({ phase: 'refuse-new-fire', reason: 'deduped-place', slug: setting.slug }));
    process.exit(2);
  }
  ...
  const dump = {
    ...
    kind: 'aggressive-s2-settings-leftover',
    wave: wave.id,
    setting: setting.slug,
    sheet_dir: SHEET_DIR,
    variant_count: setting.variants.length,
  ...
  if (!pollOnly) {
    ...
    const created = await createTaskBackoff({
      title: setting.title,
