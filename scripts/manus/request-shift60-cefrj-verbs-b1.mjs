/**
 * Shift60 — CEFR-J B1 verbs via Manus.
 * Source: tmp/cefrj-manus/b1/manus-verbs-b1.txt
 ...
 * Import as picturable-verbs wave=13.
 *
 *   node scripts/manus/request-shift60-cefrj-verbs-b1.mjs           # dry-run / plan
 *   node scripts/manus/request-shift60-cefrj-verbs-b1.mjs --send
 *   node scripts/manus/request-shift60-cefrj-verbs-b1.mjs --send --task=1
 *   node scripts/manus/request-shift60-cefrj-verbs-b1.mjs --send --fill-slots
 *     # create next unsent tasks until ≤3 sent-not-yet-marked-done (see --done-tasks=)
 */
import fs from 'fs';
...
const QUEUE_PATH = path.join(ROOT, 'tmp', 'cefrj-manus', 'b1', 'manus-verbs-b1.txt');
...
const OUT_ROOT = path.join(ROOT, 'tmp', 'manus-shift60-picturable-verbs-wave13');
const CELLS = 9;
...
function buildSheets(rows) {
  ...
  for (let i = 0; i < rows.length; i += CELLS) {
    ...
    sheets.push({
      id: `S${n}`,
      theme: `cefrj-verbs-b1-${String(n).padStart(2, '0')}`,
      title: `CEFRJ VERBS B1 ${n}`,
...
LOOK: rich ESL kid-illustration cutouts with soft shading — NOT emoji-flat sticker glyphs, NOT featureless silhouettes, NOT UI symbol metaphors when a clear action scene or prop works. Anti-iconey: soft depth, colorful objects on pure white.

This is CEFR-J B1 verbs task ${taskNo}/${taskCount} (Shift60; import wave ${WAVE}). Keep working until EVERY listed sheet PNG exists (Perfect-${n} multi-call — one image call per sheet, all in THIS task).

Filename each PNG with the theme slug in the name (e.g. esl_cefrj_verbs_b1_sheet_01_cefrj-verbs-b1-01.png). Prefer one zip of all PNGs plus CDN links in chat.
  ...
  for (const ent of fs.readdirSync(OUT_ROOT, { withFileTypes: true })) {
    ...
    const normalized = (j.sheets || []).map((s) => {
      if (Array.isArray(s.cells) && s.cells.length) {
        return { id: s.id, theme: s.theme, title: s.title || s.theme, cells: s.cells, incomplete: !!s.incomplete };
      }
      const keys = s.keys || [];
      return {
        id: s.id,
        ...
        cells: keys.map((k) => [k, k + ' action — kid-friendly figure and/or hands+object', k]),
...
function selectTasksToFire(tasks) {
  ...
  if (!FILL_SLOTS) {
    // Full send: still cap initial create burst at MAX_IN_FLIGHT unless --task=
...
async function fireTask(task, taskCount, meta) {
  ...
  const created = await createTask({
    title: `ESL white vocab 3×3: Shift60 CEFR-J B1 verbs t${task.taskNo}/${taskCount} (${task.sheets.length} sheets)`,
    message: brief,
    agent_profile: profile,
    force_skills: force,
...
let rawCount, kept, dropped, sheets, tasks;
if (FILL_SLOTS) {
  ...
  console.log(JSON.stringify({ phase: 'frozen-plan', tasks: tasks.length, keys: rawCount, taskNos: tasks.map((t) => t.taskNo) }));
} else {
