/**
 * A1 visual operating-system harvest (P0 then P1).
 * Stockpile only — no import / keying / wiring.
 *
 *   node scripts/manus/request-a1-p0-harvest.mjs --wave=1 --fire
 *   node scripts/manus/request-a1-p0-harvest.mjs --wave=1 --poll-only
 *
 * Sheets: tmp/manus-a1-stockpile/<wave-id>/sheets/
 * Inventory: tmp/manus-a1-stockpile/inventory.json
 * Tracked copy: docs/a1-stockpile-inventory.json
 */
import fs from 'fs';
...
function sheetBlock(sheet, index) {
  ...
  const format =
    sheet.format === 'landscape-shells'
      ? 'landscape 16:9 board/shell PNGs, one listed shell per PNG'
...
function buildBrief(wave, sheets) {
  return withEslAssetGeneratorBrief(`TASK: Produce **${sheets.length} A1 visual operating-system PNG sheet groups** for ClassIn ESL.

This is A1 CONTROLLED RECOMBINATION, not another vocab-image wave. The deliverable is reusable visual infrastructure: slots, rails, boards, cues, badges, docks, response surfaces, and text-ready shells.

${wave.style}

HARD RULES:
- Reading order left to right, top to bottom for any contact sheet.
- For board/shell sheets: make each listed key its own landscape 16:9 PNG board/shell with empty functional space.
- For black-contact sheets: one concept per cell, pure #000000 black field, clear gutters, nothing crossing cell boundaries.
- NO readable text, fake writing, labels, question words, sentences, numbers, menus, profile fields, or baked English.
- Relationship must be visible and manipulable; stable viewpoint/scale/framing for paired states.
...
function emptyInv() {
  return {
    spec: 'a1-visual-operating-system',
    updated_at: null,
    running_total: {
      waves_fired: 0,
      waves_downloaded: 0,
...
function writeSpec() {
  const spec = {
    spec: 'a1-visual-operating-system',
    locked_level: 'A1 controlled recombination',
    firewall: [
      'no baked English sentences',
      'no fake text in micro-text shells',
      'no A2 past narratives',
      'no route giving',
      'no shopping negotiations',
      'no because-reasoning',
      'no paragraphs',
      'no open roleplay',
    ],
    waves: Object.fromEntries(
      Object.values(WAVES).map((w) => [
        w.id,
        { phase: w.phase, family: w.family, title: w.title, sheets: w.sheets.length, concepts: conceptCount(w) },
      ]),
    ),
  };
...
const NEED_IMAGES = sheets.reduce((n, s) => n + s.cells.length, 0);
...
const dump = {
  ...
  kind: 'a1-visual-operating-system',
  wave: wave.id,
  phase: wave.phase,
  family: wave.family,
  sheet_dir: SHEET_DIR,
...
if (large.length < Math.min(NEED_IMAGES, 5)) {
  console.log(JSON.stringify({ phase: 'need-more-images', have: large.length, need_at_least: Math.min(NEED_IMAGES, 5) }, null, 2));
  await sendMessage(taskId, {
    force_skills: [MANUS_SKILLS.ESL_ASSET_GENERATOR],
    message: withEslAssetGeneratorBrief(
      `Continue THIS task. You returned ${large.length} usable PNG(s); keep generating the missing A1 visual operating-system PNGs from the original brief. Do not restart. Do not add text. Do not change the key list.`,
    ),
  });
