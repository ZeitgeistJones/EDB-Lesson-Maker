/**
 * Visual-grammar stockpile manufacturing.
 * Stockpile only: no PropBank merge, producer wiring, renderer changes.
 *
 *   node scripts/manus/visual-grammar-keys.mjs
 *   node scripts/manus/request-visual-grammar-harvest.mjs --wave=vg1 --fire
 *   node scripts/manus/request-visual-grammar-harvest.mjs --wave=vg2 --fire
 *   node scripts/manus/request-visual-grammar-harvest.mjs --wave=vg1 --poll-only
 *
 * Sheets: harvested/manus-visual-grammar-stockpile/<wave-id>/sheets/
 */
import fs from 'fs';
...
function buildBrief(wave, sheets) {
  return withEslAssetGeneratorBrief(`TASK: Produce **${sheets.length} visual-grammar black-field PNG contact sheet(s)** for ClassIn ESL.

SOURCE OF TRUTH: scripts/manus/visual-grammar-keys.mjs, approved MANUS_WORTHY only. This is reusable visual grammar of interaction / stagecraft, NOT more vocab content art.
...
- Overlay atoms must stay compact and keyable. Surfaces must keep empty functional space.
- State variants must preserve object identity, viewpoint, scale, and baseline.
- quality: default ONLY.
- Keep generating inside THIS task until every listed PNG contact sheet exists. The 5-image cap is per generate_image call, not per task.
  ...
  if (large.length < NEED_SHEETS) {
    ...
    await sendMessage(taskId, {
      ...
      message: withEslAssetGeneratorBrief(
        `Continue THIS task. You returned ${large.length} usable PNG sheet(s); we need exactly ${NEED_SHEETS} visual-grammar black-field sheet(s) listed in the original brief. Do not restart. Do not add text. Do not change the key list. Generate only the original MANUS_WORTHY concepts. Keep firing generate_image until every listed sheet exists.`,
      ),
    });
