/**
 * Importer for CEFR-J A1/A2 nouns wave1 Manus drops (white 3×3 vocab).
 *
 * Reads expected keys from tmp/manus-shift60-cefrj-nouns-wave1/taskN/run.json,
 * matches inbox PNGs by theme slug (preferred) or sheet order fallback,
 * imports via scripts/import-vocab-sheet.mjs into public/assets/07_vocab-pack/.
 *
 *   node scripts/manus/import-cefrj-nouns-wave1.mjs
 *   node scripts/manus/import-cefrj-nouns-wave1.mjs --dry-run
 *   node scripts/manus/import-cefrj-nouns-wave1.mjs --task=1
 *   node scripts/manus/import-cefrj-nouns-wave1.mjs --inbox=assets-inbox/manus-cefrj-nouns-w1
 */
import fs from 'fs';
...
const PLAN_ROOT = path.join(ROOT, 'tmp', 'manus-shift60-cefrj-nouns-wave1');
const DEFAULT_INBOX = path.join(ROOT, 'assets-inbox', 'manus-cefrj-nouns-w1');
const TASK_NOS = [1, 2, 3, 4, 5, 6];
...
function scorePngForTheme(pngPath, theme, sheetId) {
  ...
  let score = 0;
  // Numbered CEFR-J themes must match exact nouns-NN / verbs-NN (never "any cefrj-nouns-*").
  const numbered = themeSlug.match(/^(cefrj-(?:nouns|verbs)-)(\d+)$/);
  if (numbered) {
    const n = numbered[2];
  ...
  if (m) {
    ...
    if (base.includes(`nouns-${n.padStart(2, '0')}`) || base.includes(`nouns-${n}`)) {
      score += 40;
    }
  }
  return score;
...
function loadJobs() {
  ...
  for (const taskNo of TASK_NOS) {
    ...
    // Prefer task subdir only — never fall back to a shared flat inbox (cross-task contamination).
    const pngs = listPngs(taskInbox);
    const used = new Set();
    const orderedFallback = []; // disabled: order fallback without theme match caused wrong-sheet imports
    for (let i = 0; i < sheets.length; i++) {
      ...
      let png = pickPng(pngs, { ...sheet, id }, used);
      if (!png && orderedFallback[i] && !used.has(orderedFallback[i])) {
        png = orderedFallback[i];
        used.add(png);
      }
      if (!png) {
    ...
    for (const png of pngs) {
      if (!used.has(png) && listPngs(taskInbox).includes(png)) {
        unmatched.push({
          taskNo,
          id: 'ORPHAN-PNG',
...
function main() {
  ...
  const outDir = path.join(ROOT, 'tmp', 'manus-shift60-cefrj-nouns-wave1');
