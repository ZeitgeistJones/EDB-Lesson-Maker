...
async function main() {
  ...
  // Rows collected here and replayed onto a fresh manifest read under the
  // lock at the end — the keying loop below is slow (spawns import-prop per
  // crop) and must not hold the manifest lock the whole time.
  ...
  if (newRows.size) {
    await mutateManifest(MANIFEST, (m) => {
      for (const [name, row] of newRows) m.props[name] = row;
    });
  }
