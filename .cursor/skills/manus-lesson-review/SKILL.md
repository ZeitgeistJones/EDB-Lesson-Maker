...
| Field | Purpose |
|-------|---------|
| `knownIssues` | Optional context — what we've been working on / intentional |
| `justFixed` | Optional — please verify these if you can → `just_fixed_results` |
| `localChecks` | Optional — what we already tried (hints, not skip-lists) |
| `focus` | Optional emphasis tilt — never “ignore everything else” |
...
## Standing review bar (baked into every brief)

`buildReviewBrief` leads every pass-off with holistic judgment — not a skip catalog:

1. **Primary ask.** “What do you think of this lesson?” — teacher + student eyes on the baked JPGs.
2. **Everything is fair game.** Pedagogy, visual/UX, overlay, match honesty, timing, aims, backgrounds,
   completeness, delivery, mechanical misses — no category off-limits. Structured fields
   (`scorecard`, `gate_holes`, `just_fixed_results`, `weakest_link`, `escalation_homework`, …)
   are **output** requirements for foldability, not don’t-look rules.
3. **Optional context only.** `knownIssues` / `justFixed` / `localChecks` / `focus` = light “we’ve been
   working on X / please verify Y” — never “do not re-report” / “do not spend score on.”
4. **Anti-inflation + escalation_homework.** Still required — they *add* feedback quality; they do not
   shrink the lens. Known-intentional items: escalate to blocking only if THIS bake is worse.
