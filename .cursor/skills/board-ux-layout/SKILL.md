...
### New Words (picture beside word)

- **Picture beside each word** on the card (row: image | label) — full-width
  grid. Never a bare word list + separate “Remember these words” / picture-bin
  side collage (Sports-and-Games failure).
- `pageShell({ reserveDock: false })` for vocab — no side dock chrome.
- Prefer keyed PropBank alpha over white-plate `07_vocab-pack` icons
  (`VocabArt` preferProp when `isDockSharp`).
- Glyph fallbacks: **no purple/white chip** behind the emoji — transparent like
  props (colored plates read as pack icons on New Words).
- Word cards: CSS grid `1fr` rows + `flex:1` / `height:100%` so cards fill.
...
- **Frames densify (4–5 rows):** keep `line-height ≥ 1.4` and never use
  `overflow:hidden` on the frame text — dense `1.25` clipped blanks/descenders
  on Sports Frames. Prefer slightly smaller `fontPx` over clipping.
- **Vocab sentences:** when 6 board words, cap **2** example lines per card so
  the third line does not spill out of the cell. Prefer the same VocabArt thumb
  (prop/glyph) beside the word — no empty checkerboard dead zone.
...
- Bare word list + separate “Remember / Picture bin” side collage on New Words
  (picture must sit beside each word on the card).
- Bottom `reserveDock` on vocab (crushes word cards into a top strip).
- Shipping 07_vocab-pack white-plate icons when a dock-sharp prop or curated
  glyph exists (or as a twin fallback after glyph collision).
...
- Sports/games title charm that reads as flute / relay-baton / music stick.
